...
function buildManifest() {
  ...
  const rows = fr.rows.filter((row) => row.english.id !== "remittance-v2").map((row) => {
    ...
    return {
      ...
      culturalScope: row.english.id === "mobile-money-fees" ? "Official published MTN Uganda and Airtel Tanzania tariff bands with explicit unavailable states, plus a local user-entered quote comparator." : row.english.id === "burial-cost" ? "Respectful family funeral budget using only user-entered costs; no country, faith, ceremony, provider or insurance price assumptions." : row.english.id === "japa-calculator" ? "User-entered relocation budget with verified official fees and quotes; no embedded visa, eligibility, FX, price, tax or legal claims." : row.english.id === "brideprice-advisor" ? "Consent-first family conversation and user-entered affordability plan; no cultural price list, average, authenticity claim or universal custom." : row.culturalScope,
      countryCodes: row.english.id === "mobile-money-fees" ? ["UG","TZ"] : row.countryCodes,
      ...
      dataOwner: row.english.id === "mobile-money-fees" ? "data/fintech/mobile-money-tariffs.json + data/fintech/official-sources.json + user-entered-quote-receipts" : row.english.id === "burial-cost" ? "user-entered-confirmed-family-costs" : row.english.id === "japa-calculator" ? "user-entered-verified-official-fees-and-local-quotes" : row.english.id === "brideprice-advisor" ? "user-entered-consent-and-agreed-budget" : row.dataOwner,
