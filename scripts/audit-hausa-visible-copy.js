...
const REPORT_JSON = path.join(ROOT, "reports", "hausa-visible-copy-ledger.json");
const REPORT_MD = path.join(ROOT, "reports", "hausa-visible-copy-ledger.md");
...
const ACCEPTED_CONTEXT_TERMS = [
  "AfroTools", "Hausa", "Yoruba", "Swahili", "Igbo", "Amharic", "Pidgin",
  "Nigeria", "Najeriya", "Naira", "WhatsApp", "MTN", "Airtel", "Glo",
  "9mobile", "Paystack", "Flutterwave", "OPay", "M-Pesa", "Kenya", "Ghana",
  "Tanzania", "Rwanda", "South Africa", "Africa", "Afrika",
];
...
const FALSE_POSITIVE_PATTERNS = [
  ...
  { label: "English route label marked as fallback", re: /\bShafin Turanci\b/i },
  { label: "language name in adjacent-language card", re: /\b(?:Yoruba|Swahili|Igbo|Amharic|Pidgin) Translator\b/i },
...
function ownerBatch(route) {
  if (route === "/ha/" || route === "/ha/kayan-aiki/" || route === "/ha/najeriya/") return "Batch 2 - Hausa hub visible-copy cleanup";
  if (route.includes("albashi") || route.includes("haraji") || route.includes("vat")) return "Batch 2 - Salary/tax and VAT visible-copy cleanup";
  if (route.includes("jamb") || route.includes("ilimi") || route.includes("waec") || route.includes("neco") || route.includes("nysc") || route.includes("makaranta")) return "Batch 2 - Education and JAMB visible-copy cleanup";
  if (route.includes("takardu") || route.includes("pdf") || route.includes("invoice") || route.includes("resit") || route.includes("naira")) return "Batch 2 - Document, PDF, invoice, and Naira cleanup";
  if (route.includes("sadarwa") || route.includes("ussd") || route.includes("whatsapp")) return "Batch 2 - Telecom and USSD visible-copy cleanup";
  if (route.includes("harshe") || route.includes("fassara")) return "Batch 2 - Language and translation hub cleanup";
  if (route.includes("noma") || route.includes("gona") || route.includes("taki") || route.includes("rogo") || route.includes("kwandon-kasuwa")) return "Batch 2 - Agriculture visible-copy cleanup";
  if (route.includes("lafiya") || route.includes("asibiti") || route.includes("magani") || route.includes("genotype") || route.includes("sickle") || route.includes("haihuwa") || route.includes("abincin")) return "Batch 2 - Health and family visible-copy cleanup";
  return "Batch 2 - General Hausa visible-copy cleanup";
}
...
function buildMarkdown(report) {
  ...
  lines.push("# Hausa Visible Copy Leakage Ledger");
  lines.push("");
  lines.push(`Generated: ${report.generatedAt.slice(0, 10)}`);
  lines.push("");
  lines.push("This audit scans `ha/**/*.html` for visible English leakage only. It ignores scripts, styles, head metadata, JSON blobs, URLs, code/pre blocks, and accepted technical acronyms.");
  lines.push("");
  lines.push("## Headline Metrics");
  lines.push("");
  lines.push(`- Hausa routes scanned: ${counts.routesScanned}`);
...
function buildReport() {
  ...
  if (!recommendedCleanupOrder.length) {
    recommendedCleanupOrder.push("No blocker cleanup needed from this audit.");
  }
...
function main() {
  ...
  console.log(`Hausa visible-copy audit scanned ${report.counts.routesScanned} route(s).`);
