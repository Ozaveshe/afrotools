...
function buildHubSummaryHtml(status) {
  ...
  return [
    '<div class="gov-source-stats" aria-label="Government source monitor status">',
    `  <div class="gov-source-stat"><strong>${escapeHtml(dateLabel)}</strong><span>Last source check</span></div>`,
    `  <div class="gov-source-stat"><strong>${status.summary.sourceCount}</strong><span>Official sources watched</span></div>`,
    `  <div class="gov-source-stat"><strong>${status.summary.changedSources}</strong><span>Source changes queued</span></div>`,
    `  <div class="gov-source-stat"><strong>${escapeHtml(currentLabel)}</strong><span>Current review status</span></div>`,
    '</div>'
  ].join('\n');
}
