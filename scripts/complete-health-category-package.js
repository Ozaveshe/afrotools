...
const competitorById = {
  ...
  'bmi-calculator': ['NHS BMI calculator', 'NHS asks for height, weight, ethnicity context, waist follow-up, and clear limits.', 'Added a saveable BMI plan with waist follow-up, private device storage, and local PDF export.', sourceBank.bmi],
  ...
  'hospital-cost': ['Healthcare Bluebook', 'Price tools need facility quotes, benchmark comparison, and what-is-included prompts.', 'Added care-cost workflow, quote-proof prompts, and a private on-device PDF plan.', sourceBank.bluebook],
  ...
  'blood-group': ['Blood compatibility references', 'Compatibility tools need transfusion and pregnancy Rh-factor caveats.', 'Added labs workflow handoff, clinic-question prompts, and local PDF export.', sourceBank.antenatal],
  ...
  'cholera-risk': ['WHO cholera guidance', 'Outbreak checklists need water, sanitation, dehydration, and urgent-care escalation.', 'Added clinical-safety workflow with a private local PDF checklist.', sourceBank.cholera],
...
function ensureWorkflowScript(html) {
  ...
  if (html.includes('/assets/js/lazy-analytics.js')) {
    return html.replace(/<script\s+defer\s+src="\/assets\/js\/lazy-analytics\.js"/, `${script}\n<script defer src="/assets/js/lazy-analytics.js"`);
  }
...
function buildActionKit(tool, registryById) {
  const bucket = bucketById[tool.id] || 'health';
  const journey = journeyByBucket[bucket] || journeyByBucket.vitals;
  const meta = competitorById[tool.id] || ['Comparable health app', 'Useful health apps turn a single result into a saveable next-step plan.', 'Added private device save, local PDF export, and related Health workflow routing.', sourceBank.diet];
  ...
  return [
    ...
    `<p>This app now has its own benchmarked improvement layer, private device save, local PDF plan, and a route into the ${escHtml(journey.label)} workflow.</p>`,
    ...
    '</div>',
    '<div class="health-action-card">',
    '<h3>Private save and PDF actions</h3>',
    '<ul>',
    '<li>Save this health plan privately on this device.</li>',
    '<li>Download a PDF locally for follow-up and visit prep. No email or account is required.</li>',
    '<li>Optional account sync is a separate, explicit metadata-only action; Health values stay on this device.</li>',
    '</ul>',
    '<div class="health-action-buttons">',
    `<button type="button" class="health-workflow-btn" data-health-action="save" data-health-tool-id="${attr(tool.id)}">Save on this device</button>`,
    `<button type="button" class="health-workflow-btn secondary" data-health-action="pdf" data-health-tool-id="${attr(tool.id)}">Download PDF</button>`,
    `<button type="button" class="health-workflow-btn ghost" data-health-action="sync" data-health-tool-id="${attr(tool.id)}">Save metadata to account (optional)</button>`,
    '</div>',
    '</div>',
    '</div>',
...
function homepageBlock() {
  return [
    ...
    '<p class="ss">Every Health app now has competitor-informed improvements, a private device-save action, and a local PDF plan where a user naturally needs a portable record. Pick a workflow below, then move across related tools without losing the thread.</p>',
    '<div class="health-journey-panel">',
    '<h3>Health workflow builder</h3>',
    '<p>Choose a journey to open the first app, save privately on this device, or download a local PDF checklist without an email or account gate.</p>',
...
function writeDoc(healthTools) {
  ...
  const content = [
    ...
    '- Added a shared Health workflow runtime for private device saves, ungated local PDF plans, optional metadata-only account sync, and cross-tool continuation.',
    '- Improved generated Health app form semantics with names, required fields, input modes, and result snapshots for saved plans.',
    '- Kept Health values local while making account sync a separate, explicit, metadata-only action.',
    '',
    'App-by-app pass:',
    '',
    ...
    '',
    '- `assets/js/health-workflow.js` stores Health plans only after an explicit device-save action in `localStorage.afro_health_plans`; PDF download does not create a saved record.',
    '- PDF export uses the vendored `/assets/vendor/jspdf/jspdf.umd.min.js` runtime, downloads locally, and does not require email, account, lead capture, or workspace sync.',
    '- Optional account sync sends non-clinical metadata only after a separate confirmation. Health values, results, questions, and PDF content remain on the device.',
