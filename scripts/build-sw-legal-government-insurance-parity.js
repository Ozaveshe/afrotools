...
function head(app, category, imageId) {
  ...
  ${app.frenchRoute ? `<link rel="alternate" hreflang="fr" href="${SITE}${app.frenchRoute}">` : ''}
...
<html lang="sw"><head>
  ...
  <main class="mp-shell" data-sw-legal-property-app data-english-id="${esc(contract.englishId)}">
    <nav class="mp-breadcrumb" aria-label="Mfuatano"><a href="/sw/">Mwanzo</a> / <a href="/sw/nyumba-na-ardhi/">Nyumba na ardhi</a> / ${esc(contract.name)}</nav>
...
module.exports = { LEGAL, GOVERNMENT, INSURANCE, legalContracts, governmentConfig };
