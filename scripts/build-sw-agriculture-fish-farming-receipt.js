...
const FAMILY = 'fish-farming';
const manifest = require('../data/localization/sw-agriculture-parity-manifest.json');
const contract = require('./lib/sw-agriculture-family-contracts/fish-farming');
const oraclePath = path.join(ROOT, 'reports/sw-agriculture-fish-farming-oracles.json');
const browserPath = path.join(ROOT, 'reports/sw-agriculture-browser-raw/fish-farming-playwright.json');
const receiptPath = path.join(ROOT, 'reports/sw-agriculture-acceptance/fish-farming.json');
const artworkPath = path.join(ROOT, 'reports/sw-agriculture-fish-farming-missing-artwork.json');
...
for (const spec of collectSpecs(browser)) {
  const match = String(spec.title || '').match(/^(fish-farming-(?:roi|[a-z-]+)) full route local proof$/);
...
const accepted = rows.map(row => {
  ...
  requireTrue(html.includes('FAO SOFIA 2024 — Hali ya Uvuvi na Ufugaji wa Samaki Duniani</a>'), `${row.english.id} FAO named source link is missing.`);
  requireTrue(html.includes('WorldFish Center</a>'), `${row.english.id} WorldFish named source link is missing.`);
  ...
  if (row.country) {
    ...
    requireTrue(html.includes(source.source), `${row.english.id} exact source list is missing.`);
    requireTrue(html.includes(`${source.dataReviewed}; si data ya moja kwa moja.`), `${row.english.id} freshness is missing.`);
  } else requireTrue(html.includes(`${contract.DATA_REVIEWED}; si data ya moja kwa moja.`), `${row.english.id} hub freshness is missing.`);
  return {
    ...
    state: 'accepted',
    evidence: {
      staticOracle: row.country ? 'independent formula/data/currency oracle matched the shared English aquaculture ROI engine' : '15-country hub route/source/artwork proof',
      ...
      seo: ['canonical', 'Open Graph URL', 'WebApplication schema in sw', 'reciprocal en/sw hreflang'],
...
const receipt = {
  schemaVersion: 2, family: FAMILY, locale: 'sw', baseSha: BASE_SHA,
  candidateState: 'accepted-in-family-lane-pending-coordinator-integration',
  reconstructedFromPreservedCandidate: 'c3b232bb7f99ef677360a353d994d47e1a1fb7eb',
  ...
  evidenceOwners: {
    ...
    contract: 'scripts/lib/sw-agriculture-family-contracts/fish-farming.js',
    controller: 'assets/js/pages/sw-agriculture-fish-farming.js',
    engine: 'engines/src/aquaculture-roi-engine.js', data: 'data/agriculture/aquaculture-data.js',
    staticTest: 'tests/sw-agriculture-fish-farming-family.test.js',
    browserTest: 'tests/e2e/sw-agriculture-fish-farming-family.spec.js',
    browserConfig: 'tests/playwright.sw-fish-farming.config.js',
  ...
  invalidProof: { countryRoutes: 15, boundariesPerRoute: 15, assertions: 225 },
  ...
  scopeGuards: {
    ...
    frenchEdited: false, otherLocaleEdited: false, sitemapEdited: false, distEdited: false, deployEdited: false
  },
  integrationFollowUp: {
    required: true,
    reason: 'The coordinator requested an English-and-Swahili-only candidate. The global validator requires the connected English/French/Swahili/Hausa equivalence group to be a complete reciprocal mesh.',
    commandAfterIntegration: 'node scripts/build-sw-agriculture-family.js --family fish-farming --full-mesh',
    expectedMetadataOnlyChanges: {
      swahiliRoutesRegeneratedWithFullMesh: 16,
      frenchCountryPagesReceivingSwAlternate: 15,
      hausaPagesReceivingSwAlternate: ['ha/kayan-aiki/ribar-kiwon-kifi/index.html']
    },
    currentGlobalHreflangGate: {
      state: 'blocked-by-scoped-locale-boundary',
      missingReciprocalRelationships: 33
    },
    staleDerivedArtifacts: [
      ...
      'reports/swahili-free-app-parity-inventory.json',
      'reports/swahili-free-app-parity-inventory.md',
      'Swahili product-surface stamps for the 16 Fish Farming routes'
    ],
    coordinatorOrder: [
      'Integrate this candidate and update the central acceptance ledger.',
      'Run node scripts/build-sw-agriculture-family.js --family fish-farming --full-mesh.',
      'Run npm run sw:surface:build.',
      'Run npm run sw:parity:build and npm run sw:ai-routes:build.',
      'Regenerate localization coverage through its source owner.',
      'Rerun npm run validate:hreflang and the Swahili parity/surface/AI checks.'
    ]
  }
};
