...
const acceptedIds = new Set(['color-picker', 'colour-palette', 'image-crop', 'image-format-convert', 'image-resize', 'qr-generator', 'watermark-bulk']);
...
expect(accepted.length === 7, `accepted candidate count drifted: expected 7, found ${accepted.length}`);
expect(blocked.length === 46, `blocked count drifted: expected 46, found ${blocked.length}`);
...
const receipt = {
  ...
  browserMatrix: {
    engine: 'Chromium',
    workers: 1,
    isolatedPorts: [4398, 4401, 4404, 4405, 4406],
    widths: [320, 375, 640],
