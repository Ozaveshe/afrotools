...
const SW_FENCE = require("./lib/sw-fence-cost-contract.js");
const SW_POOL = require("./lib/sw-swimming-pool-cost-contract.js");
...
const apps = rows.map((row) => {
  ...
  if (row.englishId === SW_FENCE.id) {
    if (!exists(SW_FENCE.file)) throw new Error("Fence cost owner missing.");
    return {
      englishId: row.englishId, categoryKey: row.categoryKey, englishRoute: row.englishRoute,
      swahiliRoute: SW_FENCE.swRoute, swahiliFile: SW_FENCE.file, status: "accepted-candidate",
      sourceOwner: "scripts/build-sw-fence-cost-parity.js -> assets/js/engines/fence-cost-engine.js -> assets/js/pages/sw-fence-cost-parity.js",
      formulaDecision: "The exact English length, height factor, material, labour, gate, topping, per-metre and duration arithmetic is shared through one DOM-free engine consumed by English and Swahili.",
      sourceDecision: "Exact Engineering ownership is proved by inventory, locale coverage and registry sourceId. Ten-country embedded rates have no date or maintained source ledger and are visibly stale/low-confidence; current written quotes and qualified boundary, structural, electrical and permit review control decisions.",
      browserProof: "Chromium: exact Nigeria and Kenya oracles, invalid/stale clearing, reset, 320px, 375px and 200% reflow, themes, keyboard/focus, reciprocal metadata, no console/page errors or raw-input egress; English shared-engine regression passed.",
      exportProof: "JSON downloaded and parsed; CSV and TXT parsed; PDF downloaded and reopened through the repository-vendored PDF.js parser.", artwork, blocker: null,
    };
  }

  if (row.englishId === SW_POOL.id) {
    if (!exists(SW_POOL.file)) throw new Error("Swimming pool cost owner missing.");
    return {
      englishId: row.englishId, categoryKey: row.categoryKey, englishRoute: row.englishRoute,
      swahiliRoute: SW_POOL.swRoute, swahiliFile: SW_POOL.file, status: "accepted-candidate",
      sourceOwner: "scripts/build-sw-swimming-pool-cost-parity.js -> assets/js/engines/swimming-pool-cost-engine.js -> assets/js/pages/sw-swimming-pool-cost-parity.js",
      formulaDecision: "The exact English area, volume, wall/finish area, construction, finish, equipment, usage, extras, maintenance and cost-per-area arithmetic is shared through one DOM-free engine.",
      sourceDecision: "Exact Engineering ownership is proved by inventory, locale coverage and registry sourceId. Six-country embedded rates have no date or maintained source ledger and are visibly stale/low-confidence; current quotes and qualified excavation, structural, waterproofing, filtration, electrical, barrier and permit review control decisions.",
      browserProof: "Chromium: exact residential and commercial oracles, invalid/stale clearing, reset, 320px, 375px and 200% reflow, themes, keyboard/focus, reciprocal metadata, no console/page errors or raw-input egress; English regression passed.",
      exportProof: "JSON downloaded and parsed; CSV and TXT parsed; PDF downloaded and reopened through the repository-vendored PDF.js parser.", artwork, blocker: null,
    };
  }
...
const receipt = {
  ...
  outcome: {
    ...
    byCategory: {
      engineering: { denominator: 20, acceptedCandidates: 17, blocked: 3 },
  ...
  proof: {
    static: ["tests/swahili-energy-remaining-static.test.js", "tests/swahili-engineering-materials-parity.test.js", "tests/swahili-boq-builder-parity.test.js", "tests/swahili-structural-screening-parity.test.js", "tests/swahili-electrical-load-parity.test.js", "tests/swahili-paint-parity.test.js", "tests/swahili-roof-parity.test.js", "tests/swahili-borehole-parity.test.js", "tests/swahili-generator-sizing-parity.test.js", "tests/swahili-boq-gen-parity.test.js", "tests/sw-property-construction-planning-parity.test.js", "tests/swahili-fence-cost-parity.test.js", "tests/swahili-swimming-pool-cost-parity.test.js", "tests/swahili-transport-static-candidate.test.js", "tests/swahili-transport-cost-parity.test.js"],
    browser: ["tests/e2e/sw-engineering-energy-transport-candidate.spec.js", "tests/e2e/sw-engineering-materials-parity.spec.js", "tests/e2e/sw-boq-builder-parity.spec.js", "tests/e2e/sw-structural-screening-parity.spec.js", "tests/e2e/sw-electrical-load-parity.spec.js", "tests/e2e/sw-paint-parity.spec.js", "tests/e2e/sw-roof-parity.spec.js", "tests/e2e/sw-borehole-parity.spec.js", "tests/e2e/sw-generator-sizing-parity.spec.js", "tests/e2e/sw-boq-gen-parity.spec.js", "tests/e2e/sw-home-renovation-parity.spec.js", "tests/e2e/sw-fence-cost-parity.spec.js", "tests/e2e/sw-swimming-pool-cost-parity.spec.js", "tests/e2e/sw-transport-cost-parity.spec.js"],
    browserMatrix: "54 physical routes at 320px, 375px and 640px/200% reflow; 17 deep Energy workflows; 17 deep Engineering workflows plus English regressions; fleet-fuel, vehicle-operating-cost and truck-load deep Swahili and English regressions; car-import focused invalid/reset/privacy flow.",
...
if (accepted.length !== 37 || blocked.length !== 18) throw new Error(`Expected 37 accepted candidates and 18 blocked; received ${accepted.length}/${blocked.length}.`);
...
Status: **37 accepted candidates / 18 blocked / exact denominator 55**. This receipt does not edit or imply coordinator acceptance.

## Outcome

| Category | Denominator | Accepted candidate | Blocked |
|---|---:|---:|---:|
| Engineering & Construction | 20 | 17 | 3 |
...
- \`swimming-pool-cost\` owns exactly \`/sw/zana/gharama-ya-dimbwi-la-kuogelea/\`. Its six-country legacy rate table is undated and low-confidence; one shared engine preserves the English formula while the UI requires current quotes and qualified excavation, structure, waterproofing, filtration, electrical, barrier and permit review.
...
- Swimming pool cost: exact residential and commercial/extras oracles, invalid/reset and explicit undated-source/stale state; JSON parsed, CSV/TXT parsed and PDF reopened through PDF.js; English shared-engine regression and exact Engineering ownership passed.
...
- Swimming-pool owner/engine/routes: \`scripts/build-sw-swimming-pool-cost-parity.js\`, \`scripts/templates/sw-swimming-pool-cost.template\`, \`assets/js/engines/swimming-pool-cost-engine.js\`, \`assets/js/pages/sw-swimming-pool-cost-parity.js\`, \`tools/swimming-pool-cost/index.html\`, and \`sw/zana/gharama-ya-dimbwi-la-kuogelea/index.html\`.
...
- \`node --test tests/swahili-swimming-pool-cost-parity.test.js\`
- \`npx playwright test -c playwright.sw-swimming-pool-cost.config.js --workers=1\`
...
- \`npm run sw:surface:check\` reports 37 source-owned serial parity routes as stale against the older broad product-surface generator, including the dedicated swimming-pool owner. \`npm run sw:parity:check\` also requests coordinator-owned inventory regeneration after the registry ownership correction. This one-app lane did not overwrite the dedicated route owners or mutate the central inventory.
