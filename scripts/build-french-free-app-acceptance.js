...
function validateCatalog(report, catalog) {
  ...
  if (report.totals['native-candidate'] !== EXPECTED_APPS || report.totals.definiteBuildGaps !== 0) {
    throw new Error(`Acceptance is blocked until all ${EXPECTED_APPS.toLocaleString('en-US')} primary French owners are native candidates.`);
  }
...
function buildAcceptance(report, catalog) {
  return {
    ...
    policy: `Generated from the exact native ${EXPECTED_APPS.toLocaleString('en-US')}-row inventory and the reviewed 32-category evidence catalog. Route shape alone cannot create an entry.`,
