...
const scope = {
  fr: '12 exercices originaux de mathématiques, 2 d’anglais, 33 guides de mathématiques WAEC, 10 guides de rédaction et 4 de lecture WAEC, 59 guides de mathématiques NECO 2023, 4 guides de rédaction et 2 de lecture NECO. Il ne s’agit pas d’une épreuve complète WAEC ou NECO. Les réponses rédigées sont vérifiées par l’utilisateur, sans notation automatique.',
  sw: 'Kazi 12 asilia za Hisabati, 2 za Kiingereza, miongozo 33 ya Hisabati WAEC, 10 ya uandishi na 4 ya kusoma WAEC, 59 ya Hisabati NECO 2023, 4 ya uandishi na 2 ya kusoma NECO. Hii si karatasi kamili ya WAEC au NECO. Majibu ya kuandika yanahakikiwa na mwanafunzi mwenyewe, bila kutolewa alama kiotomatiki.'
};
...
function writtenBank(locale) {
  assertComplete(written.items.filter(q => q.year !== 2022 || q.subject !== 'Mathematics').map(q => q.id), Object.keys(writtenText[locale]), `Written ${locale}`);
  const bank = structuredClone(written);
  bank.locale = locale; bank.scope = scope[locale]; bank.ui = ui[locale];
  bank.items.forEach(q => {
    if (q.year === 2022 && q.subject === 'Mathematics') {
      const row=expansion[locale].maths['q'+q.number+(q.subpart ? q.subpart.replace('a–b','ab') : '')];
      if(!row)throw Error('Missing WAEC guidance: '+q.id);
      q.title=row[0]; q.steps=row[1]; q.checks=row[2];
      q.questionLanguage='en'; q.answerLanguage='en'; q.sourceLabel=ui[locale][q.sourceLabel];
      q.sourceUse=locale==='fr'?'Énoncé adapté de la source WAEC liée ; solution expliquée par AfroTools. '+(q.subpart?'Partie '+q.subpart+' uniquement.':''):'Muhtasari umetokana na chanzo cha WAEC kilichounganishwa; maelezo ya suluhisho ni ya AfroTools. '+(q.subpart?'Sehemu '+q.subpart+' pekee.':'');
      if(q.id==='waec-2022-mathematics-p2-q1a')q.sourceUse=locale==='fr'?'Partie (a) adaptée d’une copie liée du sujet original ; solution élaborée indépendamment par AfroTools. Ce guide n’est pas un barème officiel.':'Sehemu (a) imebadilishwa kutoka nakala iliyounganishwa ya karatasi asili; suluhisho limefanyiwa kazi na AfroTools. Huu si mwongozo rasmi wa usahihishaji.';
      return;
    }
...
const pageCopy = {
  fr: {
    ...
    description: '52 questions et 126 exercices rédigés de préparation WAEC / NECO, avec explications en français, sauvegardes locales et comptes rendus téléchargeables.',
    education: 'Éducation', hub: '/fr/education/', eyebrow: 'Préparation WAEC / NECO · Pilote', h1: 'Comprendre.<br>Puis s’entraîner.',
    intro: '24 questions de mathématiques et 16 d’anglais, 12 questions de physique, puis 126 exercices rédigés et guides WAEC 2021–2023 et NECO 2023. Vérifiez vos réponses, conservez votre travail et reprenez vos erreurs.',
  ...
  sw: {
    ...
    description: 'Maswali 52 na kazi 126 za kuandika za maandalizi ya WAEC / NECO, maelezo ya Kiswahili, nakala za maendeleo na ripoti zinazopakuliwa kwenye kifaa.',
    education: 'Elimu', hub: '/sw/elimu/', eyebrow: 'Maandalizi ya WAEC / NECO · Majaribio', h1: 'Elewa.<br>Kisha fanya mazoezi.',
    intro: 'Maswali 24 ya Hisabati na 16 ya Kiingereza, maswali 12 ya Fizikia, pamoja na kazi 126 za kuandika na miongozo ya WAEC 2021–2023 na NECO 2023. Hakiki majibu, hifadhi kazi yako na urudie uliyokosea.',
