...
function preserveRecordedReview(entry, previous, asOf) {
  const lastCheckedAt = dateOnly(previous && previous.lastCheckedAt) || null;
  const lastReviewedAt = dateOnly(previous && previous.lastReviewedAt) || null;
  return Object.assign({}, entry, {
    lastCheckedAt,
    lastReviewedAt,
    freshnessStatus: freshnessStatus(lastCheckedAt, lastReviewedAt, entry.reviewCadenceDays, asOf),
  });
}
...
function buildRegistry(asOf) {
  ...
  // A build can age a recorded review, but cannot perform a new review.
  function addRecordedMethod(entry) {
    const previous = existing.sources.find(source => source.id === entry.id);
    add(preserveRecordedReview(entry, previous, today));
  }
...
module.exports = { preserveRecordedReview, atomicWriteJson, buildRegistry, buildTelecomSnapshotEntry, dateOnly, freshnessStatus, normalizedRoute, syncSelectedSourceRegistry, syncTelecomSourceRegistry };
