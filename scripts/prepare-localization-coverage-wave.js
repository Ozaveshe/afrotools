...
function buildSwahiliWave() {
  ...
  return documented.map((enSlug) => {
    ...
    const generatedOutput = outputExists
      && fs.readFileSync(output, "utf8").includes("Generated by scripts/generate-sw-tool-gap-pages.js");
    return {
      enSlug,
      swSlug,
      name,
      ...
      category: SWAHILI_CATEGORY_OVERRIDES[enSlug] || record.category_key || "other",
      priority: Number(record.priority || 0),
      reuseExisting: outputExists && !generatedOutput,
    };
  });
}
...
function refreshDerivedCopy(manifest) {
  ...
  manifest.swahili = manifest.swahili.map((entry) => {
    ...
    return {
      ...entry,
      category: SWAHILI_CATEGORY_OVERRIDES[entry.enSlug] || entry.category,
      reuseExisting: outputExists && !generatedOutput,
    };
  });
  return manifest;
