...
function build() {
  ...
  const rows = scope.map((row) => {
    ...
    return {
      ...
      sourceOwners: row.categoryKey === 'religious-cultural'
        ? ['assets/js/engines/religious-cultural-parity.js','scripts/build-sw-religious-cultural-parity.js','assets/js/pages/sw-religious-cultural-parity.js']
  ...
  if (accepted !== 25 || blocked !== 8) throw new Error(`Expected 25 accepted and 8 blocked, found ${accepted}/${blocked}`);
  const receipt = {
    schemaVersion:1,
  ...
  const md = `# Swahili Religious, Cultural and African parity lane receipt\n\n`+
    `Baseline: \`${BASELINE}\`. Exact denominator: **33** (19 Religious & Cultural, 14 Uniquely African). Candidate accepted: **25**. Blocked: **8**. The coordinator acceptance ledger was not edited.\n\n`+
