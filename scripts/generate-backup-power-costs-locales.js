...
for(const locale of locales){if(!outputs[locale])throw new Error('Unsupported locale: '+locale);const target=path.join(ROOT,outputs[locale]);const html=render(locale).replace('<meta name="afrotools-source-owner"',`<meta name="afrotools-content-id" content="backup-power-costs-locale:${locale}"><meta name="afrotools-source-owner"`);fs.mkdirSync(path.dirname(target),{recursive:true});fs.writeFileSync(target,html.normalize('NFC')+'\n','utf8');console.log('Generated '+outputs[locale]);}
