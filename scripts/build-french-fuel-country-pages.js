...
function visiblePage(row, rows, country) {
  ...
  const description = `Relevé des prix du carburant ${location}, daté du ${date}. Consultez essence, diesel et GPL, puis calculez un budget avec un prix vérifié localement.`;
  const canonical = `${BASE_URL}/fr/tools/suivi-carburant/${slug}/`;
  ...
  <div class="fuel-country-shell">
    ...
    <div class="fuel-country-grid">
      <div>
        ...
        <p class="fuel-trust">Ligne du jeu de données datée du ${escapeHtml(date)} · Ne constitue pas un prix actuel en station · Vérifiez localement avant tout achat.</p>
        <p class="fuel-date">Relevé tiers, sans vérification officielle du prix. Les dates d’observation propres à chaque carburant, y compris le GPL, ne sont pas fournies. <a href="/data/fuel/latest.json">Consulter le jeu de données source</a>.</p>
      </div>
  ...
  const planner = `<section class="fuel-section fuel-planner" aria-labelledby="fuel-planner-title" data-fr-fuel-planner data-currency="${escapeHtml(row.currency)}">
    ...
    <div class="fuel-source-row"><a href="#related-countries">Consulter les relevés d’autres pays</a><a href="/fr/tools/couts-secours-energie/">Calculateur de groupe électrogène</a></div>
  </section>`;
