...
function inventory(raw,published,targets){
  ...
  const pairs=new Map();
  for(const q of raw){
    if(!['mathematics','english'].includes(q.subject)||!Number.isInteger(q.year))continue;
  ...
  for(const subject of ['mathematics','english'])for(const year of targets){const key=subject+'|'+year;if(!pairs.has(key))pairs.set(key,{exam:'JAMB',subject,year,source:[],live:[]});}
...
function build(root=ROOT,check=false){
  ...
  const report={schema_version:1,source_review_revision:published.review_revision,scope:'Nigeria Mathematics and English; JAMB compilation coverage and WAEC/NECO acquisition',
    interpretation:'Counts and observed numbering do not prove a complete exam paper. Null expected totals mean unknown, not zero.',jamb:rows,
    written_practice:{original_tasks:bank.items.filter(q=>q.exam===null).length,source_linked_writing_companions:bank.items.filter(q=>q.exam==='WAEC'&&q.subject==='English').length,source_linked_math_companions:bank.items.filter(q=>q.exam==='WAEC'&&q.subject==='Mathematics').length,imported_waec_past_questions:0,imported_neco_past_questions:0,complete_past_papers:0},sources:sources.candidates};
  ...
  const md=['# Nigeria exam-content coverage','',report.interpretation,'','## Recent-year priority gaps','', '| Exam | Subject | Year | Source records | Published | Next step |','| --- | --- | ---: | ---: | ---: | --- |',...rows.filter(r=>sources.target_years.includes(r.year)).map(r=>`| JAMB | ${r.subject} | ${r.year} | ${r.source_records} | ${r.published_records} | Acquire and authenticate source |`),'','## First written-practice delivery','',`${report.written_practice.original_tasks} original written tasks and ${report.written_practice.source_linked_writing_companions} WAEC 2023 writing companions, plus ${report.written_practice.source_linked_math_companions} Mathematics companions. No new imported WAEC/NECO past questions and no complete past papers are claimed.`,'','## Acquisition queue','',...sources.candidates.map(s=>`- **${s.id}** (${s.status}): ${s.next_action} Source: ${s.url}`),'','## Definition of complete','', 'Match a fingerprinted source and its exam, session, year and paper. Inventory every question and subpart; verify all passages, figures, instructions and answers. Do not infer completeness from a familiar total or consecutive numbers. UTME compilations may combine versions.',''].join('\n');
