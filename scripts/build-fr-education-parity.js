...
const rows = inventory.rows.filter((row) => row.categoryKey === "education");
const acceptanceState = process.argv.includes("--accept") ? "accepted" : "implemented-proof-pending";
...
const definitions = {
  ...
  "percentage-calc": {
    ...
    metrics: [["result", "Résultat"], ["modeFrench", "Méthode"], ["firstValue", "Première valeur"], ["secondValue", "Deuxième valeur"]]
  },
  "ke-helb": {
  ...
  "scholarship-finder": {
    ...
    metrics: [["total", "Bourses comparées"], ["topName", "Première correspondance"], ["topPercent", "Score indicatif (%)"], ["topCategoryFrench", "Niveau de correspondance"], ["feedLabel", "État de la source"]]
  },
  "tutoring-rate": {
    frenchRoute: "/fr/tools/calculateur-tarif-tutorat", title: "Calculateur de tarif de tutorat", description: "Calculez un tarif soutenable à partir de votre revenu cible, coûts, charge de travail et réserves; aucun tarif de marché n’est prétendu.",
...
function alternateLinks(existing, english, french) {
  const found = existing.match(/<link rel="alternate"[^>]+>/g) || [];
  ...
  byLang.set("fr", `<link rel="alternate" hreflang="fr" href="https://afrotools.com${normalize(french)}">`);
...
function page(row, definition, route, existing) {
  ...
  const sourceBadge = definition.recipe === "scholarship"
    ? '<p class="crumb" data-scholarship-feed-badge>Flux vérifié</p>'
    : "";
  return `<!doctype html>
<html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>${definition.title} | AfroTools</title>
<meta name="description" content="${definition.description}">
<link rel="canonical" href="https://afrotools.com${french}">
${alternateLinks(existing, english, french)}
<meta property="og:title" content="${definition.title}"><meta property="og:description" content="${definition.description}">
<meta property="og:url" content="https://afrotools.com${french}"><meta property="og:image" content="https://afrotools.com${art}">
...
<script type="application/ld+json">${JSON.stringify({"@context":"https://schema.org","@type":["WebApplication","EducationalApplication"],name:definition.title,url:`https://afrotools.com${french}`,inLanguage:"fr",isAccessibleForFree:true,applicationCategory:"EducationalApplication",operatingSystem:"Any modern browser",description:definition.description})}</script>
</head><body data-education-id="${row.englishId}">
<main class="wrap"><section class="hero"><a class="crumb" href="/fr/education/">Éducation AfroTools</a><h1>${definition.title}</h1><p>${definition.description}</p>${sourceBadge}</section>
<div class="grid"><section class="card"><h2>Votre scénario</h2><form data-education-form><div class="fields">${definition.fields.map(fieldHtml).join("")}</div><div class="actions"><button class="primary" type="submit">Calculer et vérifier</button><button type="reset">Réinitialiser</button></div></form><p class="status" data-education-status role="status" aria-live="polite"></p><section class="result" data-education-result aria-live="polite"><h2>Résultat du scénario</h2><div class="metrics" data-education-metrics></div><div class="actions"><button type="button" data-action="copy">Copier</button><button type="button" data-action="json">JSON</button><button type="button" data-action="csv">CSV</button><button type="button" data-action="txt">TXT</button><button type="button" data-action="pdf">PDF</button><button type="button" data-action="save">Enregistrer localement</button><button type="button" data-action="print">Imprimer</button></div></section></section>
<aside class="card boundary"><h2>Limites et confidentialité</h2><p>Aucun compte, téléversement ni envoi à une IA. Les données saisies et les exports restent dans ce navigateur.</p><ul><li>Ce résultat est une feuille de planification, pas une décision d’admission, une bourse, une note officielle, un tarif officiel ou un conseil financier.</li><li>Les règles, dates, frais et seuils variables doivent être confirmés auprès du propriétaire officiel et de l’établissement concerné.</li><li>L’anglais et le français utilisent le même propriétaire de calcul déterministe.</li></ul><img src="${art}" alt="" width="240" height="135" loading="lazy"></aside></div></main>
<script type="application/json" id="education-parity-config">${config.replace(/</g, "\\u003c")}</script>
${prelude}${extraScripts}<script src="${definition.script}"></script><script src="/assets/js/pages/fr-education-parity.js"></script>
</body></html>`;
}
...
function categoryHub(manifest) {
  ...
  const items = manifest.routes.map((route, index) => {
    ...
    return `<li><a href="${route.french}"><span>${String(index + 1).padStart(2, "0")}</span><strong>${title}</strong><small>Ouvrir l'application</small></a></li>`;
  }).join("");
  const itemList = manifest.routes.map((route, index) => ({
    "@type": "ListItem",
    position: index + 1,
    url: `https://afrotools.com${route.french}`,
    name: definitions[route.id] ? definitions[route.id].title : labels[route.id] || route.id
  }));
  return `<!doctype html>
<html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
...
for (const row of rows) {
  ...
  fs.writeFileSync(file, page(row, definition, frenchRoute, existing), "utf8");
}
