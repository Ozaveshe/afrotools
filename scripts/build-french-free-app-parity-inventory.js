...
function classifyFrenchPage(page, englishRouteSet) {
  ...
  const verifiedNativeRuntime = /\bdata-fr-creative-native=["'][^"']+["']/i.test(html)
    || /\/assets\/css\/fr-creative-route-real-proof\.css\b/i.test(html);
  const generatedBridge = !verifiedNativeRuntime && (
    /class=["'][^"']*(?:source-launch|prep-panel)\b/i.test(html)
...
function buildReport() {
  ...
  const rows = englishRows.map((english) => {
    ...
    const categoryEvidence = acceptance
      ? acceptanceCategoryByKey.get(acceptance.evidenceKey || acceptance.categoryKey)
      : null;
    ...
    return {
      ...
      acceptanceEvidence: accepted ? {
        status: 'accepted',
        englishId: acceptance.englishId,
        frenchRoute: acceptance.frenchRoute,
        categoryKey: categoryEvidence.categoryKey,
        receipt: categoryEvidence.receipt || FINAL_ACCEPTANCE_RECEIPT
      } : null,
