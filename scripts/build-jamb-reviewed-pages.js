...
${approved.length ? `<p>${approved.length} reviewed questions with answers and explanations.</p><p>${publisherCollection ? `These adapted questions come from ${collectionArticle}publisher-labelled ${esc(year)} ${collectionLabel}. The original UTME sitting and question numbers are unconfirmed. ` : ''}Practice selection: full-paper coverage has not been confirmed.</p><div class="qcard-list">${approved.map(q => renderCard(q, year === null)).join('\n')}</div>`
