...
].map((app) => ({ ...app, fields: fields.map((field) => app.englishId === 'home-renovation-cost' && field.name === 'currency' ? { ...field, sourceDefault: '', initialValue: '', placeholder: 'Sarafu yako' } : field), results: [{ key: 'total', label: 'Jumla ya makisio', format: 'money' }], formula: 'jumla = (kiasi × gharamaKwaKipimo + gharamaZisizobadilika) × (1 + akiba ÷ 100)', fixture: { currency: 'KES', quantity: '10', unitCost: '100', fixed: '50', contingency: '10' }, expected: { total: 1155 }, source: SOURCE, sharedEngine: 'property-assumption', exports: ['copy', 'txt', 'json', 'pdf', 'print'], privacy: 'local-only-no-storage-no-ai-no-account' }));
...
function input(field) {
  const attrs = [`name="${field.name}"`, `id="sw-pcp-${field.name}"`, `type="${field.type}"`, `value="${esc(field.initialValue)}"`, field.type === 'number' ? 'inputmode="decimal"' : 'autocomplete="off"', field.min != null ? `min="${field.min}"` : '', field.max != null ? `max="${field.max}"` : '', field.step ? `step="${field.step}"` : '', field.placeholder ? `placeholder="${esc(field.placeholder)}"` : '', field.required ? 'required' : ''].filter(Boolean).join(' ');
