...
function buildMinimumWagePage() {
  ...
  html = replaceAll(html, [
    ...
    ['<p class="tl-desc">Check the current minimum wage for any African country - with compliance checker, living wage gap, inflation erosion tracker, sector rates, and country alerts. All 54 countries covered.</p>', '<p class="tl-desc">Angalia kima cha chini cha mshahara kwa nchi yoyote ya Afrika pamoja na uhakiki wa ulinganifu, pengo la gharama ya maisha, mabadiliko ya nguvu ya manunuzi, viwango vya sekta na tahadhari za nchi. Nchi zote 54 zimejumuishwa.</p>'],
    ...
    ['<label class="tl-label" for="country">COUNTRY</label>', '<label class="tl-label" for="country">NCHI</label>'],
    ['<option value="">- Select a country -</option>', '<option value="">- Chagua nchi -</option>'],
    ...
    ['<option value="">- National / all states -</option>', '<option value="">- Taifa / majimbo yote -</option>'],
    ...
    ['1. Collect evidence - payslips, bank statements, employment contract<br>', '1. Kusanya ushahidi - payslip, taarifa ya benki na mkataba wa ajira<br>'],
    ...
    ['Generate a formal complaint letter ->', 'Tengeneza barua rasmi ya malalamiko ->'],
    ...
    ['<span class="mw-info-label">-ˆ USD Equivalent</span>', '<span class="mw-info-label">-ˆ Sawa na USD</span>'],
    ...
    ['<span>Living Wage - Individual</span>', '<span>Gharama ya maisha - mtu mmoja</span>'],
    ['<span>Living Wage - Family of 4</span>', '<span>Gharama ya maisha - familia ya watu 4</span>'],
    ...
    ['<div class="mw-alert-title">Get Notified When <span id="alert-country-name">This Country\'s</span> Minimum Wage Changes</div>', '<div class="mw-alert-title">Pata taarifa kima cha chini cha mshahara cha <span id="alert-country-name">nchi hii</span> kinapobadilika</div>'],
    ['<div class="mw-alert-sub">We\'ll email you when the rate is updated - no spam, just the change.</div>', '<div class="mw-alert-sub">Tutakutumia barua pepe kiwango kinapobadilika - hakuna spam, taarifa ya mabadiliko tu.</div>'],
    ...
    ['<option value="">- Select -</option>', '<option value="">- Chagua -</option>'],
    ['<h2 class="tl-section-title" style="margin-top:28px">All 54 African Countries - Minimum Wage Comparison</h2>', '<h2 class="tl-section-title" style="margin-top:28px">Nchi zote 54 za Afrika - Ulinganisho wa kima cha chini cha mshahara</h2>'],
    ['All 54', 'Zote 54'],
    ...
    ['Minimum wage comparison - all 54 African countries, sortable by column header', 'Ulinganisho wa kima cha chini cha mshahara - nchi zote 54 za Afrika, linaweza kupangwa kwa kichwa cha safu'],
    ['Country <span aria-hidden="true">-</span>', 'Nchi <span aria-hidden="true">-</span>'],
    ['Local Rate <span aria-hidden="true">-</span>', 'Kiwango cha ndani <span aria-hidden="true">-</span>'],
    ['USD/Month <span aria-hidden="true">-</span>', 'USD/Mwezi <span aria-hidden="true">-</span>'],
    ['Effective Date <span aria-hidden="true">-</span>', 'Tarehe ya kuanza <span aria-hidden="true">-</span>'],
    ['Loadingâ€¦', 'Inapakia...'],
    ['Report a Wage Violation', 'Ripoti ukiukaji wa mshahara'],
    ['Know of an employer paying below the legal minimum wage? Report it anonymously. Verified reports earn 50 AfroPoints. Aggregate stats only - no employer names without verification.', 'Unamfahamu mwajiri anayelipa chini ya kiwango cha chini cha kisheria? Ripoti bila kutaja jina lako. Ripoti zilizothibitishwa hupata AfroPoints 50. Tunahifadhi takwimu za jumla tu hadi uthibitisho upatikane.'],
    ...
    ['Minimum wage is the legal floor - employers cannot pay below it', 'Kima cha chini cha mshahara ni kiwango cha kisheria - mwajiri haruhusiwi kulipa chini yake'],
    ...
    ['<li><a href="/salary-tax/">Salary Calculator</a> - Net pay after tax</li>', '<li><a href="/sw/mshahara-na-kodi/paye/">Kikokotoo cha mshahara</a> - Malipo halisi baada ya kodi</li>'],
    ['<li><a href="/salary-tax/">PAYE Tax Calculator</a> - 50+ countries</li>', '<li><a href="/sw/mshahara-na-kodi/paye/">Kikokotoo cha PAYE</a> - Nchi 50+</li>'],
    ['<li><a href="/tools/cost-of-living/">Cost of Living</a> - Compare cities</li>', '<li><a href="/tools/cost-of-living/">Gharama ya maisha</a> - Linganisha miji</li>'],
    ['<li><a href="/tools/currency-converter/">Currency Converter</a> - Convert wages</li>', '<li><a href="/sw/zana/kibadilishaji-sarafu/">Kibadilishaji cha sarafu</a> - Badilisha mishahara</li>'],
    ['<li><a href="/tools/employee-cost/">Staff Cost Calculator</a> - Employer burden</li>', '<li><a href="/tools/employee-cost/">Kikokotoo cha gharama ya mfanyakazi</a> - Mzigo wa mwajiri</li>'],
    ['<li><a href="/salary-tax/">All Salary &amp; Tax Tools</a> - Full toolkit</li>', '<li><a href="/sw/mshahara-na-kodi/">Zana zote za mshahara na kodi</a> - Zana kamili</li>'],
...
function buildSwObservation(c) {
  ...
  html = replaceRegex(html, [
    ...
    [/document\.getElementById\('r-usd'\)\.textContent = usd \? '-ˆ US\$' \+ usd\.toLocaleString\(\) \+ '\/month' : 'N\/A';/, "document.getElementById('r-usd').textContent = usd ? '-ˆ US$' + usd.toLocaleString('sw-TZ') + '/mwezi' : 'Hakuna';"],
    ...
    [/canvas\.setAttribute\('aria-label', c\.name \+ ' purchasing power erosion ' \+ inf\.baseYear \+ '-' \+ inf\.points\[inf\.points\.length-1\]\.year\);/, "canvas.setAttribute('aria-label', 'Kupungua kwa nguvu ya manunuzi ya ' + c.name + ' ' + inf.baseYear + '-' + inf.points[inf.points.length-1].year);"],
    ...
    [/titleEl\.innerHTML = '- No statutory minimum wage in ' \+ result\.flag \+ ' ' \+ result\.country;/, "titleEl.innerHTML = '- Hakuna kima cha chini cha mshahara cha kitaifa katika ' + result.flag + ' ' + swCountryName(_currentCode, result.country);"],
    ...
    [/titleEl\.textContent = 'OK Compliant';/, "titleEl.textContent = 'OK Inafuata sheria';"],
    ...
    [/titleEl\.textContent = '- Below Minimum Wage - This is Illegal';/, "titleEl.textContent = '- Chini ya kima cha chini cha mshahara - si halali';"],
    ...
    [/actionEl\.innerHTML = '<button onclick="toggleWhatToDo\(\)" style="background:none;border:none;color:#991b1b;font-weight:700;cursor:pointer;padding:0;font-size:.82rem">What to do -><\/button>';/, "actionEl.innerHTML = '<button onclick=\"toggleWhatToDo()\" style=\"background:none;border:none;color:#991b1b;font-weight:700;cursor:pointer;padding:0;font-size:.82rem\">Cha kufanya -></button>';"],
    [/msgEl\.textContent = 'Please enter a valid email address\.';/, "msgEl.textContent = 'Tafadhali weka anwani sahihi ya barua pepe.';"],
    [/msgEl\.textContent = 'OK Subscribed! We\\'ll notify you when ' \+ cc \+ ' minimum wage changes\.';/, "msgEl.textContent = 'OK Umejiandikisha! Tutakujulisha kima cha chini cha mshahara cha ' + swCountryName(cc, cc) + ' kinapobadilika.';"],
    ...
    [/opts = '<option value="">- Select -<\/option>';/, "opts = '<option value=\"\">- Chagua -</option>';"],
    ...
    [/res\.textContent = 'OK Report submitted\. Thank you - \+50 AfroPoints credited once verified\.';/, "res.textContent = 'OK Ripoti imewasilishwa. Asante - AfroPoints 50 zitaongezwa baada ya kuthibitishwa.';"],
    [/res\.textContent = 'OK Report queued\. Thank you for helping protect workers\.';/, "res.textContent = 'OK Ripoti imepokelewa. Asante kwa kusaidia kulinda wafanyakazi.';"],
    [/opts = '<option value="">- Select country -<\/option>';/, "opts = '<option value=\"\">- Chagua nchi -</option>';"],
  ...
  html = replaceRegex(html, [
    ...
    [/<option value="">[^<]*Select a country[^<]*<\/option>/, '<option value="">- Chagua nchi -</option>'],
    [/<option value="">[^<]*National \/ all states[^<]*<\/option>/, '<option value="">- Taifa / majimbo yote -</option>'],
    [/<strong>Cha kufanya ukilipwa chini ya kima cha chini cha mshahara:<\/strong><br>\s*1\.[\s\S]*?5\. <a href="\/tools\/document-generator\/" onclick="return false">Generate a formal complaint letter[^<]*<\/a>/, '<strong>Cha kufanya ukilipwa chini ya kima cha chini cha mshahara:</strong><br>\n              1. Kusanya ushahidi - payslip, taarifa ya benki na mkataba wa ajira<br>\n              2. Ripoti kwa Wizara ya Kazi au mkaguzi wa kazi wa nchi yako<br>\n              3. Wasiliana na chama cha wafanyakazi kilichosajiliwa kwa msaada<br>\n              4. Wasilisha malalamiko kwa mahakama ya kazi au chombo husika<br>\n              5. <a href="/tools/document-generator/" onclick="return false">Tengeneza barua rasmi ya malalamiko -></a>'],
    [/<span>Living Wage - Individual<\/span>/g, '<span>Gharama ya maisha - mtu mmoja</span>'],
    [/<span>Living Wage - Family of 4<\/span>/g, '<span>Gharama ya maisha - familia ya watu 4</span>'],
    [/<div class="mw-alert-title">[\s\S]*?Minimum Wage Changes<\/div>/, '<div class="mw-alert-title">Pata taarifa kima cha chini cha mshahara cha <span id="alert-country-name">nchi hii</span> kinapobadilika</div>'],
    [/<div class="mw-alert-sub">[\s\S]*?<\/div>/, '<div class="mw-alert-sub">Tutakutumia barua pepe kiwango kinapobadilika - hakuna spam, taarifa ya mabadiliko tu.</div>'],
    [/<option value="">[^<]*Select[^<]*<\/option>/g, '<option value="">- Chagua -</option>'],
    [/<h2 class="tl-section-title" style="margin-top:28px">[\s\S]*?<\/h2>/, '<h2 class="tl-section-title" style="margin-top:28px">Nchi zote 54 za Afrika - Ulinganisho wa kima cha chini cha mshahara</h2>'],
    [/<caption class="sr-only">[\s\S]*?<\/caption>/, '<caption class="sr-only">Ulinganisho wa kima cha chini cha mshahara - nchi zote 54 za Afrika, linaweza kupangwa kwa kichwa cha safu</caption>'],
    [/<div class="mw-crowdsource-title">[\s\S]*?<\/div>/, '<div class="mw-crowdsource-title">Ripoti ukiukaji wa mshahara</div>'],
    ...
    [/<div class="tl-info-title">[^<]*KNOW YOUR RIGHTS<\/div>/, '<div class="tl-info-title">- JUA HAKI ZAKO</div>'],
    [/<li>Minimum wage is the legal floor[\s\S]*?<\/li>/, '<li>Kima cha chini cha mshahara ni kiwango cha kisheria - mwajiri haruhusiwi kulipa chini yake</li>'],
    [/<div class="tl-info-title">[^<]*RELATED TOOLS<\/div>/, '<div class="tl-info-title">ZANA ZINAZOHUSIANA</div>'],
    [/<li><a href="\/salary-tax\/">[^<]*Salary Calculator<\/a>[^<]*Net pay after tax<\/li>/, '<li><a href="/sw/mshahara-na-kodi/paye/">Kikokotoo cha mshahara</a> - Malipo halisi baada ya kodi</li>'],
    [/<li><a href="\/salary-tax\/">[^<]*PAYE Tax Calculator<\/a>[^<]*50\+ countries<\/li>/, '<li><a href="/sw/mshahara-na-kodi/paye/">Kikokotoo cha PAYE</a> - Nchi 50+</li>'],
    [/<li><a href="\/tools\/cost-of-living\/">[^<]*Cost of Living<\/a>[^<]*Compare cities<\/li>/, '<li><a href="/tools/cost-of-living/">Gharama ya maisha</a> - Linganisha miji</li>'],
  ...
  html = replaceRegex(html, [
    ...
    [/titleEl\.textContent = '[^']*Compliant';/, "titleEl.textContent = 'OK Inafuata sheria';"],
    [/titleEl\.textContent = '[^']*Below Minimum Wage[^']*';/, "titleEl.textContent = '- Chini ya kima cha chini cha mshahara - si halali';"],
    [/actionEl\.innerHTML = '<button onclick="toggleWhatToDo\(\)"[^>]*>What to do [^<]*<\/button>';/, "actionEl.innerHTML = '<button onclick=\"toggleWhatToDo()\" style=\"background:none;border:none;color:#991b1b;font-weight:700;cursor:pointer;padding:0;font-size:.82rem\">Cha kufanya -></button>';"],
    [/msgEl\.textContent = '[^']*Subscribed! We\\'ll notify you when ' \+ cc \+ ' minimum wage changes\.';/, "msgEl.textContent = 'OK Umejiandikisha! Tutakujulisha kima cha chini cha mshahara cha ' + swCountryName(cc, cc) + ' kinapobadilika.';"],
...
if (process.argv.includes('--sync-minimum-wage-runtime')) {
  ...
  const submissionTag = '<script src="/assets/js/pages/minimum-wage-submissions.js"></script>';
  const submissionMatches = html.match(/<script src="\/assets\/js\/pages\/minimum-wage-submissions\.js(?:\?[^"]*)?"><\/script>/g) || [];
  if (submissionMatches.length > 1) throw new Error('Ambiguous submissions runtime boundary');
  html = submissionMatches.length ? html.replace(submissionMatches[0], submissionTag) : html.replace('</body>', submissionTag + '\n</body>');
  write(route, html);
} else {
  buildOvertimePage();
