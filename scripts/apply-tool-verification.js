...
function buildPanel(entry) {
  ...
  const limitations = htmlList(entry.known_limitations.map(escapeHtml));
  const cases = entry.test_cases.map((testCase) => (
    `<div class="tool-verification-case"><p class="tool-verification-meta"><strong>Input:</strong> ${escapeHtml(testCase.input)}<br><strong>Expected:</strong> ${escapeHtml(testCase.expected_output)}<br><strong>Why:</strong> ${escapeHtml(testCase.why)}</p></div>`
  )).join('\n');
  const history = entry.change_history.map((item) => `<li><strong>${escapeHtml(item.date)}:</strong> ${escapeHtml(item.note)}</li>`).join('\n');
  ...
  <div class="container">
    <div class="tool-verification-card">
      <div class="tool-verification-head">
        <div>
          <span class="tool-verification-kicker">Official evidence</span>
          <h2 class="tool-verification-title">Sources &amp; verification</h2>
          <p class="tool-verification-lead">This high-stakes calculator links the authority sources, method notes, test cases, and limitations used to check the numbers shown on this page.</p>
        </div>
        <div class="tool-verification-status">
          <span class="tool-verification-badge">Last verified ${escapeHtml(entry.last_verified)}</span>
          <p class="tool-verification-note">${escapeHtml(entry.jurisdiction)} - ${escapeHtml(entry.risk_level)} risk - ${escapeHtml(entry.verified_by)}</p>
        </div>
      </div>
...
${sourceItems}
        ...
        <div class="tool-verification-block">
          <h3>Law, regulation, or version</h3>
          <p class="tool-verification-meta">${escapeHtml(entry.law_or_version)}</p>
        </div>
        <div class="tool-verification-block">
          <h3>Calculation methodology</h3>
          <p class="tool-verification-meta">${escapeHtml(entry.methodology_markdown)}</p>
        </div>
        <div class="tool-verification-block">
...
${history}
      ...
      <div class="tool-verification-actions">
        <p class="tool-verification-disclaimer">AfroTools calculators are decision-support tools. Always confirm filing, registration, and remittance duties with the linked authority or a qualified local adviser.</p>
        <a class="tool-verification-report" href="/contact/?topic=calculation-error&amp;tool=${encodeURIComponent(entry.tool_id)}">Report calculation error</a>
      </div>
    </div>
  </div>
</section>
...
function applyPanelToFile(file, entry) {
  ...
  html = insertPanel(html, buildPanel(entry));
...
function main() {
  ...
  const manifest = buildManifest(targetFiles);
  const dataPath = path.join(ROOT, 'data', 'tool-verification.json');
  const dataJson = `${JSON.stringify(manifest, null, 2)}\n`;
  if (WRITE) fs.writeFileSync(dataPath, dataJson, 'utf8');
