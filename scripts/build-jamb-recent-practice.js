...
<section aria-labelledby="reference-heading"><h2 id="reference-heading">Question sources and worked guides</h2>${bank.items.map(q=>`<details class="written-explanation"><summary>${esc(q.title)}</summary><p>${esc(q.prompt)}</p><p><a href="${esc(q.source)}" rel="noopener noreferrer" target="_blank">Source item ${q.sourceItem}: Myschool’s JAMB 2023 collection</a></p><details class="written-explanation"><summary>Show worked solution</summary><p>${esc(q.answer)}</p><ol>${q.steps.map(s=>`<li>${esc(s)}</li>`).join('')}</ol></details></details>`).join('\n')}</section>
