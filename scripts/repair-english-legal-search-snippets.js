...
const DEFINITIONS = Object.freeze({
  "employment-contract": {
    ...
    description(country) {
      return `Draft an employment contract for ${country} with pay, leave, notice and key clauses; review it under local law before signing.`;
    }
  },
  "tenancy-agreement": {
    title: "Tenancy Agreement",
    description(country) {
      return `Draft a tenancy agreement for ${country} with rent, deposit, utilities and key clauses; review it under local law before signing.`;
    }
  }
});
...
function metadataFor(tool, slug) {
  ...
  if (!definition || !country) throw new Error("Unknown legal snippet target: " + tool + "/" + slug);
  const schemaName = definition.title + " — " + country.name;
  return {
    title: schemaName + " | AfroTools",
    schemaName,
    description: definition.description(country.name)
  };
}
...
function main() {
  ...
  console.log("English legal snippets: " + result.targets + " pages checked; " + result.stale.length + (WRITE ? " updated." : " stale."));
}
