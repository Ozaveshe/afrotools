...
for(const row of ROUTES){
  for(const locale of ["fr","sw"]){
    ...
    write(file,decorateDiscovery(content,locale,localizedRow),[`hreflang="sw"`,`href="https://afrotools.com${row[2]}"`,'name="afrotools-source-owner"',`name="afrotools-content-id" content="${contentId(locale,localizedRow)}"`]);
  }
  const enFile=fileFor(row[0]);let en=fs.readFileSync(enFile,"utf8");en=addAlternate(addAlternate(en,"fr",row[1]),"sw",row[2]);write(enFile,en,[`hreflang="sw"`,`href="https://afrotools.com${row[2]}"`]);
}
