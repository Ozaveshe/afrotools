/**
 * ThumbnailForge — Netlify Function
 * Handles project save/load for authenticated users and template analytics
 */
  ...
  try {
    ...
    if (action === 'list' && params.user_id) {
      const res = await sbFetch(
        `creator_thumb_projects?user_id=eq.${params.user_id}&select=id,title,template_id,created_at,updated_at&order=updated_at.desc&limit=50`
      );
    ...
    if (action === 'get' && params.id) {
      const res = await sbFetch(
        `creator_thumb_projects?id=eq.${params.id}&select=*&limit=1`
      );
      const data = await res.json();
    ...
    if (event.httpMethod === 'POST') {
      ...
      if (body.action === 'save' && body.project) {
        ...
        if (project.id) {
          res = await sbFetch(
            `creator_thumb_projects?id=eq.${project.id}`,
            { method: 'PATCH', body: project }
          );
        } else {
          project.created_at = new Date().toISOString();
          res = await sbFetch('creator_thumb_projects', { method: 'POST', body: project });
        }
        const data = await res.json();
    ...
    if (event.httpMethod === 'DELETE' && params.id && params.user_id) {
      const res = await sbFetch(
        `creator_thumb_projects?id=eq.${params.id}&user_id=eq.${params.user_id}`,
        { method: 'DELETE' }
      );
