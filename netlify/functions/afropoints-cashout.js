...
exports.handler = async function (event) {
  ...
  if (event.httpMethod === 'GET') {
    ...
    return reply(200, Array.isArray(history) ? history : [], headers, sessionResponse);
  }

  if (event.httpMethod !== 'POST') return reply(405, { error: 'Method not allowed' }, headers, sessionResponse);
  ...
  if (!method || !PAYOUT_METHODS.includes(method)) return reply(400, { error: 'Invalid payout method' }, headers, sessionResponse);
  if (!Number.isFinite(pointsAmount) || pointsAmount <= 0) return reply(400, { error: 'Invalid points amount' }, headers, sessionResponse);

  const minRequired = method === 'pro_credit' ? PRO_COST : MIN_CASHOUT;
  if (pointsAmount < minRequired) return reply(400, { error: 'Minimum ' + minRequired + ' points required' }, headers, sessionResponse);

  if (method === 'pro_credit' && pointsAmount % PRO_COST !== 0) {
    return reply(400, { error: 'AfroTools Pro credit must be redeemed in 500-point blocks' }, headers, sessionResponse);
  }

  try {
    const profile = await fetchPointsProfile(user.id);
    if (!profile) return reply(400, { error: 'No points profile found' }, headers, sessionResponse);
    if ((profile.current_balance || 0) < pointsAmount) return reply(400, { error: 'Insufficient balance' }, headers, sessionResponse);
    ...
    if (validation.error) return reply(400, { error: validation.error }, headers, sessionResponse);

    if (method !== 'pro_credit') {
      const pending = await sbFetch('cashout_requests?user_id=eq.' + user.id + '&status=eq.pending');
      if (Array.isArray(pending) && pending.length > 0) {
        return reply(400, { error: 'You already have a pending cashout request' }, headers, sessionResponse);
      }
    }
