...
const file=path.join(__dirname,'check-recovery-001.cjs'),local=mod.createRequire(file),batch=local('./recovery-001.json');
const original=JSON.parse(fs.readFileSync(path.join(__dirname,'../../source-pool.json'))),code=fs.readFileSync(file,'utf8');
function run({integrated=false,missing=false,badPdf=false,alterHeld=false}={}){
 const pool=JSON.parse(JSON.stringify(original));
 if(integrated)for(const r of batch.records)if(r.publication_candidate)pool.questions[pool.questions.findIndex(q=>q.id===r.id)]=r.candidate;
...
console.log('Recovery001 mode tests: local and portable integration pass; absent pre-intake PDF, wrong PDF and unauthorized held mutation correctly rejected.');
