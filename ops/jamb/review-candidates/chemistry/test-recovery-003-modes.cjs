...
const file=path.join(__dirname,'check-recovery-003.cjs'),local=mod.createRequire(file),batch=local('./recovery-003.json');
...
console.log('Recovery003 mode tests: local and portable integration pass; absent pre-intake PDF, wrong PDF and unauthorized held mutation correctly rejected.');
