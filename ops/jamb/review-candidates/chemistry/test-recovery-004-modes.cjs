...
const file=path.join(__dirname,'check-recovery-004.cjs'),local=mod.createRequire(file),batch=local('./recovery-004.json');
...
console.log('Recovery004 mode tests: local and portable integration pass; absent pre-intake PDF, wrong PDF and unauthorized held mutation correctly rejected.');
