...
const file=path.join(__dirname,'check-recovery-006.cjs'),local=mod.createRequire(file),batch=local('./recovery-006.json');
...
console.log('Recovery006 mode tests: local and portable integration pass; absent pre-intake PDF, wrong PDF and unauthorized held mutation correctly rejected.');
