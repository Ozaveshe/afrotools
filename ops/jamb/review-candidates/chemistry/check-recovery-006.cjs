...
const b=require('./recovery-006.json'),integrated=process.argv.includes('--integrated');
const pool=JSON.parse(fs.readFileSync(path.join(__dirname,'../../source-pool.json'))).questions;
const expected=['chemistry-1998-35-a269c5c80043'];
...
for(const r of b.records){
 ...
 assert.equal(q.verification?.method,'ai-source-checked');assert.equal(q.verification?.reviewed_at,'2026-09-12');assert.equal(q.explanation,q.ai_explanation);assert.ok(q.options[q.answer]);assert.equal(q.format,Object.keys(q.options).length);
...
const priorFiles=['recovery-001.json','recovery-002.json','recovery-003.json','recovery-004.json','recovery-005.json'];
...
const q=get(expected[0]);assert.equal(q.answer,'B');assert.ok(q.question.includes('nitrogen gas (N₂)'));assert.ok(q.question.includes('over water'));assert.equal(q.options.B,'Sodium nitrite and ammonium chloride');
// NaNO2 + NH4Cl -> NaCl + N2 + 2H2O: atom totals Na,N,O,H,Cl.
assert.deepEqual([1,1+1,2,4,1],[1,2,2,2*2,1]);
assert.equal(q.has_diagram,false);assert.ok(!q.image);assert.equal(b.records[2].source_pdf_page,52);
console.log('Recovery006: 3 examined, 1 recovered, 2 held; 25 exact prior recoveries, source context and independent atom balance passed.');
