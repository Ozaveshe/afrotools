...
const root=path.resolve(__dirname,'../../../..'),b=require('./math-uncovered-002.json'),pool=JSON.parse(fs.readFileSync(path.join(root,'ops/jamb/source-pool.json'))),ledger=JSON.parse(fs.readFileSync(path.join(root,'data/jamb/review-ledger.json'))),dir=fs.mkdtempSync(path.join(os.tmpdir(),'math-uncovered002-'));
...
for(const r of b.records.filter(r=>r.candidate)){pool.questions[pool.questions.findIndex(q=>q.id===r.id)]=r.candidate;ledger.questions[r.id]={source_id:b.source_id,content_sha256:r.content_sha256,question_review:review,answer_review:review,explanation_review:review};}
function run(p,l){fs.writeFileSync(path.join(dir,'pool.json'),JSON.stringify(p));fs.writeFileSync(path.join(dir,'ledger.json'),JSON.stringify(l));return cp.spawnSync(process.execPath,[path.join(__dirname,'check-uncovered-002.cjs'),'--integrated','--pdf='+path.join(dir,'absent.pdf'),'--pool='+path.join(dir,'pool.json'),'--ledger='+path.join(dir,'ledger.json')],{encoding:'utf8'});}
