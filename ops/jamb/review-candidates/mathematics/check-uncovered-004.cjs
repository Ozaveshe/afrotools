...
const trust=require(path.join(root,'scripts/lib/jamb-content-trust')),b=require('./math-uncovered-004.json'),audit=require('./original-coverage-audit.json'),sha=v=>crypto.createHash('sha256').update(v).digest('hex');
assert.equal(sha(fs.readFileSync(path.join(__dirname,'math-uncovered-004.json'))),'156d02c41b672378cdf9192f5e1523a455c9d366c61233618ce4492f42afdacc');
...
assert.deepEqual(b.records.map(r=>r.id),audit.uncovered_ids.slice(49));
...
const solve=require('./solve-uncovered-004.cjs');
...
assert.equal(candidates,5);assert.equal(held,23);
console.log(JSON.stringify({pass:true,examined:28,candidates,held,wrong_answer_negatives:wrong,publication_negatives:negative,shared_mutation:false,private_pdf_checked:fs.existsSync(pdf)}));
