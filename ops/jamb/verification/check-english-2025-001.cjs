...
const { manifestPath, batchPath } = require('../../nigeria-exams/import-jamb-english-2025-batch-01.cjs');
...
function verify(manifest, manifestHash, pool, ledger, batch) {
  ...
  assert.equal(manifest.records.length + manifest.held_source_items.length, 15);
  const checked = [];
