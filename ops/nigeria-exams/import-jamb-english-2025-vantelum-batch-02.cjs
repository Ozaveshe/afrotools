...
// Imports newly worded revision items from a publisher-labelled collection.
// A publisher year is not proof of an authenticated UTME sitting or paper order.
...
const manifestPath = 'ops/nigeria-exams/jamb-english-2025-vantelum-batch-02.json';
const batchPath = 'ops/jamb/verification/english-2025-publishable-006.json';
const checkerPath = 'ops/jamb/verification/check-english-2025-006.cjs';
const reviewedAt = '2026-09-26';
const expectedItems = [16, 19, 27, 29, 33, 34, 35, 36, 37, 39];
...
function itemId(item) {
  return `english-2025-vantelum-${String(item).padStart(2, '0')}`;
}

function validateManifest(manifest) {
  if (manifest.schema_version !== 1 || manifest.publisher !== 'Vantelum'
      || manifest.collection_url !== 'https://vantelum.com/cbt/jamb/use-of-english/2025/answers'
      || manifest.collection_year !== 2025 || manifest.year_basis !== 'publisher-collection'
      || manifest.sitting_authenticated !== false || !Array.isArray(manifest.records)
      || !Array.isArray(manifest.held_source_items)) {
    throw Error('Unexpected 2025 English Vantelum source manifest');
  }
  ...
  if (JSON.stringify(positions) !== JSON.stringify(expectedItems)) {
    throw Error('2025 English Vantelum item set changed without review');
  }
  ...
  for (const row of manifest.held_source_items) {
    if (!Number.isInteger(row.source_item) || row.source_item < 1 || row.source_item > 40
  ...
  for (const row of manifest.records) {
    if (!Number.isInteger(row.source_item) || row.source_item < 1 || row.source_item > 40
        ...
        || row.independent_source_urls.some(url => !url.startsWith('https://dictionary.cambridge.org/')
          && !url.startsWith('https://learnenglish.britishcouncil.org/'))) {
      throw Error('Invalid curated Vantelum item: ' + row.source_item);
    }
  }
}
...
function prepareBatch(manifest, manifestHash, sourcePool, reviewLedger) {
  ...
  for (const row of manifest.records) {
    ...
    const sourceId = `owner-directed-vantelum-english-2025-${String(row.source_item).padStart(2, '0')}`;
    ...
    ledger.sources[sourceId] = {
      ...
      source_url: manifest.collection_url,
      ...
      label: `Vantelum publisher-labelled 2025 English revision item ${row.source_item}; sitting unconfirmed`,
      reuse_authorization: {
    ...
    const question = {
      ...
      source_provenance: { publisher: manifest.publisher, url: manifest.collection_url, year_basis: manifest.year_basis }
    };
