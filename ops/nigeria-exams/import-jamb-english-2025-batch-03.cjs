...
// Replays fifteen owner-directed, short adapted revision items. Myschool's year
...
const manifestPath = 'ops/nigeria-exams/jamb-english-2025-curated-batch-03.json';
const batchPath = 'ops/jamb/verification/english-2025-publishable-003.json';
const checkerPath = 'ops/jamb/verification/check-english-2025-003.cjs';
const reviewedAt = '2026-09-25';
const expectedItems = [222, 224, 228, 229, 230, 232, 233, 235, 236, 239, 240, 241, 243, 244, 245];
const auditedItems = Array.from({ length: 25 }, (_, i) => i + 221);
...
function validateManifest(manifest) {
  ...
  if (JSON.stringify([...actualItems, ...held].sort((a, b) => a - b)) !== JSON.stringify(auditedItems)) {
    throw Error('The 25-position audit must classify every position exactly once');
  }
  for (const row of manifest.held_source_items) {
    if (row.source_question_id !== row.source_item + 74611
