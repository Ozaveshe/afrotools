...
const manifestPath = 'ops/nigeria-exams/jamb-math-2022-curated-batch-02.json';
const snapshotPath = 'ops/nigeria-exams/jamb-math-2022-source-snapshot-02.json';
const receiptPath = 'ops/jamb/verification/mathematics-2022-publishable-002.json';
const checkerPath = 'ops/jamb/verification/check-mathematics-2022-002.cjs';
...
function validateManifest(manifest) {
  if (manifest.schema_version !== 1 || manifest.observed_at !== '2026-09-26' ||
      ...
      !Array.isArray(manifest.items) || manifest.items.length !== 14) {
  ...
  const firstBatch = read('ops/nigeria-exams/jamb-math-2022-curated-batch-01.json');
  const usedPositions = new Set(firstBatch.items.map(item => item.position));
  const usedItems = new Set(firstBatch.items.map(item => item.sourceItem));
  if (new Set(positions).size !== 14 || new Set(sourceItems).size !== 14 ||
      positions.some(position => !Number.isInteger(position) || position < 1 || position > 40) ||
      sourceItems.some(id => !/^\d{5}$/.test(id)) ||
      positions.some(position => usedPositions.has(position)) ||
      sourceItems.some(id => usedItems.has(id))) throw Error('Duplicate or invalid source position');
...
function prepareBatch(manifest, pool, ledger) {
  ...
  for (const item of manifest.items) {
    ...
    const evidence = `${snapshotPath}#${item.sourceItem}; ${manifestPath}#${item.sourceItem}; ${path.basename(receiptPath)}#${id}; ${path.basename(checkerPath)}; tests/jamb-math-2022-batch-02.test.js`;
