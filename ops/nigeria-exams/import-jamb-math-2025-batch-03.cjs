...
const manifestPath = 'ops/nigeria-exams/jamb-math-2025-curated-batch-03.json';
const snapshotPath = 'ops/nigeria-exams/jamb-math-2025-source-snapshot-03.json';
const receiptPath = 'ops/jamb/verification/mathematics-2025-publishable-003.json';
const checkerPath = 'ops/jamb/verification/check-mathematics-2025-003.cjs';
...
function validateManifest(manifest) {
  if (manifest.schema_version !== 1 || manifest.observed_at !== '2026-09-25'
      ...
      || !Array.isArray(manifest.items) || manifest.items.length !== 2
      || !Array.isArray(manifest.held) || manifest.held.length !== 8) {
    throw Error('Unexpected Mathematics 2025 batch-03 provenance or size');
  }
  const positions = [...manifest.items, ...manifest.held].map(item => item.position).sort((a, b) => a - b);
  if (JSON.stringify(positions) !== JSON.stringify(Array.from({ length: 10 }, (_, i) => i + 46))) {
    throw Error('Every collection position 46–55 must have one disposition');
  }
...
function prepareBatch(manifest, pool, ledger) {
  ...
  for (const item of manifest.items) {
    ...
    const evidence = `${snapshotPath}#${item.sourceItem}; ${manifestPath}#${item.sourceItem}; ${path.basename(receiptPath)}#${id}; ${path.basename(checkerPath)}; tests/jamb-math-2025-batch-03.test.js`;
