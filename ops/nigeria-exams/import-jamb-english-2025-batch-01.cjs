...
// Replays three owner-directed, short adapted revision items. Myschool's year
...
const manifestPath = 'ops/nigeria-exams/jamb-english-2025-curated-batch-01.json';
const batchPath = 'ops/jamb/verification/english-2025-publishable-001.json';
const checkerPath = 'ops/jamb/verification/check-english-2025-001.cjs';
const reviewedAt = '2026-09-25';
const expectedItems = [194, 198, 199];
const auditedItems = Array.from({ length: 15 }, (_, i) => i + 193);
...
function validateManifest(manifest) {
  ...
  if (JSON.stringify([...actualItems, ...held].sort((a, b) => a - b)) !== JSON.stringify(auditedItems)) {
    throw Error('The 15-position audit must classify every position exactly once');
  }
  for (const row of manifest.held_source_items) {
    if (row.source_question_id !== 74804 + row.source_item - 193
