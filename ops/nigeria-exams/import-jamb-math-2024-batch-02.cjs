...
const manifestPath = 'ops/nigeria-exams/jamb-math-2024-curated-batch-02.json';
const snapshotPath = 'ops/nigeria-exams/jamb-math-2024-source-snapshot-02.json';
const receiptPath = 'ops/jamb/verification/mathematics-2024-publishable-002.json';
const checkerPath = 'ops/jamb/verification/check-mathematics-2024-002.cjs';
...
function validateManifest(manifest) {
  if (manifest.schema_version !== 1 || manifest.observed_at !== '2026-09-25'
      ...
      || !Array.isArray(manifest.items) || manifest.items.length !== 12
      || !Array.isArray(manifest.held) || manifest.held.length !== 3) {
    throw Error('Unexpected Mathematics 2024 batch-02 provenance or size');
  }
  const positions = [...manifest.items, ...manifest.held].map(item => item.position).sort((a, b) => a - b);
  if (JSON.stringify(positions) !== JSON.stringify(Array.from({ length: 15 }, (_, i) => i + 26))) {
    throw Error('Every collection position 26–40 must have one disposition');
  }
  const sourceItems = [...manifest.items, ...manifest.held].map(item => item.sourceItem);
  if (new Set(sourceItems).size !== 15 || sourceItems.some(id => !/^\d{5}$/.test(id))) {
...
function prepareBatch(manifest, pool, ledger) {
  ...
  for (const item of manifest.items) {
    ...
    const evidence = `${snapshotPath}#${item.sourceItem}; ${manifestPath}#${item.sourceItem}; ${path.basename(receiptPath)}#${id}; ${path.basename(checkerPath)}; tests/jamb-math-2024-batch-02.test.js`;
