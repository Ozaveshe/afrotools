...
// Replays fourteen owner-directed, short adapted revision items. Myschool's year
...
const batchPath = 'ops/jamb/verification/english-2024-publishable-904.json';
const checkerPath = 'ops/jamb/verification/check-english-2024-904.cjs';
const reviewedAt = '2026-09-26';
const expectedItems = [76, 80, 91, 92, 93, 94, 95, 100, 101, 109, 112, 113, 114, 115];
const auditedItems = Array.from({ length: 40 }, (_, i) => i + 76);
...
function validateManifest(manifest) {
  ...
  if (JSON.stringify([...actualItems, ...held].sort((a, b) => a - b)) !== JSON.stringify(auditedItems)) {
    throw Error('The 40-position audit must classify every position exactly once');
  }
