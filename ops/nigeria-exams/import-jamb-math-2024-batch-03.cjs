...
const manifestPath = 'ops/nigeria-exams/jamb-math-2024-curated-batch-03.json';
const snapshotPath = 'ops/nigeria-exams/jamb-math-2024-source-snapshot-03.json';
const receiptPath = 'ops/jamb/verification/mathematics-2024-publishable-003.json';
const checkerPath = 'ops/jamb/verification/check-mathematics-2024-003.cjs';
...
function validateManifest(manifest) {
  if (manifest.schema_version !== 1 || manifest.observed_at !== '2026-09-25'
      ...
      || !Array.isArray(manifest.held) || manifest.held.length !== 1) {
    throw Error('Unexpected Mathematics 2024 batch-03 provenance or size');
  }
  const positions = [...manifest.items, ...manifest.held].map(item => item.position).sort((a, b) => a - b);
  if (JSON.stringify(positions) !== JSON.stringify(Array.from({ length: 10 }, (_, i) => i + 41))) {
    throw Error('Every collection position 41–50 must have one disposition');
  }
  const sourceItems = [...manifest.items, ...manifest.held].map(item => item.sourceItem);
  if (new Set(sourceItems).size !== 10 || sourceItems.some(id => !/^\d{5}$/.test(id))) {
  ...
  for (const item of manifest.items) {
    if (typeof item.question !== 'string' || item.question.length < 20
        ...
        || (item.source_figure && (!/^https:\/\/myschool\.ng\/storage\/classroom\//.test(item.source_figure.url)
          ...
          || item.source_figure.transcription.length < 80))) {
...
function prepareBatch(manifest, pool, ledger) {
  ...
  const snapshot = {
    ...
    description: 'Individually inspected source pages and adapted mathematical briefs. The Venn diagram was visually transcribed into its adapted prompt. No authenticated sitting, official key, paper order or exam-board licence is asserted.',
  ...
  const receipt = {
    ...
    authority_note: 'Owner-directed public source use; adapted questions and independent calculations. Source figure visually transcribed; source defects repaired only in adapted practice. Collection year only; no official sitting, exam-board licence or teacher approval asserted.',
    records: []
  };
  ...
  for (const item of manifest.items) {
    ...
    const evidence = `${snapshotPath}#${item.sourceItem}; ${manifestPath}#${item.sourceItem}; ${path.basename(receiptPath)}#${id}; ${path.basename(checkerPath)}; tests/jamb-math-2024-batch-03.test.js`;
