...
// Replays eleven owner-directed, short adapted revision items. Myschool's year
// label is a publisher collection label, not proof of a 2025 UTME sitting.
...
const batchPath = 'ops/jamb/verification/english-2025-publishable-004.json';
const checkerPath = 'ops/jamb/verification/check-english-2025-004.cjs';
const reviewedAt = '2026-09-25';
const expectedItems = [246, 247, 249, 250, 251, 252, 253, 254, 255, 256, 257];
const auditedItems = Array.from({ length: 12 }, (_, i) => i + 246);
// The publisher skips question ID 74863 at collection position 252.
const sourceQuestionByItem = new Map([
  [246, 74857], [247, 74858], [248, 74859], [249, 74860],
  [250, 74861], [251, 74862], [252, 74864], [253, 74865],
  [254, 74866], [255, 74867], [256, 74868], [257, 74869]
]);
...
function validateManifest(manifest) {
  if (manifest.schema_version !== 1 || manifest.publisher !== 'Myschool'
      || manifest.collection_year !== 2025 || manifest.year_basis !== 'publisher-collection'
      || manifest.sitting_authenticated !== false
      || !Array.isArray(manifest.records) || manifest.records.length !== expectedItems.length
      || !Array.isArray(manifest.held_source_items)) {
    throw Error('Unexpected 2025 English source manifest');
  }
  ...
  if (JSON.stringify(actualItems) !== JSON.stringify(expectedItems)) {
    throw Error('2025 English source item set changed without review');
  }
  ...
  if (JSON.stringify([...actualItems, ...held].sort((a, b) => a - b)) !== JSON.stringify(auditedItems)) {
    throw Error('The 12-position audit must classify every position exactly once');
  }
  for (const row of manifest.held_source_items) {
    if (row.source_question_id !== sourceQuestionByItem.get(row.source_item)
        || row.source_url !== `https://myschool.ng/classroom/english-language/${row.source_question_id}?exam_type=jamb&exam_year=2025&page=${Math.ceil(row.source_item / 5)}`
        || typeof row.reason !== 'string' || row.reason.length < 20) {
      throw Error('Invalid held source item: ' + row.source_item);
    }
  }
  for (const row of manifest.records) {
    ...
    if (row.source_question_id !== sourceQuestionByItem.get(row.source_item)
        ...
        || url.searchParams.get('exam_year') !== '2025'
        || url.searchParams.get('page') !== String(Math.ceil(row.source_item / 5))
      ...
      throw Error('Invalid curated 2025 English item: ' + row.source_item);
    }
  }
}
...
function prepareBatch(manifest, manifestHash, sourcePool, reviewLedger) {
  ...
  for (const row of manifest.records) {
    ...
    const sourceId = `owner-directed-myschool-english-2025-${row.source_question_id}`;
    ...
    ledger.sources[sourceId] = {
      ...
      label: `Myschool publisher-labelled 2025 English revision item ${row.source_item}; sitting unconfirmed`,
      reuse_authorization: {
