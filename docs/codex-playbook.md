...
- Treat the file shape as the default route source of truth: `foo/index.html` serves `/foo/`, while `foo.html` serves `/foo` with no trailing slash. When the route contract applies an explicit permanent redirect, use its final destination as the preferred public URL. Keep canonicals, hreflang, JSON-LD, sitemap entries, and injected internal links aligned to that resolved route rather than to a stale in-page canonical.
