...
- The same production publisher runs at 18:30 after the PM batch, with 21:30 retry and 08:30 backlog recovery (Asia/Tashkent). It consumes validated ready handoffs oldest first and refuses a verified no-op while a safely recoverable batch is unconsumed or a producer run already due lacks truthful terminal evidence. It scans and resumes its checkpoint before rebuilding. Use the candidate-scoped `automation:release:check`; retain fleet-health errors separately so observer/worktree housekeeping cannot deadlock publication. Delayed receipts require exact-identity source and validation review, never timestamp restamping.
