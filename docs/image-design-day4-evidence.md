...
| 9 | Image to Text | Light-DOM overflow 0 at 320/360/375/768 and 200% after hero min-content fix | Manual + system: panel `#121f33`, text `#eef5ff` | Local OCR reads synthetic receipt text and extracts `TOTAL USD 125.00`; text file rejected explicitly | Signed-out TXT/Markdown/JSON/CSV downloads reopen; matching MIME types verified | OCR runtime, worker, WASM, and five language models now load from bundled same-origin assets; no source data request; 5 URLs created/5 revoked | Title/description tightened; canonical + WebApplication/WebPage/Breadcrumb/FAQ JSON-LD valid; real upload name; 0 unnamed/undersized controls | Accepted |
