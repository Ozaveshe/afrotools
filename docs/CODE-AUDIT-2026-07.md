...
## Sixth pass — closing the two recorded gaps

The fifth pass deliberately left two things unfixed: three Swahili pages that had drifted on the tax logic itself, and three whose PDF button was dead. Both are now closed, and chasing the first surfaced a worse defect on an English flagship.

| # | Area | Severity | Issue | Files |
|---|---|---|---|---:|
| 69 | correctness | **high** | **`tunisia/tn-paye.html` was modelling the wrong country.** It computed a fund it called **CNPS** at 2.5%/9.5% and ran a scale of 0% to TND 300,000 then 2%, 5%, 10%, 15%, 20%, 25% above TND 9,600,000. CNPS is the *Caisse Nationale de Prévoyance Sociale* — Côte d'Ivoire's and Cameroon's fund — and the FAQ expanded the acronym that way; those thresholds are XOF magnitudes, not dinars. The page was an Ivorian calculator with the country name changed. It also **disagreed with itself**: `_grossToNet` already read a `cnss` toggle at 9.18% while the visible calculator read a `cnps` toggle at 2.5%. Corrected to Tunisia's regime — **CNSS 9.68% employee / 17.07% employer** (9.18/16.57 plus Finance Law 2025's 0.5% each for the Unemployment Insurance Fund) and the **Finance Law 2025 IRPP scale** (0% to 5,000; 15/25/30/33/36/38 to 70,000; 40% above), plus the 0.5% Social Solidarity Contribution. Hand-verified at TND 30,000: CNSS 2,904, IRPP 5,378.80, CSS 135.48, net 21,581.72. Markup toggle, helper, band card, comparison table, badges, meta and FAQ JSON-LD all updated; grep for CNPS and for 2.5%/9.5% returns zero. | 2 |
| 70 | correctness | **high** | `sw/uganda` taxed **gross minus NSSF**. URA states in writing, with a worked example at exactly UGX 600,000: *"The gross salary of UGX 600,000 is subjected to PAYE before deducting the 5% NSSF contribution by the employee."* NSSF is not deductible in Uganda. It also charged Local Service Tax as a **flat 100,000/year** above a threshold; the Local Government (Amendment) No.2 Act 2008 Table 1 is a ten-band graduated scale (5,000 to 100,000/yr), transcribed from KCCA's published FAQ. At 600,000 gross that was **8,333/month against 2,500 — a 3.3× overcharge**. Both fixed; the page now matches its English original exactly at 400k, 600k and 1.2M. | 1 |
| 71 | correctness | **high** | `sw/rwanda` deducted RSSB pension at **5%**. RSSB's own pension-reform notice: from **January 2025** the rate rose from 6% to 12%, split **6% employee / 6% employer**, rising 2%/yr from 2027 to 20% by 2030. (The RRA page still shows the superseded 3%+3%.) Separately, the 0.3% employee maternity levy was computed but **never deducted from the employee** — it was added to the *employer's* cost instead, so the page overstated net pay and misattributed the charge. Both fixed; matches the English page exactly at 300k, 600k and 1.5M. | 1 |
| 72 | correctness | **high** | `sw/burundi` computed INSS on **uncapped gross** and applied the secondary-employment flat 30% to **gross** rather than to taxable income, while its own primary path used `gross − INSS`. INSS Burundi publishes: pension **4% worker + 6% employer capped at 450,000 FBU/month**; occupational risks **3% employer-only capped at 80,000 FBU/month**. The employer side was also a flat 6% of uncapped gross with no risk component. All fixed; matches the English page exactly at 300k, 600k and 1.5M. | 1 |
| 73 | correctness | med | The "Pakua PDF" button on `sw/liberia`, `sw/sierra-leone` and `sw/guinea-bissau` called `window.openPayePdfModal` — **defined nowhere in the repository** — and otherwise alerted "PDF inatengenezwa…" ("PDF is being generated"), which was untrue. None of the three loaded `/assets/js/lib/pdf-template.js`, which is what provides `window.AfroTools.pdf`. All three now load it and generate a real Swahili summary. Driven headless with a stub: Liberia LRD 368,625 net on 500,000 gross, Sierra Leone SLE 355,000, Guinea-Bissau XOF 129,940 — each matching its own page's calculation. | 3 |

### Where the Swahili estate stands now

Driving gross 600,000 through all 47 Swahili pages and their English originals: **33 agree on net pay** (was 29), **1 diverges**, **13 are not comparable**.

The one divergence is **Kenya, and it is a harness artifact, not a defect**: `kenya/ke-paye.html:775` sets its own input to `'50,000'` at load, overwriting whatever the driver supplied, so the English side never received the test salary. The 13 "not comparable" are pages whose result objects use different shapes or whose `calculate` is not a global — a limitation of the comparison, not evidence of a bug.

### Still open

- **Tunisia's professional-expenses deduction is not modelled** on either the English or the Swahili page. Recorded rather than guessed: PwC's deductions page does not give the rate and cap, and the unsourced TND 1,200 relief the Swahili page carried was removed rather than kept.
- **KCCA's worked example assesses Uganda's LST on gross and deducts it *before* PAYE**; both AfroTools pages assess it on take-home after PAYE and do not reduce the PAYE base. The two Acts genuinely conflict and KCCA's FAQ says so. The pages now agree with each other; whether to adopt KCCA's ordering is a separate call that changes the headline PAYE figure.
