...
- Curated French widget parent pages are generated by `node scripts/generate-fr-widget-parent-pages.js`.
- These pages live under `/fr/widgets/<slug>/` and should link to the full French tool experience when one exists.
- Keep the iframe URL as technical embed code only; do not promote `widgets/iframe/` routes as French SEO pages.
- The French localization ledger treats these parent pages as generated French widget output, not as mapped English completion or registry-eligible money pages.
- After changing the curated list, run `node scripts/generate-fr-widget-parent-pages.js`, `npm run seo:widgets`, the French ledger, and the usual i18n/hreflang checks.
