...
Scheduled runs cap new article inserts at five per invocation. Existing rows can still receive creator-mention links without consuming that insert budget, but recurring feed observations must not rewrite an existing row's title, slug, category, or publication state. This preserves reversible editorial unpublishes and manual corrections. The cap limits burst risk; it does not replace source review, duplicate control, or the 48-hour editorial audit.
