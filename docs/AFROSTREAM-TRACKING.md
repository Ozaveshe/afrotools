...
Scheduled and manual live runs cap new article inserts at five per invocation, with a slot reserved before each concurrent insert. Netlify scheduled deliveries also reserve the current fire before reading any source: the monitor normalizes the platform `event.body.next_run`, derives a deterministic UUID from that value and the scraper id, and inserts it as the `public.scraper_runs.id` with the schema-allowed provisional `status=anomaly`, a pending-final-status message, and `resolution=ignore-duplicates`. An overlapping or retried delivery for the same `next_run` receives HTTP 200 with `duplicate_delivery_skipped=true` and performs no feed, news, mention, or source-health work; the owner patches the reserved row to `ok` or `error` when it finishes. If the owner is interrupted before that patch, the anomaly remains visible instead of claiming success. The reservation deliberately provides at-most-once processing per six-hour slot: a same-slot retry after `error` or an interrupted `anomaly` is skipped, and the next natural fire recovers still-recent items through the three-day scheduled lookback. This favors burst prevention over immediate same-slot retry recovery. It uses the existing UUID primary key and status constraint, so it needs no schema migration. Manual runs retain their independent run rows, and dry-run mode never creates a reservation or any other write.

Existing rows can still receive source-matching creator-mention links without consuming the article insert budget, but recurring feed observations must not rewrite an existing row's title, slug, category, or publication state. Feeds that reuse one GUID across multiple items use each article link for identity so one row cannot rotate through unrelated stories and creator mentions. Dry-run mode can report more than five candidates because it performs no inserts. These controls preserve reversible editorial unpublishes and manual corrections; they do not replace source review, duplicate control, or the 48-hour editorial audit.

The public AfroStream health response exposes `automation.news_monitor_scheduled_48h` as aggregate operational evidence: scheduled-run rows, distinct six-hour slots, duplicate slots, and extra rows in those duplicate slots. It includes only slot timestamps and counts, never reservation ids, source URLs, or article data. The slot grouping is based on each scheduled row's `fetched_at`, so it detects historical duplicate rows or a future bypass of the reservation; skipped conflict attempts intentionally create no extra row and therefore are not counted. `metrics_complete=false` means the 200-row observation cap was reached and duplicate details may be incomplete. After the idempotency change is deployed, duplicate counts should fall to zero as repeated deliveries resolve to the same reserved row; a latest news-monitor row with `status=anomaly` and the pending-final-status message indicates that the reservation was acquired but did not reach its final patch yet.
