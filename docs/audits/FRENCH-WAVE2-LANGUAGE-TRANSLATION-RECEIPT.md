...
The preservation review on `codex/fr-wave2-language-repair` strengthened the
acceptance contract without expanding category scope:

- removed residual English from visible UI, help, methodology, structured FAQ
  data, status/export copy and related-tool labels;
- corrected Zulu related links so they resolve to the actual French routes;
- made the Zulu save action persist the generated result, not only its filters;
- reopened every downloaded TXT file with a fatal UTF-8 decoder and verified
  its route oracle;
- parsed every advertised save record and read every copied result;
- exercised every advertised translator/converter direction;
- verified no app action sends input through fetch, XHR, beacon, form
  navigation or another document request;
- verified 320px, 375px and 200% text reflow, explicit light/dark themes and a
  separate automatic system-dark session;
- reviewed all 11 previews semantically and replaced two generic/text-heavy
  images with dedicated, text-free French previews.

Final fail-closed result: **11 accepted, 0 blocked**. Detailed route evidence is
recorded in `reports/french-language-wave2-repair-browser-evidence.json`.
