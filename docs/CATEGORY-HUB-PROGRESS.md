...
| 11 | Mining & Commodities | `/mining/` | ✅ complete | ✅ | ✅ | ✅ | opus-mining-2026-07-17 | 2026-07-17: hub was a facade (10 dead `/mining/` self-links, 0 tools). Now **6 live tools** — mining-royalty (18 jurisdictions), diamond-valuation, oil-well-production, oil-gas-revenue, artisanal-mining-income + commodity ref; 4 honestly marked Planned. Ledger + validator + `.claude/rules/mining.md` + `npm run test:mining` (FAQ↔JSON-LD parity + hub honesty). **Rate review: 8 of 12 defaults were stale** — NG gold 3%→15%, GH gold→sliding 5-12%, KE all 4 cut, ZW Pt 2.5%→7%, ZM cobalt→8%, ML/BF→sliding; 6 gap jurisdictions bound. Deploys cee469c, +this |
