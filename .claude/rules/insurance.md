...
- Only 19 of 54 markets have a bound insurance-regulator URL today. The other 35 live in the ledger's `gaps.regulatorsWithoutUrl` block. An entry there is a known liability, not a to-do. Do not delete one without either finding the source or showing the figure as planning-grade.
- Statutory health-fund rates (NHIA, SHIF/SHA, AMO/CNSS, CNAM, Mutuelle, CBHI, CMU) are set by national health funds, **not** by the insurance regulator. They are recorded as a separate unsourced class. Bind each fund's circular before presenting a rate as current.
