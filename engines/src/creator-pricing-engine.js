var CreatorPricingEngine = function() {
  ...
  var e = {
    ...
    photography: [ {
      name: "Half-day shoot (4hrs)",
      hourMult: .5
    }, {
      name: "Full-day shoot (8hrs)",
      hourMult: 1
    }, {
      name: "Wedding (full day)",
      hourMult: 2.5
    }, {
      name: "Per edited photo",
      hourMult: .04
    }, {
    ...
    videography: [ {
      name: "Short-form video (30–60s)",
      hourMult: .6
    }, {
      name: "Music video",
      hourMult: 2
    }, {
      name: "Corporate video (3–5 min)",
      hourMult: 1.5
    }, {
      name: "Wedding film",
      hourMult: 2.5
    ...
    design: [ {
      ...
      name: "Social media set (10 posts)",
      hourMult: 1
    }, {
      name: "Flyer / poster",
      hourMult: .4
    ...
    music: [ {
      ...
      name: "Album production (10 tracks)",
      hourMult: 12
    } ],
    writing: [ {
      ...
      name: "Website copy (5 pages)",
      hourMult: 1.5
    }, {
      name: "Social media captions (30)",
      hourMult: .8
    }, {
      name: "Press release",
      hourMult: .5
    }, {
      name: "Ebook / whitepaper",
      hourMult: 3
    } ],
    development: [ {
      name: "Landing page",
    ...
    voiceover: [ {
      name: "Radio commercial (30s)",
      hourMult: .5
    }, {
      name: "Narration (per minute)",
      hourMult: .15
    }, {
  ...
  function u(e, a) {
    return e < 100 ? 5 * Math.round(e / 5) : e < 1e3 ? 50 * Math.round(e / 50) : e < 1e4 ? 500 * Math.round(e / 500) : e < 1e5 ? 5e3 * Math.round(e / 5e3) : 1e4 * Math.round(e / 1e4);
  }
  function m(a, n) {
    var o = null;
    for (var i in e) {
    ...
    var r = o ? o.symbol : n + " ";
    return a >= 1e6 ? r + (a / 1e6).toFixed(1).replace(/\.0$/, "") + "M" : r + a.toLocaleString("en-US");
  }
  return {
    COUNTRIES: e,
    CRAFTS: a,
    EXPERIENCE_LEVELS: n,
    PROJECT_TYPES: l,
    calculateRate: function(a) {
      var l = a.craft || "other", m = a.specialty || "", c = a.country || "NG", s = a.city || "", d = a.experience || "established", h = e[c] || e.NG, g = a.currency || h.currency, M = t[g] || 1, p = o[l] || o.other, y = p.min, b = p.max, A = p.median, C = i[m] || 1;
      y *= C, b *= C, A *= C;
      var f = n.find(function(e) {
        return e.id === d;
      }) || n[2];
      y *= f.multiplier, b *= f.multiplier, A *= f.multiplier;
      var S = r[s] || 1;
      y *= S, b *= S, A *= S;
      var v = .6 + .8 * (h.col || .5);
      y *= v, b *= v, A *= v;
      var P = Math.round(y * M), E = Math.round(b * M), F = Math.round(A * M);
      P = u(P), E = u(E), F = u(F);
      var G = Math.round(P / 8), B = Math.round(E / 8);
      return {
        daily: {
          min: P,
          max: E,
        ...
        hourly: {
          min: u(G),
          max: u(B),
          median: u(Math.round(F / 8))
        },
        project: {
          min: Math.round(1.5 * P),
          max: Math.round(2.5 * E),
          median: Math.round(2 * F)
        },
        currency: g,
        symbol: h.symbol,
        experience: f,
    ...
    getComparison: function(e, a) {
      var n = a.daily.median, o = Math.round(.85 * n), i = Math.round(.7 * n), t = Math.round(1.6 * n), l = {
        beginner: 20,
        developing: 35,
        established: 55,
        expert: 72,
        master: 88
      }[e.experience] || 50, m = r[e.city] || 1;
      return m > 1.2 && (l = Math.min(l + 8, 95)), m < .85 && (l = Math.max(l - 8, 10)),
      {
        yourEstimate: n,
        cityAvg: u(o, a.currency),
        nationalAvg: u(i, a.currency),
        top10pct: u(t, a.currency),
        percentileRank: l
      };
    },
    getBreakdown: function(e, a) {
      return (l[e] || l.other).map(function(e) {
        var n = u(Math.round(a.daily.min * e.hourMult), a.currency), o = u(Math.round(a.daily.max * e.hourMult), a.currency);
        return {
          name: e.name,
          min: n,
          max: o
    ...
    formatRate: m,
    formatRange: function(e, a, n) {
      return m(e, n) + " — " + m(a, n);
    },
    getSpecialties: function(e) {
      var n = a.find(function(a) {
        return a.id === e;
      });
      return n ? n.specialties : [];
    },
