...
- French HTML numeric/hexadecimal encoding now uses the same Unicode engine as English; French URL encoding adds RFC 3986 and form-value modes. Four coordinator browser tests passed with the analytics-disabled seam; the default analytics request guard is being refined separately.
...
- Read-only French category sweep ran 27 commands across 26 categories: 24 passed initially. Tunisia's inventory mismatch was repaired; developer evidence hashes and engineering generation composition still require resolution. Six categories had no permitted static command in that sweep and were not counted as passed.
