...
Preview test setup was separately corrected: local-storage fixtures now apply only to the top-level document, avoiding writes in third-party/sandboxed preview frames. The study journey waits for the same explicit planner-ready flag through Playwright's navigation-aware function wait; a transient navigation destroyed the previous one-shot evaluation context. Product error assertions and expected results are unchanged. The combined browser run must validate these harness changes before release.

This is local source/browser proof. Cache keys, deploy artifact, hosted CI and production evidence remain pending the combined release. No claim of authenticated education cloud synchronization or real student retention is made.
