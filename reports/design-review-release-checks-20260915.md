...
- Repeated `npm run build:deploy`: passed (main build 539.3 seconds; 18,074 files packaged). Final service-worker stamp uses the current navbar version. After the country-picker keyboard repair, scoped cache regeneration, final `build-dist.js` packaging, repeated security scanning, and `npm run audit:dist` also passed.
