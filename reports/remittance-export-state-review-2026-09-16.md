...
Updated relevant FR/SW fixtures to supply confirmed country inputs; native oracle hashes are deliberately unchanged pending reviewed evidence refresh. French alias test now locates the effective-rate metric by native label, replays the corridor flag/country inputs and expects its intended canonical main route. Five targeted legacy cases passed11.1s (English main/alias workflow and French invalid/reset/mobile/zoom/theme/metadata).

The full historical FR alias suite retained a failure for real Google analytics requests under declined consent. Source inspection finds lazy-analytics.js intentionally initializes consent-mode cookieless pings; this is analytics metadata, not an essential static SDK request. Synthetic quote content was not observed in these requests. The no-network assertion has not been weakened. Full private diagnostics remain in remittance-legacy-contract traces; no request identifiers are copied into this report. Scoped consent correction is a separate candidate.

## Scoped analytics consent correction

The shared lazy analytics loader now requires accepted consent before initializing analytics on the six checked-quote main/alias routes (including index.html spellings). Ordinary pages retain their existing consent-mode policy. Decline/withdrawal enables GA collection suppression; reload remains analytics-free. The local consent manager still loads so users can change their choice. This does not change calculation/export behavior or send quote content.

Evidence: all five historical French alias workflow tests now pass18.4s, including the unchanged no-remote-request assertion. Seven final live-browser consent cases pass18.5s: six main/alias routes plus direct index.html, declined initial state with no analytics requests, explicit acceptance initiating the real Google tag request (no provider mocking), withdrawal suppression, declined reload with no analytics requests, and no synthetic quote label in observed URL/body traffic. This proves initialization/request boundaries, not delivery of an event to a provider dashboard. Existing analytics-consent node tests and JS syntax pass. Build-managed loader version hashes require coordinator build regeneration; no generated page hashes were manually patched.

## Final scoped quality and native-owner review

Only formula-engines-remittance-quote-comparator-engine was refreshed through the protected reviewed-change command. Its previous generic third-party FX snapshot provenance was inaccurate for this user-entered comparator. Metadata now describes entered quotes and CLDR locale-name reference only, with explicit lack of provider freshness/service eligibility verification and separate crypto caller ambiguity. Unicode CLDR48 French summary was inspected for locale-name provenance: https://www.unicode.org/cldr/charts/48/summary/fr.html . This is not a tariff source.

Nine independent literal golden cases cover same corridor, differing destination/origin, native aliases preserving raw names, unknown/missing country, exact expiry boundary, unequal debit and excessive fee. The runner checks the compiled artifact. A negative mutation of the expected destination-group count is detected. All351 existing+new golden fixtures passed; the two focused quality tests passed. Native20/20 extracted contracts and14/14 owner checks passed after refreshing only the two reviewed remittance owners and adding engine/runtime dependency fingerprints. Their historical root capture marker remains unchanged, with per-row reviewedFrom88baea24 and this report as new evidence.

Semantic preservation checks confirmed every unrelated formula and fixture, all other12native rows, and the global historical capture marker unchanged. The full build-calculation-quality --check and broad calculation-quality test remain blocked at the inherited2dd1a348 inventory missing assets/js/engines/senegal-child-leave.js. No unrelated inventory refresh was made. The coordinator must rerun that gate against its current integrated source. No full build, deployment or provider-dashboard proof is asserted.
