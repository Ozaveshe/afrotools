...
- `node --test tests/cv-ats-pdf-character-preservation.test.js`: 11 passed. Full text extraction checks for EN/FR/SW, extended Latin, NFC combining accents, punctuation, long-word wrapping, multipage content, unsupported-character failure and pinned source hashes.
...
- `playwright test tests/e2e/cv-ats-pdf-character-preservation.spec.js --workers=1`: first seven tests passed together, eighth stale-letter test passed separately after adding it. Static server is this worktree at port 4216, with analytics disabled and external requests blocked.
...
- A browser check found the CV plain-text builder still emits English `References` on French and Swahili. This needs source-owned section-label localization separately.
