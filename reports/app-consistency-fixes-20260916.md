...
Final narrow-screen check: Annual selected at 320px initially exposed a 3px overflow from the longer result amount. The result column now permits shrinking and the mobile amount scales/wraps safely. Rechecked after the fix: document/client width both 314px (320px viewport less scrollbar), annual amount and breakdown remain visible. Annual selection via Enter exposes aria-pressed=true. Inline scripts in both edited HTML pages compile successfully.
