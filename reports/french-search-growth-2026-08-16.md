...
A source-owned override registry now records only routes whose search-facing content changed materially. The sitemap owner validates every selector and date, treats the narrow reviewed registry as authoritative, and supports a prefix only for the one generator-owned fuel family that was rebuilt in full. The 11 exact repaired routes plus the fuel hub and all 54 country pages now expose 16 August 2026, while unrelated French and English routes retain their historical values. A regression test proves the covered routes, the 55-page family denominator, stated reasons and the absence of broad `/`, `/fr/`, `/fr/tools/` or `/fr/blog/` restamps.
...
- Sixty-six materially rebuilt French routes now expose a reviewed 16 August 2026 sitemap modification date without restamping unchanged routes. The selective freshness contract passes its focused suite.
