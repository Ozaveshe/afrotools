...
test('recent publisher-labelled JAMB candidates have independently verified answers without assuming official provenance',()=>{
 ...
 assert.ok(intake.records.every(q=>q.complete_options_observed));assert.equal(intake.status,'reviewed-companions-pending-release');assert.match(intake.year_status,/not been authenticated/);
});
