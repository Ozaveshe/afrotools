...
const batch=require('../ops/jamb/review-candidates/crk/crk-wave014.json');
const {verify}=require('../ops/jamb/review-candidates/crk/check-crk-wave014.cjs');
...
test('crk wave014 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,35);
});
test('crk wave014 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,35);
