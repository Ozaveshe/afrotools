...
const additions=require('../data/image-generation/placement-lifecycle-additions.json');
...
assert.equal(additions.schema_version,1);assert.ok(/^\d{4}-\d{2}-\d{2}$/.test(additions.reviewed_at));assert.ok(additions.images.length>0);
const allDecisions=[...decisions.images,...additions.images];assert.equal(new Set(allDecisions.map(x=>x.path)).size,allDecisions.length);
for(const d of additions.images){const asset=library.images.find(x=>x.path===d.path);assert.equal(crypto.createHash('sha256').update(fs.readFileSync(path.join(ROOT,d.path))).digest('hex'),d.sha256);assert.ok(d.owner&&d.reason);assert.notEqual(d.decision,'active','Post-cohort additions must resolve a non-active lifecycle');assert.equal(asset.placement_decision,d.decision);assert.equal(asset.status,d.decision);assert.equal(asset.placements.length,0,'Rejected or retired post-cohort asset must not retain a live binding '+d.path);}
...
console.log('PASS: all 792 frozen-cohort hashes retained, post-cohort lifecycle additions verified, real active bindings, exact candidate owners, idempotent blog refresh.');
