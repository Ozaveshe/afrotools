...
const batch=require('../ops/jamb/review-candidates/crk/crk-wave022.json');
const {verify}=require('../ops/jamb/review-candidates/crk/check-crk-wave022.cjs');
...
test('crk wave022 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,33);
});
test('crk wave022 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,33);
