...
const { prepareBatch, manifestPath, batchPath } = require('../ops/nigeria-exams/import-jamb-english-2025-vantelum-batch-02.cjs');
const { verify } = require('../ops/jamb/verification/check-english-2025-006.cjs');

const read = relative => JSON.parse(fs.readFileSync(path.join(root, relative), 'utf8'));

test('English 2025 Vantelum revision batch 2 is independently checked and idempotent', () => {
  ...
  assert.deepEqual(manifest.held_source_items.map(row => row.source_item), [23, 28, 38]);
  ...
  for (const item of [23, 28, 38]) {
    assert.equal(pool.questions.some(question => question.id === `english-2025-vantelum-${item}`), false);
  }
  ...
  const eligibleCount = pool.questions.filter(question => question.subject === 'english'
    && question.year === 2025 && assessQuestion(question, ledger).state === 'eligible').length;
  assert.ok(eligibleCount >= 65);
...
test('English 2025 Vantelum batch 2 rejects forged year and poisoned key', () => {
  const bytes = fs.readFileSync(path.join(root, manifestPath));
  const manifest = JSON.parse(bytes);
  const hash = crypto.createHash('sha256').update(bytes).digest('hex');
  const pool = read('ops/jamb/source-pool.json');
  ...
  acceptedAndHeld.held_source_items.push({ source_item: 16, reason: 'Synthetic conflict: this item is both held and accepted.' });
  ...
  const id = 'english-2025-vantelum-16';
  ...
  record.independently_selected_answer = 'physically strong';
