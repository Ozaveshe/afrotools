...
const batch=require('../ops/jamb/review-candidates/english/english-1980-002.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1980-002.cjs');
...
test('wave177 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,15);
});
test('wave177 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,15);
 assert.equal(verify(after,true).held,5);
