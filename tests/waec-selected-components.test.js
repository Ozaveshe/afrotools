...
test('selected WAEC component evidence matches all current briefs without claiming complete papers',()=>{
 assert.equal(manifest.components.length,3);assert.equal(manifest.sources.length,8);
 ...
 assert.deepEqual(manifest.components[2].expectedIds,['waec-2023-mathematics-p2-q10']);
});
