...
assert.strictEqual(directory.countries.filter(country => country.route).length, 54, 'every African country destination should be marked supported');
...
assert.strictEqual(directory.resolveCountry('KE', 'sw').route, '/sw/kenya/kikokotoo-kodi-mshahara/');
assert.strictEqual(directory.resolveCountry('KE', 'sw').localized, true);
assert.strictEqual(directory.resolveCountry('KE', 'sw').languageFallback, false);
assert.deepStrictEqual(directory.resolveCountry('GW', 'en'), {
  code: 'GW',
  name: 'Guinea-Bissau',
  supported: true,
  route: '/guinea-bissau/gw-paye',
  localized: false,
  languageFallback: false
});
...
for (const relative of ['tools/paye-calculator/index.html', 'fr/tools/calculateur-paye/index.html', 'sw/mshahara-na-kodi/paye/index.html']) {
...
const swahiliHtml = fs.readFileSync(path.join(root, 'sw/mshahara-na-kodi/paye/index.html'), 'utf8');
...
assert.strictEqual(browseLinkCount(englishHtml), 54, 'English crawlable index should expose every supported country route');
assert.strictEqual(browseLinkCount(frenchHtml), 21, 'French crawlable index should expose every localized country route');
assert.strictEqual(browseLinkCount(swahiliHtml), 54, 'Swahili crawlable index should expose every native country route');
assert.doesNotMatch(englishHtml, /Guinea-Bissau — unsupported/);
assert.match(englishHtml, /href="\/guinea-bissau\/gw-paye">Guinea-Bissau/);
assert.match(swahiliHtml, /data-afrotools-source-owner="scripts\/build-sw-paye-country-directory\.js"/);
