...
test('the 2024 year page discloses provenance and keeps all 32 worked answers closed by default', () => {
  const page = renderYear('mathematics', '2024', pool.questions, ledger, ['2024', '2024']);
  assert.equal(page.approvedIds.length, 32);
  ...
  for (const item of manifest.held) assert.ok(!page.html.includes(`mathematics-2024-myschool-${item.sourceItem}`));
});
