...
test('three route owner is repeatable and preserves manual tools, SEO and machine keys',()=>{
 for(const [locale,file]of Object.entries(owner.routes)){
  const html=fs.readFileSync(file,'utf8'),built=owner.build(html,locale);assert.equal(built,html);assert.equal(owner.build(built,locale),built);
  const processed=html.replace('/assets/js/pages/fuel-tracker-vip.js"','/assets/js/pages/fuel-tracker-vip.js?v=test"').replace('/assets/css/fuel-tracker-vip.css"','/assets/css/fuel-tracker-vip.css?v=test"');assert.equal(owner.build(processed,locale),html);
