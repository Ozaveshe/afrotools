...
const batch=require('../ops/jamb/review-candidates/english/english-1984-002.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1984-002.cjs');
...
test('wave147 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,10);
});
test('wave147 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,10);
 assert.equal(verify(after,true).held,10);
