...
const batch=require('../ops/jamb/review-candidates/english/english-1982-006.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1982-006.cjs');
...
test('wave157 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,2);
});
test('wave157 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,2);
 assert.equal(verify(after,true).held,18);
