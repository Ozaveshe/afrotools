...
const batch=require('../ops/jamb/review-candidates/english/english-1988-009.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1988-009.cjs');
...
test('wave117 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,5);
});
test('wave117 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,5);
 assert.equal(verify(after,true).held,15);
