...
const batch=require('../ops/jamb/review-candidates/english/english-1981-005.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1981-005.cjs');
...
test('wave174 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,14);
});
test('wave174 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,14);
 assert.equal(verify(after,true).held,6);
...
test('cumulative English examination IDs remain unique across all batches',()=>{
 ...
 assert.ok(seen.size>=3903);
});

test('wave174 permits the source noun but rejects appended editorial disclosures',()=>{
 const {questionFingerprint}=require('../scripts/lib/jamb-content-trust');
 const row=batch.records.find(r=>r.candidate.passage?.includes('compilation, arrangement and transmission to prophetic disciples'));
 assert.ok(row);
 const originalCandidate=row.candidate,originalHash=row.content_sha256;
 try {
  for(const text of [' This compilation was reviewed.',' Repair history: revised wording.']){
   row.candidate={...originalCandidate,passage:originalCandidate.passage+text};
   row.content_sha256=questionFingerprint(row.candidate);
   assert.throws(()=>verify(after.map(q=>q.id===row.id?row.candidate:q),true),e=>e.operator==='doesNotMatch');
  }
 } finally {row.candidate=originalCandidate;row.content_sha256=originalHash;}
 assert.equal(verify(after,true).candidates,14);
});
