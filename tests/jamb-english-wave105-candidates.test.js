...
const batch=require('../ops/jamb/review-candidates/english/english-1989-001.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1989-001.cjs');
...
test('wave105 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,18);
});
test('wave105 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,18);
 assert.equal(verify(after,true).held,2);
