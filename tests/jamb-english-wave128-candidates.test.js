...
const batch=require('../ops/jamb/review-candidates/english/english-1986-001.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1986-001.cjs');
...
test('wave128 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,7);
});
test('wave128 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,7);
 assert.equal(verify(after,true).held,13);
