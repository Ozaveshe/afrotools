...
const batch=require('../ops/jamb/review-candidates/crk/crk-wave013.json');
const {verify}=require('../ops/jamb/review-candidates/crk/check-crk-wave013.cjs');
...
test('crk wave013 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,34);
});
test('crk wave013 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,34);
