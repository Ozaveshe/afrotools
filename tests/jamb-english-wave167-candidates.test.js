...
const batch=require('../ops/jamb/review-candidates/english/english-1982-016.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1982-016.cjs');
...
test('wave167 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,4);
});
test('wave167 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,4);
