...
const batch=require('../ops/jamb/review-candidates/english/english-1990-005.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1990-005.cjs');
...
test('wave102 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,8);
});
test('wave102 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,8);
 assert.equal(verify(after,true).held,12);
