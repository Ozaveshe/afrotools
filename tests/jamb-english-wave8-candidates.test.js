...
const batch=require('../ops/jamb/review-candidates/english/english-2015-003.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-2015-003.cjs');
...
test('regional idiom evidence allows only its reviewed column URL',()=>{
 const r=batch.records.find(r=>r.semantic_review.source_url.startsWith('https://punchng.com/'));
 const original=r.semantic_review.source_url;
 try {
  r.semantic_review.source_url='https://punchng.com/unreviewed-column/';
  assert.throws(()=>verify(before));
 } finally {r.semantic_review.source_url=original;}
 assert.equal(verify(before).candidates,24);
});
test('wave8 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,24);
 assert.equal(verify(after,true).held,16);
