...
test('source owner has exactly the three non-overlapping construction-planning apps', () => {
  assert.equal(manifest.owner, 'scripts/build-sw-property-construction-planning-parity.js');
  assert.equal(manifest.count, 3);
  assert.deepEqual(manifest.rows.map((row) => row.englishId), ['building-materials', 'construction-budget', 'home-renovation-cost']);
});
...
test('unchanged shared English engine returns the exact fixture for all three routes', () => {
  for (const row of manifest.rows) {
...
test('shared engine fails closed at the exact input boundaries', () => {
  for (const id of ['building-materials', 'construction-budget', 'home-renovation-cost']) {
...
test('all three routes are native Swahili products with exactly five local exports', () => {
  for (const row of manifest.rows) {
    const page = html(row);
...
test('canonical AI catalog covers all three English owners without generated Swahili mutation', () => {
...
test('source generator is idempotent with exact three-route ownership', () => {
  ...
  assert.match(output, /3\/3; 0 changed outputs/);
