...
const batch=require('../ops/jamb/review-candidates/english/english-1992-010.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1992-010.cjs');
...
test('wave85 rejects self-consistently rehashed answer and numbering mutations',()=>{
 const {questionFingerprint}=require('../scripts/lib/jamb-content-trust');
 const row=batch.records[0],originalCandidate=row.candidate,originalHash=row.content_sha256;
 try {
  for(const mutation of [{answer:'A'},{num:999}]){
 ...
 assert.equal(verify(after,true).candidates,12);
});
test('wave85 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,12);
 assert.equal(verify(after,true).held,8);
