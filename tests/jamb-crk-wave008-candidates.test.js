...
const batch=require('../ops/jamb/review-candidates/crk/crk-wave008.json');
const {verify}=require('../ops/jamb/review-candidates/crk/check-crk-wave008.cjs');
...
test('crk wave008 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,27);
});
test('crk wave008 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,27);
