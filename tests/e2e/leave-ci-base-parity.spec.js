...
test('fr: CI base, remaining amount and parsed reports retain unrounded limitations',async({page})=>{
 ...
 const result=page.locator('.frhr-result[data-result]');await expect(result).toContainText('26.4 jours');await expect(result).toContainText('22.4 jours');await expect(result).toContainText('non arrondie');
