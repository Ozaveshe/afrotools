...
function failures(page) {
  const found = [];
  page.on('console', message => { if (message.type() === 'error') found.push(`console:${message.text()}`); });
  ...
  page.on('response', response => { if (response.status() >= 400) found.push(`http:${response.status()} ${response.url()}`); });
  return found;
}
...
test('accepted English Farm Size workflow delegates to shared engine', async ({ page }) => {
  await sentinel(page);
  const found = failures(page);
  await page.addInitScript(() => localStorage.setItem('afrotools_cookie_consent', 'rejected'));
  await page.goto('/agriculture/farm-size-converter/');
  await page.fill('#inputVal', '17');
  await page.selectOption('#fromUnit', 'feddan');
  await page.selectOption('#toUnit', 'acre');
  await page.getByRole('button', { name: 'Convert' }).click();
  const runtime = await page.evaluate(() => ({
    latest: window.FARM_SIZE_LAST_RESULT,
    data: window.AfroTools.FarmSizeData,
  }));
  expect(runtime.latest).toEqual(engine.calculate({ amount: 17, fromKey: 'feddan', toKey: 'acre' }, runtime.data));
  await expect(page.locator('#results')).toHaveClass(/show/);
  expect(found).toEqual([]);
});

test('French Farm Size physical-route acceptance', async ({ page, context }) => {
  await sentinel(page);
  const found = failures(page);
  ...
  await page.goto('/fr/agriculture/farm-size-converter/');
  await expect(page.locator('iframe')).toHaveCount(0);
  await expect(page.locator('html')).toHaveAttribute('lang', 'fr');
  await expect(page.locator('link[rel=canonical]')).toHaveAttribute('href', 'https://afrotools.com/fr/agriculture/farm-size-converter/');
  await expect(page.locator('link[rel=alternate][hreflang=en]')).toHaveAttribute('href', 'https://afrotools.com/agriculture/farm-size-converter/');
  ...
  expect(ai.routes['/agriculture/farm-size-converter/']).toBe('/fr/agriculture/farm-size-converter/');
  ...
  await page.fill('#amount', '17');
  await page.selectOption('#fromUnit', 'feddan');
  await page.selectOption('#toUnit', 'acre');
  const button = page.getByRole('button', { name: 'Convertir' });
  await button.focus();
  ...
  const runtime = await page.evaluate(() => ({
    ...
    data: window.__FR_AGRI_TEST__.data,
  }));
  expect(runtime.latest).toEqual(engine.calculate({ amount: 17, fromKey: 'feddan', toKey: 'acre' }, runtime.data));
  expect(runtime.report.sources.donneesEnDirect).toBe(false);
  expect(runtime.report.conversions).toHaveLength(27);
  for (const item of [
    { name: 'Exporter en TXT', ext: '.txt', verify: value => expect(value.toString('utf8')).toContain('Confidentialité') },
    { name: 'Exporter en CSV', ext: '.csv', verify: value => expect(value.toString('utf8')).toContain('football_pitch') },
    { name: 'Exporter en JSON', ext: '.json', verify: value => expect(JSON.parse(value.toString('utf8')).entrees.fromKey).toBe('feddan') },
  ...
  expect(JSON.parse(await page.evaluate(() => localStorage.getItem('afrotools:fr-agriculture:farm-size'))).entrees.fromKey).toBe('feddan');
  await page.getByRole('button', { name: 'Copier' }).click();
  await expect.poll(() => page.evaluate(() => navigator.clipboard.readText())).toContain('Convertisseur de surface');
  await page.getByRole('button', { name: 'Inverser les unités' }).click();
  expect(await page.inputValue('#fromUnit')).toBe('acre');
  ...
  await expect(page.getByRole('alert')).toContainText('positive ou nulle');
