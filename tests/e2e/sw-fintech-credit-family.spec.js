...
const apps = [
  {
    id: 'loan-shark-compare', english: '/tools/loan-shark-compare/', swahili: '/sw/zana/mkopeshaji-hatari-dhidi-ya-benki/', art: [600, 400],
    values: { '#ls-currency': 'KES', '#ls-amount': '500000', '#ls-tenor': '6', '#ls-shark-rate': '20', '#ls-bank-rate': '24' },
    result: '#ls-results', metrics: ['#ls-overpay', '#ls-shark-total', '#ls-bank-total', '#ls-shark-apr-val', '#ls-bank-apr-val', '#ls-multiple'], dirty: '#ls-shark-rate', invalid: '#ls-amount'
  },
  {
    id: 'microfinance-loan', english: '/tools/microfinance-loan/', swahili: '/sw/zana/kikokotoo-mkopo-wa-microfinance/', art: [800, 450],
    values: { '#mf-currency': 'KES', '#mf-amount': '200000', '#mf-rate': '3.5', '#mf-rate-type': 'flat', '#mf-tenor': '12', '#mf-fees': '5000', '#mf-bank-rate': '24' },
    result: '#mf-results', metrics: ['#mf-monthly', '#mf-total', '#mf-interest', '#mf-apr', '#mf-bank-total'], dirty: '#mf-rate', invalid: '#mf-amount'
  },
  {
    id: 'digital-lending', english: '/tools/digital-lending/', swahili: '/sw/zana/mikopo-ya-kidijitali/', art: [800, 450],
    values: { '#dl-currency': 'KES', '#dl-amount': '50000', '#dl-days': '30', '#dl-name-a': 'Mtoa A', '#dl-total-a': '55000', '#dl-name-b': 'Mtoa B', '#dl-total-b': '57500', '#dl-name-c': 'Mtoa C', '#dl-total-c': '60000' },
    result: '#dl-results', metrics: ['#dl-tbody tr:nth-child(1) td:nth-child(2)', '#dl-tbody tr:nth-child(1) td:nth-child(3)', '#dl-tbody tr:nth-child(1) td:nth-child(4)', '#dl-tbody tr:nth-child(1) td:nth-child(5)', '#dl-tbody tr:nth-child(1) td:nth-child(6)', '#dl-tbody tr:nth-child(3) td:nth-child(6)'], dirty: '#dl-total-a', invalid: '#dl-amount'
  },
  {
    id: 'bnpl-calc', english: '/tools/bnpl-calc/', swahili: '/sw/zana/lipa-sasa-au-baadaye/', art: [800, 450],
    values: { '#bnpl-currency': 'KES', '#bnpl-price': '50000', '#bnpl-installments': '4', '#bnpl-rate': '8.5', '#bnpl-first-payment': 'now' },
    result: '#bnpl-results', metrics: ['#bnpl-extra-cost', '#bnpl-installment-amt', '#bnpl-total', '#bnpl-fee', '#bnpl-apr'], dirty: '#bnpl-rate', invalid: '#bnpl-price'
  }
];
...
for (const app of apps) {
  test(`${app.id} matches its English controller and clears stale or invalid results`, async ({ page }) => {
    ...
    await fill(page, app.values);
    await page.locator('[data-sw-fintech-credit-form]').press('Enter');
    await expect(page.locator(app.result)).toBeVisible();
...
function contrastProof() {
  ...
  function ratio(a, b) { const x = lum(a); const y = lum(b); return (Math.max(x, y) + .05) / (Math.min(x, y) + .05); }
  ...
  const badText = Array.from(document.querySelectorAll('body *')).filter((node) => {
    const style = getComputedStyle(node); const text = Array.from(node.childNodes).some((child) => child.nodeType === Node.TEXT_NODE && child.textContent.trim());
...
test('four credit routes pass physical mobile, zoom, themes, a11y, SEO, artwork, privacy and scoped AI proof', async ({ page, request }) => {
  const failed = []; const errors = [];
  page.on('requestfailed', (item) => failed.push(item.url()));
  page.on('pageerror', (error) => errors.push(error.message));
  for (const app of apps) {
    const external = []; const writes = []; const badResources = [];
    ...
    for (const theme of ['light', 'dark']) {
      ...
      const proof = await page.evaluate(contrastProof);
    ...
    for (const scheme of ['light', 'dark']) {
      ...
      const proof = await page.evaluate(contrastProof);
      expect(proof.badText, `${app.id}:system-${scheme} text`).toEqual([]);
      expect(proof.badBoundaries, `${app.id}:system-${scheme} boundaries`).toEqual([]);
    }
