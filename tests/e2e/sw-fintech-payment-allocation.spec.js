...
const apps = [
  {
    id: 'b2b-payment',
    english: '/tools/b2b-payment/',
    swahili: '/sw/zana/malipo-ya-biashara-kwa-biashara/',
    values: {
      '#b2b-currency': 'KES', '#b2b-amount': '120000', '#b2b-frequency': '5',
      '#b2b-name-1': 'Benki A', '#b2b-pct-1': '0.5', '#b2b-flat-1': '35', '#b2b-fx-1': '1.5', '#b2b-days-1': '4',
      '#b2b-name-2': 'Fintech B', '#b2b-pct-2': '0.8', '#b2b-flat-2': '0', '#b2b-fx-2': '0.5', '#b2b-days-2': '2',
      '#b2b-name-3': 'Benki C', '#b2b-pct-3': '1', '#b2b-flat-3': '0', '#b2b-fx-3': '0.8', '#b2b-days-3': '1'
    },
    result: '#b2b-results',
    metrics: ['#b2b-winner-name', '#b2b-cheapest-fee', '#b2b-swift-fee', '#b2b-monthly-saving', '#b2b-annual-saving', '#b2b-table-body tr:nth-child(1) td:nth-child(5)', '#b2b-table-body tr:nth-child(3) td:nth-child(6)'],
    dirty: '#b2b-pct-1',
    invalid: '#b2b-amount'
  },
  {
    id: 'bill-split',
    english: '/tools/bill-split/',
    swahili: '/sw/zana/kigawanya-bili-na-bakshishi/',
    values: { '#bs-currency': 'KES', '#bs-total': '101.01', '#bs-tip': '10', '#bs-method': 'custom', '#bs-p0-pct': '10', '#bs-p1-pct': '20', '#bs-p2-pct': '30', '#bs-p3-pct': '40' },
    result: '#bs-results',
    metrics: ['#bs-per-person', '#bs-subtotal', '#bs-tip-amt', '#bs-grand-total', '#bs-person-cards .person-card:nth-child(1) .person-amt', '#bs-person-cards .person-card:nth-child(4) .person-amt'],
    dirty: '#bs-tip',
    invalid: '#bs-total'
  }
];
...
for (const app of apps) {
  test(`${app.id} matches its English owner and clears stale or invalid results`, async ({ page }) => {
    ...
    if (app.id === 'bill-split') await expect(page.locator('#bs-people-count')).toHaveText('4 watu');
...
function contrastProof() {
  ...
  const badBoundaries = Array.from(document.querySelectorAll('input,select,button,.ai-handoff')).filter((node) => getComputedStyle(node).display !== 'none').map((node) => {
    const style = getComputedStyle(node);
    return { tag: node.tagName, id: node.id, value: ratio(style.borderColor, background(node.parentElement)) };
  }).filter((item) => item.value + .01 < 3);
  return { badText, badBoundaries };
}

test('payment allocation routes pass mobile, zoom, themes, a11y, metadata, privacy, network and scoped AI proof', async ({ page }) => {
  const failed = [];
  const errors = [];
