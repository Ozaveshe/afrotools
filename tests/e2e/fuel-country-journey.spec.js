...
for (const locale of ['en', 'fr']) {
  for (const width of [375, 1280]) {
    test(`${locale} fuel country leads to a usable local-price calculator at ${width}px`, async ({ page, baseURL }) => {
      ...
      if (locale === 'fr') {
        await page.locator('[name="fuel_type"]').selectOption('lpg');
        await expect(page.locator('[data-fuel-quantity-label]')).toHaveText('Kilogrammes par jour');
        await page.locator('[name="litres_per_day"]').fill('0');
        await expect(page.locator('[data-fuel-planner-output]')).toContainText('0 kg/jour');
        await page.locator('[name="days_per_month"]').fill('0');
        await expect(page.locator('[name="days_per_month"]')).toHaveAttribute('aria-invalid', 'true');
        await expect(page.locator('[data-fuel-planner-output]')).toContainText('entre 1 et 31');
      }
