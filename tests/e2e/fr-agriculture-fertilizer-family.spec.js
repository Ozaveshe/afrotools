...
test('fertilizer-calculator hub owns all 54 manifest country links', async ({ page }) => {
...
for (const row of COUNTRY_ROWS) {
  test(`${row.english.id} full route acceptance`, async ({ page, context }) => {
    ...
    await expect(page.getByText(/Aucune donnée en direct/)).toBeVisible();
    ...
    await page.getByRole('button', { name: 'Calculer les besoins NPK' }).focus();
    await expect(page.getByRole('button', { name: 'Calculer les besoins NPK' })).toBeFocused();
    ...
    expect(runtime.result.currency).toBe(runtime.currency);
    expect(runtime.report.resultat.devise).toBe(runtime.currency);

    const cases = [
      { name: 'Exporter en TXT', ext: '.txt', verify: (value) => expect(value.toString('utf8')).toContain('Confidentialité') },
      { name: 'Exporter en CSV', ext: '.csv', verify: (value) => expect(value.toString('utf8')).toContain('n_kg_ha') },
    ...
    for (const item of cases) {
      ...
      expect(download.suggestedFilename()).toMatch(new RegExp(`${code.toLowerCase()}.*${item.ext.replace('.', '\\.')}$`));
      item.verify(await buffer(download));
    }
    await page.getByRole('button', { name: 'Enregistrer dans ce navigateur' }).click();
    expect(JSON.parse(await page.evaluate((countryCode) => localStorage.getItem(`afrotools:fr-agriculture:fertilizer:${countryCode}`), code)).pays.code).toBe(code);
    await page.getByRole('button', { name: 'Copier' }).click();
    await expect.poll(() => page.evaluate(() => navigator.clipboard.readText())).toContain('besoins en engrais');
    ...
    await page.getByRole('button', { name: 'Calculer les besoins NPK' }).click();
