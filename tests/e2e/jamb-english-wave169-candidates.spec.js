...
for(const width of [320,390])test(`English wave169 answer disclosures at ${width}px`,async({page})=>{
 ...
 for(const year of [1983,1982,1985]){
  const records=batch.records.filter(r=>r.candidate.year===year);
  if(!records.length)continue;
  const html=renderYear('english',String(year),records.map(r=>r.candidate),ledger,[String(year)]).html;
  await page.route(`**/jamb/english/${year}/`,route=>route.fulfill({contentType:'text/html',body:html}));
  await page.goto(`/jamb/english/${year}/`);
