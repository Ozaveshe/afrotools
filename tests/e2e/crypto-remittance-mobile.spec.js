...
for(const[locale,route,menu]of[['en','/crypto/remittance/','Open menu'],['fr','/fr/crypto/remittance/','Ouvrir le menu'],['sw','/sw/zana/ulinganisho-nukuu-za-kutuma-fedha/','Fungua menyu']])for(const width of[320,390])for(const theme of['light','dark'])test(`${locale} ${width} ${theme} crypto-remittance controls and contrast`,async({page})=>{
