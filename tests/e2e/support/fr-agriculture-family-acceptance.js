...
function registerFamilyAcceptance(config) {
  const rows = manifest.rows.filter((row) => row.family === config.family);
  const countries = rows.filter((row) => row.country);
  const hub = rows.find((row) => !row.country);

  test(`${config.family} hub owns all ${config.expectedCountries} manifest country links`, async ({ page }) => {
    const failures = watchFailures(page);
    await page.setViewportSize({ width: 320, height: 900 });
    await page.goto(hub.french.route);
    await expect(page.locator('.country-list a')).toHaveCount(config.expectedCountries);
  ...
  for (const row of countries) {
    test(`${row.english.id} full route acceptance`, async ({ page, context }) => {
      ...
      await expect(page.locator('iframe')).toHaveCount(0);
      await expect(page.getByRole('heading', { level: 1 })).toContainText(row.country.frenchName);
      ...
      const calculate = page.getByRole('button', { name: config.calculateLabel });
      ...
      const runtime = await page.evaluate(() => ({
        ...
        pageConfig: window.__FR_AGRI_PAGE__,
      }));
      expect(runtime.report.pays.code).toBe(code);
      config.assertRuntime(runtime, code, expect);
      const cases = [
        { name: 'Exporter en TXT', ext: '.txt', verify: (value) => expect(value.toString('utf8')).toContain('Confidentialité') },
        { name: 'Exporter en CSV', ext: '.csv', verify: (value) => expect(value.toString('utf8')).toContain(config.csvHeader) },
      ...
      expect(JSON.parse(await page.evaluate(
        ({ key, countryCode }) => localStorage.getItem(`${key}:${countryCode}`),
        { key: config.storageKey, countryCode: code },
      )).pays.code).toBe(code);
      await page.getByRole('button', { name: 'Copier' }).click();
      await expect.poll(() => page.evaluate(() => navigator.clipboard.readText())).toContain(config.copyText);
      ...
      await page.getByLabel(config.invalidLabel).fill(config.invalidValue || '0');
      await page.getByRole('button', { name: config.calculateLabel }).click();
      await expect(page.getByRole('alert')).toContainText(config.invalidMessage);
      await expect(page.getByLabel(config.invalidLabel)).toBeFocused();
...
module.exports = { registerFamilyAcceptance };
