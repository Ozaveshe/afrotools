...
function observe(page) {
  const proof = { errors: [], writes: [], egress: [], bad: [] };
  page.on('pageerror', e => proof.errors.push(e.message)); page.on('console', m => { if (m.type() === 'error') proof.errors.push(m.text()); });
  page.on('request', request => { const url = new URL(request.url()); if (!['GET', 'HEAD'].includes(request.method())) proof.writes.push(`${request.method()} ${request.url()}`); if (!['127.0.0.1', 'localhost'].includes(url.hostname) && ['fetch', 'xhr', 'websocket'].includes(request.resourceType())) proof.egress.push(request.url()); expect((request.postData() || '')).not.toContain('SIRI YA BRAND'); });
...
test('English and Swahili produce byte-identical controlled SVG and PNG', async ({ page }) => {
  ...
  expect(proof.writes).toEqual([]); expect(proof.egress).toEqual([]); expect(proof.bad).toEqual([]); expect(proof.errors).toEqual([]);
});
