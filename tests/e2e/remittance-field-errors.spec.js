...
for(const[locale,route]of[['en','/tools/remittance-compare/'],['fr','/fr/tools/transfert-argent/'],['sw','/sw/zana/ulinganisho-uhamishaji-pesa/']])test(`${locale} native field errors identify and focus invalid quote`,async({page})=>{
 ...
 for(const letter of ['a','b'])for(const[key,value]of Object.entries({label:'Synthetic '+letter,sendCountry:'GB',receiveCountry:'SN',send:'USD',debit:'500',receive:'NGN',recipient:'790000',observed:'2026-01-01T11:00'}))await page.locator(`#rm-${letter}-${key}`).fill(value);
