...
test('French practice works at 320px with translated teaching and shared saved progress',async({page})=>{
 ...
 await expect(page.locator('#practice-session h2')).toHaveText('Question 1 sur 24');await expect(page.locator('legend')).toContainText('Une école achète 240 cahiers');await expect(page.locator('fieldset')).toHaveAttribute('lang','fr');
...
test('English exercise language stays explicit while explanations and controls are French',async({page})=>{
 ...
 await expect(page.locator('fieldset')).toHaveAttribute('lang','en');await expect(page.locator('.practice-passage')).toHaveAttribute('lang','en');await expect(page.locator('.practice-passage')).toContainText('a dependable plan would keep it open.');
