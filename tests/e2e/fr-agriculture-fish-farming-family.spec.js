...
test('fish-farming hub owns all 15 manifest country links', async ({ page }) => {
...
for (const row of COUNTRY_ROWS) {
  test(`${row.english.id} full route acceptance`, async ({ page, context }) => {
    ...
    const runtime = await page.evaluate(() => ({
      costs: window.__FR_AGRI_TEST__.data.COSTS[window.__FR_AGRI_PAGE__.countryCode],
    ...
    expect(runtime.report.resultat.devise).toBe(runtime.costs.currency);
    expect(runtime.result.harvestKg).toBeGreaterThan(0);

    const cases = [
      { name: 'Exporter en TXT', ext: '.txt', verify: (value) => expect(value.toString('utf8')).toContain('Confidentialité') },
      { name: 'Exporter en CSV', ext: '.csv', verify: (value) => expect(value.toString('utf8')).toContain('resultat_cycle') },
    ...
    expect(JSON.parse(await page.evaluate((countryCode) => (
      localStorage.getItem(`afrotools:fr-agriculture:fish-farming:${countryCode}`)
    ), code)).pays.code).toBe(code);
    await page.getByRole('button', { name: 'Copier' }).click();
    await expect.poll(() => page.evaluate(() => navigator.clipboard.readText())).toContain('rentabilité piscicole');
    ...
    await page.getByLabel('Surface ou volume').fill('0');
    await page.getByRole('button', { name: 'Calculer la rentabilité' }).click();
    await expect(page.getByRole('alert')).toContainText('au moins 1');
    await expect(page.getByLabel('Surface ou volume')).toBeFocused();
