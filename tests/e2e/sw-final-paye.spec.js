...
test('AI remains disabled without consent and sends only the disclosed result payload',async({page})=>{
  let payload=null,headers=null;await page.route('**/.netlify/functions/ai-advisor',async route=>{payload=route.request().postDataJSON();headers=route.request().headers();await route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({text:'Maelezo salama ya mfano.'})});});
  await page.goto('/sw/algeria/kikokotoo-kodi-mshahara/');const app=page.locator('[data-sw-paye-app=paye]');await app.locator('[name=gross]').fill('100000');await app.locator('form button[type=submit]').click();await expect(app.locator('[data-ai]')).toBeDisabled();await app.locator('[name=aiConsent]').check();await expect(app.locator('[data-ai]')).toBeEnabled();await app.locator('[data-ai]').click();await expect(app.locator('[data-ai-result]')).toContainText('Maelezo salama');expect(payload.aiConsent).toBe('accepted');expect(payload.context.period).toBe('annual');expect(headers['x-afrotools-ai-consent']).toBe('accepted');expect(Object.keys(payload.context).sort()).toEqual(['contribution','currency','gross','locale','net','period','sourceReviewed','tax','toolId'].sort());expect(JSON.stringify(payload)).not.toMatch(/email|name|file|document|password|otp/i);
});

test('remaining 12 legacy owners preserve SEO depth and reflow at 320px, 375px and effective 200%',async({page})=>{
