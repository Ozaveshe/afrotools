...
async function stubDashboardDependencies(page, options = {}) {
  ...
  await page.route('**/*', async (route) => {
    ...
    if (requestUrl.hostname === '127.0.0.1' && requestUrl.pathname === '/api/auth/session' && sessionUser) {
      await route.fulfill({
        status: 200,
...
test('dashboard valid cookie session reveals authenticated profile UI', async ({ page, context }) => {
  await context.addCookies([sessionCookie('valid')]);
  await page.addInitScript(() => {
    window.__dashboardAuthStateEvents = [];
    window.addEventListener('dashboard-auth-state-change', (event) => {
      window.__dashboardAuthStateEvents.push(event.detail);
    });
  });
  await stubDashboardDependencies(page);
  ...
  const events = await page.evaluate(() => window.__dashboardAuthStateEvents || []);
...
test('dashboard valid browser token bridges to authenticated session', async ({ page }) => {
  await stubDashboardDependencies(page, { sessionToken: 'valid-browser-token' });
...
test('dashboard Pro session shows Pro workspace entry and hides free upsell', async ({ page }) => {
  await stubDashboardDependencies(page, {
    sessionUser: {
      id: 'pro-user',
  ...
  await expect(page.locator('html')).toHaveAttribute('data-dashboard-auth-state', 'signedIn', { timeout: 15000 });
  await expect(page.locator('#dashboardSection')).toBeVisible();
  await expect(page.locator('#profilePlanBadge')).toHaveText('Pro');
  await expect(page.locator('#proUpsell')).toBeHidden();
  await expect(page.locator('#proWelcome')).toBeVisible();
  await expect(page.locator('#proWelcome a[href="/pro/workspace/"]')).toHaveText(/open pro workspace/i);
  await expect(page.locator('#stat-plan')).toHaveText('Pro');
});

test('dashboard stale cached user is cleared when server session is signed out', async ({ page }) => {
  await page.addInitScript(() => {
    localStorage.setItem('afro_auth_v2', JSON.stringify({
      id: 'stale-user',
      email: 'stale@afrotools.test',
      name: 'Stale User',
    }));
    localStorage.setItem('afro_session_v3', 'stale-token');
    localStorage.setItem('afro_profile_cache', JSON.stringify({ user: { id: 'stale-user' } }));
  });
  await stubDashboardDependencies(page);
  ...
  }))).toEqual({ user: null, token: null, profile: null });
});
