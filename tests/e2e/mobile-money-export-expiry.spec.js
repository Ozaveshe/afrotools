...
for(const[locale,route]of Object.entries(routes))test(`${locale} country equivalence preserves user values and excludes other markets`,async({page})=>{
 ...
 await page.locator('#mm-b-market').fill('Mali');await page.locator('#mm-copy').click();await expect(page.locator('#mm-primary-value')).not.toHaveText('10 XOF');if(locale==='fr')await expect(page.locator('#mm-error')).toContainText('même pays');else await expect(page.locator('#mm-primary-value')).toContainText(locale==='sw'?'Hakuna':'No eligible');
