...
for(const [locale,route] of [['en','/tools/leave-calculator/'],['sw','/sw/zana/kikokotoo-likizo/']])test(`${locale}: parental dates and parsed ICS retain exact duration and invalidate stale export`,async({page},info)=>{
 ...
 const secondPromise=page.waitForEvent('download');await page.locator('#pp-result .lc-ical-btn').click();const second=await secondPromise;expect(events(fs.readFileSync(await second.path(),'utf8'))[0].UID).not.toBe(parsed[0].UID);
 ...
 if(locale==='en')expect(await page.evaluate(()=>lcBuildSummary())).toBe('');
