...
const batch=require('../../ops/jamb/review-candidates/english/english-2020-003.json');

for(const width of [320,390])test(`English comprehension candidate retains visible context at ${width}px`,async({page})=>{
 ...
 const html=renderYear('english','2020',batch.records.map(r=>r.candidate),ledger,['2020']).html;
 const errors=[];page.on('pageerror',e=>errors.push(e.message));
 await page.context().addInitScript(()=>localStorage.setItem('afrotools_cookie_consent','declined'));
 await page.setViewportSize({width,height:900});
 await page.route('**/jamb/english/2020/',r=>r.fulfill({contentType:'text/html',body:html}));
 await page.goto('/jamb/english/2020/');
 await expect(page.locator('[data-reviewed-question]')).toHaveCount(22);
 await expect(page.locator('[data-reviewed-question] blockquote')).toHaveCount(8);
