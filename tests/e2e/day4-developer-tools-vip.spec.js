...
function developerRoutes() {
  ...
  if (apps.length !== 32 || new Set(apps).size !== 32) {
    throw new Error(`Expected 32 unique English Developer Tools apps, found ${apps.length}`);
  }

  return ['/developer-tools/', ...apps];
}

const ROUTES = developerRoutes();
const SCENARIOS = [
...
test.describe('Day 4 Developer Tools VIP regression', () => {
  ...
  test('hub reconciles all 32 canonical app routes and segment filters', async ({ page }) => {
    await page.setViewportSize({ width: 375, height: 844 });
    await page.goto('/developer-tools/', { waitUntil: 'domcontentloaded' });

    await expect(page.locator('#s-live')).toHaveText('32');
    await expect(page.locator('#tool-sections .tool-card')).toHaveCount(32);

    await page.getByRole('button', { name: 'Flagship' }).click();
    await expect(page.locator('#tool-sections .tool-card')).toHaveCount(8);
