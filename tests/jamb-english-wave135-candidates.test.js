...
const batch=require('../ops/jamb/review-candidates/english/english-1986-008.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1986-008.cjs');
...
test('wave135 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,3);
});
test('wave135 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,3);
 assert.equal(verify(after,true).held,17);
