...
test('all 94 identities, assessment language and backup contracts survive localization', () => {
  ...
  for (const locale of ['fr', 'sw']) {
    ...
    assert.equal(quick.questions.length, 52); assert.equal(written.items.length, 42);
...
test('worked numerical results and exam references retain the English meanings', () => {
  for (const locale of ['fr','sw']) {
    ...
    assert.equal(items.filter(q => q.exam === 'WAEC').length, 25);
