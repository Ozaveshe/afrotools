...
test('source generator is current and native product surfaces retain fields, results and translated schedules',()=>{
  ...
  for(const app of manifest.apps){const html=read(app.swFile);assert.match(html,new RegExp(`data-shell-tool="${app.id}"`));assert.match(html,new RegExp(`<script src="${app.enginePublic.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}"`));assert.doesNotMatch(html,/<iframe\b|fetch\(|XMLHttpRequest/i);assert.match(html,/data-shell-export="copy" disabled/);assert.match(html,/data-shell-export="json" disabled/);assert.match(html,/data-shell-export="txt" disabled/);}
