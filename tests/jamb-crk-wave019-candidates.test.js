...
const batch=require('../ops/jamb/review-candidates/crk/crk-wave019.json');
const {verify}=require('../ops/jamb/review-candidates/crk/check-crk-wave019.cjs');
...
test('crk wave019 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,36);
});
test('crk wave019 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,36);
