...
const batch=require('../ops/jamb/review-candidates/english/english-2015-001.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-2015-001.cjs');
...
test('wave6 intake preserves passages and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,11);
 assert.equal(verify(after,true).held,29);
 ...
 assert.throws(()=>verify(after.map(q=>q.id===id?{...q,answer:'A'}:q),true),/held pool changed/);
});
