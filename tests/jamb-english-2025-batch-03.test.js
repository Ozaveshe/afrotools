...
const { prepareBatch } = require('../ops/nigeria-exams/import-jamb-english-2025-batch-03.cjs');
const { verify } = require('../ops/jamb/verification/check-english-2025-003.cjs');
...
const receipt = read('ops/jamb/verification/english-2025-publishable-003.json');
...
test('all 25 collection positions are classified, with no new held item imported', () => {
  ...
  assert.deepEqual(manifest.records.map(row => row.source_item),
    [222, 224, 228, 229, 230, 232, 233, 235, 236, 239, 240, 241, 243, 244, 245]);
  ...
  assert.equal(replay.batch.records.length, 15);
...
test('independent answer checker agrees with source-linked review and catches an altered answer', () => {
  ...
  const question = changed.questions.find(row => row.id === 'english-2025-myschool-74850');
  question.answer = 'A';
...
test('the 2025 English page presents the new reviewed items as a labelled collection', () => {
  ...
  assert.ok(page.approvedIds.length >= 29);
