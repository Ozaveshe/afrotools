...
const batch=require('../ops/jamb/review-candidates/english/english-1980-001.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1980-001.cjs');
...
test('wave176 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,9);
});
test('wave176 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,9);
 assert.equal(verify(after,true).held,11);
