...
for (const [id, slug, englishRoute] of apps) {
  ...
  const artwork = ['webp', 'svg'].some((extension) => fs.existsSync(path.join(root, `assets/img/tools/${id}.${extension}`)));
  assert.ok(artwork, `${slug}: canonical artwork`);
  assert.ok(fs.existsSync(path.join(root, `data/ai/tool-context/${id}.json`)), `${slug}: AI tool context`);
  assert.ok(
