...
const server = http.createServer(function(req, res) {
  if (req.url.split('?')[0] === '/api/auth/session') {
    ...
    if (mode === 'valid' || bearer === 'valid-browser-token') {
      sendJson(res, 200, {
        authenticated: true,
        bridged: bearer === 'valid-browser-token',
        user: {
          id: 'test-user-1',
      ...
      }, bearer === 'valid-browser-token' ? { 'Set-Cookie': 'afro_session=valid-browser-token; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=3600' } : null);
      return;
    }

    if (mode === 'expired' || bearer === 'expired-browser-token') {
      sendJson(res, 401, {
        authenticated: false,
