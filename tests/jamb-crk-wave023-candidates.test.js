...
const batch=require('../ops/jamb/review-candidates/crk/crk-wave023.json');
const {verify}=require('../ops/jamb/review-candidates/crk/check-crk-wave023.cjs');
...
test('crk wave023 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,28);
});
test('crk wave023 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,28);
