...
const batch=require('../ops/jamb/review-candidates/english/english-1984-004.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1984-004.cjs');
...
test('wave149 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,19);
});
test('wave149 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,19);
 assert.equal(verify(after,true).held,1);
