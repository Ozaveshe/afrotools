...
const batch=require('../ops/jamb/review-candidates/crk/crk-wave010.json');
const {verify}=require('../ops/jamb/review-candidates/crk/check-crk-wave010.cjs');
...
test('crk wave010 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,23);
});
test('crk wave010 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,23);
