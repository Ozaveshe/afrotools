...
const batch=require('../ops/jamb/review-candidates/english/english-1984-003.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1984-003.cjs');
...
test('wave148 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,16);
});
test('wave148 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,16);
 assert.equal(verify(after,true).held,4);
