...
(async () => {
  const oldCommit = process.env.COMMIT_REF;
  try {
    ...
    process.env.COMMIT_REF = 'not-a-commit';
    assert.equal((await captureRun(naturalEvent)).fetch_meta.release_commit, null);
  } finally {
    if (oldCommit === undefined) delete process.env.COMMIT_REF;
    else process.env.COMMIT_REF = oldCommit;
  }
