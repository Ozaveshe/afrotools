...
const batch=require('../ops/jamb/review-candidates/english/english-1985-001.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1985-001.cjs');
...
test('wave140 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,17);
});
test('wave140 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,17);
 assert.equal(verify(after,true).held,3);
