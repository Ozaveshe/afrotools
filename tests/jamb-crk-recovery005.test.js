...
const D='../ops/jamb/review-candidates/crk/',batch=require(D+'crk-recovery005.json'),recovery=require(D+'check-recovery-integrity.cjs');
...
test('CRK recovery rejects rehashed wrong answers, years, source evidence and omitted selections',()=>{
 for(const change of [b=>b.records[0].candidate.answer=b.records[0].candidate.answer==='A'?'B':'A',b=>b.records[0].candidate.year=2099,b=>b.records[0].source_pdf_page=59,b=>b.held_records.pop()]){
  ...
  assert.throws(()=>recovery.load({files:{'crk-recovery005.json':raw}}));
  manifest.batches[4].sha256=crypto.createHash('sha256').update(raw).digest('hex');assert.throws(()=>recovery.load({manifest,files:{'crk-recovery005.json':raw}}),/independent recovery pins/);
 }
});
