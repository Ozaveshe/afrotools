...
const batch=require('../ops/jamb/review-candidates/english/english-1981-001.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1981-001.cjs');
...
test('wave170 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,13);
});
test('wave170 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,13);
 assert.equal(verify(after,true).held,7);
