...
const { prepareBatch } = require('../ops/nigeria-exams/import-jamb-english-2025-batch-02.cjs');
const { verify } = require('../ops/jamb/verification/check-english-2025-002.cjs');
...
const receipt = read('ops/jamb/verification/english-2025-publishable-002.json');

test('all 25 collection positions are classified, with no new held item imported', () => {
  assert.deepEqual([...manifest.records, ...manifest.held_source_items].map(row => row.source_item).sort((a, b) => a - b),
    Array.from({ length: 25 }, (_, index) => index + 196));
  assert.deepEqual(manifest.records.map(row => row.source_item),
    [196, 200, 205, 209, 210, 211, 212, 216, 217, 218, 219]);
  ...
  for (const row of manifest.held_source_items) {
    if ([198, 199].includes(row.source_item)) continue;
...
test('independent answer checker agrees with source-linked review and catches an altered answer', () => {
  ...
  const question = changed.questions.find(row => row.id === 'english-2025-myschool-74823');
  question.answer = 'B';
...
test('the 2025 English page presents the new reviewed items as a labelled collection', () => {
  ...
  assert.ok(page.approvedIds.length >= 14);
