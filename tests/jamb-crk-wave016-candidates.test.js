...
const batch=require('../ops/jamb/review-candidates/crk/crk-wave016.json');
const {verify}=require('../ops/jamb/review-candidates/crk/check-crk-wave016.cjs');
...
test('crk wave016 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,32);
});
test('crk wave016 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,32);
