...
assert.deepStrictEqual(
  ...
  'Insurance must remain a verified native 16-app French hub'
);
...
assert.match(directoryHtml, /const params = new URLSearchParams\(window\.location\.search\);[\s\S]*params\.get\('category'\)/, 'filtered-directory routes must initialize the requested category');
assert.match(directoryHtml, /params\.get\('q'\)/, 'homepage search queries must initialize the French directory search');
...
const acceptedParityRows = parityInventory.rows.filter((row) => row.accepted);
assert.strictEqual(
  parityInventory.totals.accepted,
  acceptedParityRows.length,
  'discovery coverage must not create product-parity acceptance credit outside the acceptance ledger'
);
acceptedParityRows.forEach((row) => {
  assert.strictEqual(row.acceptanceEvidence?.status, 'accepted', `${row.englishRoute} needs explicit acceptance evidence`);
});

console.log(`French discovery foundation passed: 32 categories, ${frenchPublished.length} published French rows, ${acceptedParityRows.length} accepted apps.`);
