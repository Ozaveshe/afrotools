...
const batch=require('../ops/jamb/review-candidates/english/english-2001-003.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-2001-003.cjs');
...
test('wave41 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,20);
 assert.equal(verify(after,true).held,0);
 ...
 assert.throws(()=>verify(after.map(q=>q.id===id?{...q,question:q.question+' changed'}:q),true),/integrated candidate mismatch/);
});
