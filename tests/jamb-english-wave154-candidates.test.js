...
const batch=require('../ops/jamb/review-candidates/english/english-1982-003.json');
const {verify}=require('../ops/jamb/review-candidates/english/check-english-1982-003.cjs');
...
test('wave154 rejects self-consistently rehashed answer and numbering mutations',()=>{
 ...
 assert.equal(verify(after,true).candidates,6);
});
test('wave154 intake preserves candidates and held records at either integration stage',()=>{
 assert.equal(verify(before).candidates,6);
 assert.equal(verify(after,true).held,14);
