...
const assigned = inventory.rows.filter(row => row.categoryKey === "agriculture" && appIds.has(row.englishId));
...
for (const app of apps) {
  ...
  assert.strictEqual(normalizeReleaseOwnedHtml(html, { stripReleaseMetadata: true }), normalizeReleaseOwnedHtml(render(app), { stripReleaseMetadata: true }), `${app.id}: generated owned content current`);
