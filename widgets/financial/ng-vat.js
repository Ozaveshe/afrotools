(function(){
  ...
  window.AfroWidgets.ng_vat = function(container, opts) {
    opts = opts || {};
    ...
    var COUNTRY = 'Nigeria';
    var CURRENCY = '\u20A6';
    var INFO = 'Standard rate 7.5%. Basic food items exempted from Jan 2026. Authority: FIRS. Filing: Monthly. Threshold: \u20A625M/yr.';

    var uid = 'aw_ngvat_' + Math.random().toString(36).substr(2,6);
    ...
    document.getElementById(uid+'_addBtn').onclick = function() {
      mode = 'add'; this.style.background = accent; this.style.color = '#fff'; this.style.borderColor = accent;
      var o = document.getElementById(uid+'_extBtn'); o.style.background = 'transparent'; o.style.color = fg; o.style.borderColor = border;
      document.getElementById(uid+'_lbl').textContent = 'Amount (excl. VAT)';
    };
    document.getElementById(uid+'_extBtn').onclick = function() {
      mode = 'extract'; this.style.background = accent; this.style.color = '#fff'; this.style.borderColor = accent;
      var o = document.getElementById(uid+'_addBtn'); o.style.background = 'transparent'; o.style.color = fg; o.style.borderColor = border;
      document.getElementById(uid+'_lbl').textContent = 'Amount (incl. VAT)';
    };

    function calculate() {
      var amount = parseAmt(document.getElementById(uid+'_amt').value);
      if (amount <= 0) return;
      var rate = RATE / 100; var net, vat, total;
      ...
      document.getElementById(uid+'_resLbl').textContent = mode === 'add' ? 'Total (VAT inclusive)' : 'Net Amount (excl. VAT)';
      document.getElementById(uid+'_resMain').textContent = fmt(mode === 'add' ? total : net);
      document.getElementById(uid+'_net').textContent = fmt(net);
      document.getElementById(uid+'_vat').textContent = fmt(vat);
      document.getElementById(uid+'_total').textContent = fmt(total);
    }
