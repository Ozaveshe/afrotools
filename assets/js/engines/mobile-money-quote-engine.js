(function(root,factory){
  ...
  function quoteTariff(catalog,input){
    ...
    var allChargesIncluded=!(provider.taxExcludedActions||[]).includes(action);
    return {available:true,reason:null,providerId:provider.id,country:provider.country,countryCode:provider.countryCode,provider:provider.provider,action:action,amount:amount,currency:provider.currency,fee:band.fee,recipientReceives:amount,amountPlusPublishedFee:amount+band.fee,totalIncludesAllCharges:allChargesIncluded,totalDebited:allChargesIncluded?amount+band.fee:null,sourcePeriod:provider.sourcePeriod||null,band:{min:band.min,max:band.max,label:band.min+"-"+band.max+" "+provider.currency},rule:band.rule||("Published "+action+" fee for this amount band"),feeComponents:band.feeComponents?Object.assign({},band.feeComponents):null,effectiveDate:provider.effectiveDate,lastVerified:provider.lastVerified,source:Object.assign({},provider.source),confidence:provider.confidence,caveats:Array.isArray(provider.caveats)?provider.caveats.slice():[],methodology:catalog.methodology};
  }
