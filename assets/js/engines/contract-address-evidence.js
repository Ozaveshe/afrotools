(function(root,factory){'use strict';var api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.AfroTools=root.AfroTools||{};root.AfroTools.ContractAddressEvidence=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){'use strict';
  ...
  function text(result,locale){
    var lang=locale===true?'fr':locale==='sw'?'sw':'en';
    var messages={en:{title:'Local contract address evidence record',chain:'Chain: ',address:'Address: ',none:'Result: no exact reviewed record in this small registry. This does not prove safety.',unavailable:'Result: registry unavailable. No record lookup was completed.',found:'Result: an exact reviewed record exists. This is not a verdict.',record:['Title: ','Publisher: ','Reviewed: ','Evidence status: ','Confidence: ','Source: '],limits:'Limits: EVM syntax only; no existence, checksum, token, source, permission, holder, liquidity, sellability, honeypot or safety proof.'},fr:{title:'Relevé local de preuve d’adresse de contrat',chain:'Chaîne : ',address:'Adresse : ',none:'Résultat : aucun enregistrement exact révisé dans ce petit registre. Ceci ne prouve pas la sécurité.',unavailable:'Résultat : registre indisponible. Aucune recherche d’enregistrement n’a été effectuée.',found:'Résultat : un enregistrement révisé exact existe. Ce n’est pas un verdict.',record:['Titre : ','Éditeur : ','Révisé le : ','Statut de preuve : ','Confiance : ','Source : '],limits:'Limites : syntaxe EVM seulement; aucune preuve d’existence, checksum, jeton, code source, permissions, détenteurs, liquidité, vente, honeypot ou sécurité.'},sw:{title:'Rekodi ya ndani ya ushahidi wa anwani ya mkataba',chain:'Mtandao: ',address:'Anwani: ',none:'Matokeo: hakuna rekodi sahihi iliyokaguliwa katika sajili hii ndogo. Hii haithibitishi usalama.',unavailable:'Matokeo: sajili haipatikani. Hakuna utafutaji wa rekodi uliokamilika.',found:'Matokeo: rekodi sahihi iliyokaguliwa ipo. Huu si uamuzi wa usalama.',record:['Kichwa: ','Mchapishaji: ','Ilikaguliwa: ','Hali ya ushahidi: ','Kiwango cha uhakika: ','Chanzo: '],limits:'Mipaka: umbo la EVM pekee; hakuna uthibitisho wa kuwepo, checksum, tokeni, source code, ruhusa, wamiliki, ukwasi, kuuzika, honeypot au usalama.'}},m=messages[lang];
    var lines=[m.title,m.chain+result.chainLabel,m.address+result.address];
    if(result.status==='no-reviewed-record')lines.push(m.none);
    if(result.status==='registry-unavailable')lines.push(m.unavailable);
    if(result.status==='reviewed-record'){lines.push(m.found);lines.push(m.record[0]+result.record.title);lines.push(m.record[1]+result.record.sourcePublisher);lines.push(m.record[2]+result.record.reviewedAt);lines.push(m.record[3]+result.record.evidenceStatus);lines.push(m.record[4]+result.record.confidence);lines.push(m.record[5]+result.record.sourceUrl);}
    lines.push(m.limits);
    return lines.join('\n');
  }
