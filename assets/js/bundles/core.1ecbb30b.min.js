...
!function(t){"use strict";const e={number:(t,e=0)=>null==t||isNaN(t)?"0":Number(t).toLocaleString("en",{minimumFractionDigits:e,maximumFractionDigits:e}),percent:(t,e=1,o=!0)=>null==t||isNaN(t)?"0%":(o?100*t:t).toFixed(e)+"%",compact(t,e=1){if(null==t||isNaN(t))return"0";const o=Math.abs(t),n=t<0?"-":"";return o>=1e12?n+(o/1e12).toFixed(e)+"T":o>=1e9?n+(o/1e9).toFixed(e)+"B":o>=1e6?n+(o/1e6).toFixed(e)+"M":o>=1e3?n+(o/1e3).toFixed(e)+"K":n+Math.round(o).toString()},date(t,e="en-GB",o){const n=t instanceof Date?t:new Date(t);return isNaN(n.getTime())?"":n.toLocaleDateString(e,o||{day:"numeric",month:"long",year:"numeric"})},dateShort(t){return this.date(t,"en-GB",{day:"numeric",month:"short",year:"numeric"})},dateISO(t){const e=t instanceof Date?t:new Date(t);return isNaN(e.getTime())?"":e.toISOString().slice(0,10)},timeAgo(t){const e=t instanceof Date?t:new Date(t),o=Date.now()-e.getTime(),n=Math.abs(o),r=o<0,a=[{label:"year",ms:315576e5},{label:"month",ms:2630016000.0000005},{label:"week",ms:6048e5},{label:"day",ms:864e5},{label:"hour",ms:36e5},{label:"minute",ms:6e4}];for(const t of a){const e=Math.floor(n/t.ms);if(e>=1){const o=1===e?"":"s";return r?`in ${e} ${t.label}${o}`:`${e} ${t.label}${o} ago`}}return"just now"},ordinal(t){const e=["th","st","nd","rd"],o=t%100;return t+(e[(o-20)%10]||e[o]||e[0])},duration(t,e=!1){const o=Math.floor(t/3600),n=Math.floor(t%3600/60),r=Math.floor(t%60),a=[];return o>0&&a.push(o+"h"),n>0&&a.push(n+"m"),e&&r>0&&a.push(r+"s"),a.join(" ")||"0m"},fileSize(t){if(0===t)return"0 B";const e=Math.floor(Math.log(t)/Math.log(1024));return(t/Math.pow(1024,e)).toFixed(0===e?0:1)+" "+["B","KB","MB","GB","TB"][e]},parseNum(t){if("number"==typeof t)return t;if(!t)return 0;const e=String(t).replace(/[^\d.\-]/g,"");return parseFloat(e)||0},clamp:(t,e,o)=>Math.min(Math.max(t,e),o),round(t,e=0){const o=Math.pow(10,e);return Math.round((t+Number.EPSILON)*o)/o}};t.AfroTools=t.AfroTools||{},t.AfroTools.fmt=Object.assign(e,t.AfroTools.fmt||{})}(window);;
...
!function(){"use strict";var e="afrotools_cookie_consent",o=null;try{o=localStorage.getItem(e)}catch(e){}if("accepted"!==o)if("declined"!==o){var a={en:{label:"Cookie consent",message:"AfroTools uses cookies for analytics and saving your preferences.",accept:"Accept",privacy:"Privacy Policy",dismiss:"Dismiss"},fr:{label:"Consentement aux cookies",message:"AfroTools utilise des cookies pour l'analyse et la sauvegarde de vos préférences.",accept:"Accepter",privacy:"Politique de confidentialite",dismiss:"Fermer"},sw:{label:"Idhini ya kuki",message:"AfroTools inatumia kuki kwa uchambuzi na kuhifadhi mapendeleo yako.",accept:"Kubali",privacy:"Sera ya Faragha",dismiss:"Funga"},ha:{label:"Izinin kukis",message:"AfroTools yana amfani da kukis domin nazarin yadda ake amfani da shafin da kuma adana zabinka.",accept:"Yarda",privacy:"Manufar sirri",dismiss:"Rufe"}},i=a[(document.documentElement.lang||"en").substring(0,2)]||a.en,t=document.createElement("style");t.id="afro-cookie-consent-style",t.textContent=["#afro-cookie-consent{position:fixed;left:50%;right:auto;bottom:calc(14px + env(safe-area-inset-bottom));z-index:99999;width:min(680px,calc(100vw - 24px));transform:translate(-50%,calc(100% + 24px));background:#102033;border:1px solid rgba(203,213,225,.24);border-radius:10px;padding:10px 12px;font-family:'DM Sans',system-ui,sans-serif;box-shadow:0 14px 34px rgba(2,8,23,.22);transition:transform .24s ease}","#afro-cookie-consent .afro-cc-inner{display:flex;align-items:center;gap:12px}","#afro-cookie-consent p{flex:1;margin:0;min-width:180px;color:#e5eef8;font-size:13px;line-height:1.45}","#afro-cookie-consent .afro-cc-actions{display:flex;align-items:center;gap:8px;flex-shrink:0}","#afro-cookie-consent button,#afro-cookie-consent a{font-family:inherit}","#afro-cc-accept{align-items:center;background:#0062cc;border:1px solid #0062cc;border-radius:8px;color:#fff;cursor:pointer;display:inline-flex;font-size:13px;font-weight:800;justify-content:center;min-height:44px;padding:0 16px}","#afro-cc-learn{align-items:center;border:1px solid transparent;border-radius:8px;color:#bfdbfe;display:flex;font-size:13px;font-weight:800;justify-content:center;min-height:44px;padding:0 10px;text-decoration:none}","#afro-cc-close{align-items:center;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.14);border-radius:8px;color:#e5eef8;cursor:pointer;display:inline-flex;font-size:20px;height:44px;justify-content:center;line-height:1;padding:0;width:44px}","#afro-cc-accept:hover{background:#004fa8;border-color:#004fa8}","#afro-cc-learn:hover,#afro-cc-close:hover{background:rgba(255,255,255,.1);color:#fff}","#afro-cc-accept:focus-visible,#afro-cc-learn:focus-visible,#afro-cc-close:focus-visible{outline:3px solid rgba(147,197,253,.5);outline-offset:2px}","@media(max-width:560px){#afro-cookie-consent{bottom:calc(8px + env(safe-area-inset-bottom));width:calc(100vw - 20px);padding:10px;border-radius:10px}#afro-cookie-consent .afro-cc-inner{display:grid;gap:9px}#afro-cookie-consent p{font-size:12.6px;line-height:1.42;min-width:0}#afro-cookie-consent .afro-cc-actions{display:grid;grid-template-columns:1fr 1fr 44px;gap:7px;min-width:0;width:100%}#afro-cookie-consent #afro-cc-accept,#afro-cookie-consent #afro-cc-learn{font-size:12.4px;min-width:0;padding:0 8px;width:auto!important}#afro-cookie-consent #afro-cc-close{height:44px;width:44px!important}}","@media(prefers-reduced-motion:reduce){#afro-cookie-consent{transition:none}}"].join(""),document.head.appendChild(t);var n=document.createElement("div");n.id="afro-cookie-consent",n.setAttribute("role","dialog"),n.setAttribute("aria-label",i.label),n.innerHTML='<div class="afro-cc-inner"><p>'+i.message+'</p><div class="afro-cc-actions"><button id="afro-cc-accept" type="button">'+i.accept+'</button><a href="/privacy/" id="afro-cc-learn">'+i.privacy+'</a><button id="afro-cc-close" type="button" aria-label="'+i.dismiss+'">&times;</button></div></div>',document.body.appendChild(n),requestAnimationFrame(function(){requestAnimationFrame(function(){n.style.transform="translate(-50%,0)"})}),document.getElementById("afro-cc-accept").addEventListener("click",function(){c("accepted"),r()}),document.getElementById("afro-cc-close").addEventListener("click",function(){c("declined"),s(),r()})}else s();function c(o){try{localStorage.setItem(e,o)}catch(e){}try{window.dispatchEvent(new CustomEvent("afrotools:cookie-consent",{detail:{status:o}}))}catch(e){}}function r(){n.style.transform="translate(-50%,calc(100% + 24px))",setTimeout(function(){n.parentNode&&n.parentNode.removeChild(n),t.parentNode&&t.parentNode.removeChild(t)},300)}function s(){window["ga-disable-G-D859CGF391"]=!0}}();;
