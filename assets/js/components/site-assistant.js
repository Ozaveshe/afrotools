/**
 * AFROTOOLS SITE ASSISTANT v3 — Floating Ask AfroTools AI
...
(function () {
  ...
  ═══════════════════════════════════════════════════ */
  const BASE_SYSTEM_PROMPT = `You are the AfroTools AI workflow assistant for AfroTools.com, an Africa-focused tools platform. You help users:
- Find and use AfroTools calculators and workflows (tax, salary, PDF, health, image, education, developer tools, and more)
...
4. If asked about a specific country's tax/finance rules, give planning guidance and tell users to verify high-stakes decisions with official sources or a qualified professional.
5. Always guide users toward the right practical tool on AfroTools.
...
7. When listing multiple tools, put each on its own line.`;
  ...
  const LIGHT_VARS = {
    fabBg: '#ffffff',
    fabShadow: '0 2px 16px rgba(0,98,204,0.18), 0 4px 24px rgba(0,0,0,0.08)',
    fabHoverShadow: '0 4px 24px rgba(0,98,204,0.28), 0 8px 32px rgba(0,0,0,0.10)',
    panelBg: '#ffffff',
    ...
    sugBorder: '#E2E8F0',
    sugColor: '#0062CC',
    sugHoverBg: 'rgba(0,98,204,0.06)',
    copyBg: 'rgba(0,0,0,0.06)',
    aiBadgeBg: 'rgba(0,98,204,0.08)',
    aiBadgeColor: '#0062CC',
    aiBadgeBorder: 'rgba(0,98,204,0.15)',
    linkColor: '#0062CC',
  };

  const DARK_VARS = {
    fabBg: '#131D2E',
    fabShadow: '0 2px 16px rgba(0,98,204,0.22), 0 4px 24px rgba(0,0,0,0.3)',
    fabHoverShadow: '0 4px 24px rgba(0,98,204,0.35), 0 8px 32px rgba(0,0,0,0.35)',
    panelBg: '#131D2E',
    ...
    sugBorder: '#1E2D40',
    sugColor: '#60b5ff',
    sugHoverBg: 'rgba(0,98,204,0.12)',
    copyBg: 'rgba(255,255,255,0.08)',
    aiBadgeBg: 'rgba(0,98,204,0.12)',
    aiBadgeColor: '#60b5ff',
  ...
  function parseMd(text) {
    ...
    s = s.replace(/\[([^\]]+)\]\(((?:https?:\/\/[^\s)]+|\/[^\s)]+))\)/g,
      '<a href="$2" target="_blank" rel="noopener" style="color:var(--link-color,#0062CC);text-decoration:underline;font-weight:600">$1</a>');
    // Bold: **text**
  ...
  class AfroSiteAssistant extends HTMLElement {
        ...
        <style>
          *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          .fab {
            ...
            box-shadow: var(--fab-shadow, 0 2px 16px rgba(0,98,204,0.18), 0 4px 24px rgba(0,0,0,0.08));
          ...
          .badge {
            ...
            box-shadow: 0 0 6px rgba(0,98,204,0.4);
          }
          ...
          .p-badge {
            font-size: 0.52rem;
            font-weight: 700;
            color: var(--ai-badge-color, #0062CC);
            background: var(--ai-badge-bg, rgba(0,98,204,0.08));
            padding: 2px 6px;
            border-radius: 6px;
            letter-spacing: .05em;
            text-transform: uppercase;
            border: 1px solid var(--ai-badge-border, rgba(0,98,204,0.15));
          ...
          .qn-item:hover { background: var(--qn-hover-bg, rgba(0,98,204,0.08)); color: #0062CC; }
          ...
          .msg-ai a { color: var(--link-color, #0062CC); }
          .msg-sys {
          ...
          .tool-card:hover { background: var(--qn-hover-bg, rgba(0,98,204,0.08)); transform: translateY(-1px); }
          ...
          .tool-card-arrow { color: #0062CC; font-weight: 700; font-size: 0.9rem; }
          ...
          .sug-btn:hover { background: var(--sug-hover-bg, rgba(0,98,204,0.06)); border-color: #0062CC; }
          ...
          .chat-input:focus { border-color: #0062CC; box-shadow: 0 0 0 3px rgba(0,98,204,0.1); }
          .send-btn {
            background: #0062CC;
            color: #fff;
            border: none;
    ...
    _offlineReply(q) {
      ...
      if (q.includes('currency') || q.includes('exchange'))
        return 'The [Currency Converter](https://afrotools.com/tools/currency-converter) supports 160+ currencies with latest available rates where the feed is available.';
