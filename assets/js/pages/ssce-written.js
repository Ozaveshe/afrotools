(function(){
  ...
  function render(skipCapture){
    ...
    var q=current,entry=draft.entries[q.id],heading=el('h3',q.title);area.append(heading,el('p',t(q.origin)+(q.year?' · '+q.year+t(' · Paper ')+q.paper+t(' · Question ')+q.number:''),'practice-meta'));
  ...
  function tasks(){capture();select.replaceChildren();bank.items.filter(function(q){return q.collection===collection.value;}).forEach(function(q){select.add(new Option((q.number?t('Question ')+q.number+' · ':'')+q.title,q.id));});render();}
