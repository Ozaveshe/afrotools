(function(){
  ...
  function button(text,fn){var b=el('button',text,'btn btn-secondary');b.type='button';b.addEventListener('click',function(){try{fn();}catch(e){message(e.message);}});return b;}
  ...
  function diagram(kind){
    ...
    svg.setAttribute('aria-label',description);add('title',{},description);
    ...
    add('text',{x:160,y:297,'text-anchor':'middle'},'Not to scale');return svg;
  }
  function render(skipCapture){
    ...
    function count(){var text=answer.value.trim();words.textContent=(text?text.split(/\s+/).length:0)+' words · saved only when you choose Save response';}
    answer.addEventListener('input',function(){dirty.add(q.id);count();});count();area.append(label,answer,words);
    var details=el('details',undefined,'written-explanation'),steps=el('ol');details.append(el('summary',q.subject==='Mathematics'?'Show worked solution':'Show writing guide'));q.steps.forEach(function(step){steps.append(el('li',step));});details.append(el('p',q.answer),steps);area.append(details);
