(function(){
  ...
  let last=null;
  const translations={
    en:{sendCountry:'Sending country',receiveCountry:'Receiving country',corridorHelp:'Enter country names or two-letter codes. Only quotes for the same countries, currencies and total debit are ranked. Payout methods and delivery times may differ; this does not establish equivalent service.',payout:'Payout method',delivery:'Delivery (minutes)',expires:'Expires at',asOf:'Compared at',payouts:{bank:'Bank','mobile-wallet':'Mobile wallet',cash:'Cash','crypto-wallet':'Crypto wallet',other:'Other'},invalid:'Complete every quote with valid values.',updated:'Comparison calculated on this device.',copied:'Summary copied.',copyFailed:'Could not copy. Download JSON instead.',downloaded:'JSON downloaded.',highest:'Highest recipient amount',none:'No eligible comparable quotes.',debit:'Total debit',recipient:'Recipient amount',rate:'Effective rate',fee:'Stated fee',checked:'Checked',expiry:'Expiry state',difference:'Difference from highest',boundary:'Highest recipient amount is not a recommendation. Recheck the quote, expiry, payout route, safety and limits before sending.',expiryStates:{expired:'expired',unknown:'unknown','not-expired':'not expired'}},
    fr:{sendCountry:'Pays d’envoi',receiveCountry:'Pays de réception',corridorHelp:'Saisissez les pays ou leurs codes à deux lettres. Seuls les devis pour les mêmes pays, devises et total débité sont classés. Les modes et délais de réception peuvent différer : les services ne sont pas nécessairement équivalents.',payout:'Mode de réception',delivery:'Délai (minutes)',expires:'Expire le',asOf:'Comparé le',payouts:{bank:'Banque','mobile-wallet':'Portefeuille mobile',cash:'Espèces','crypto-wallet':'Portefeuille crypto',other:'Autre'},invalid:'Remplissez chaque devis avec des valeurs valides.',updated:'Comparaison calculée sur cet appareil.',copied:'Résumé copié.',copyFailed:'Impossible de copier. Téléchargez le fichier JSON.',downloaded:'Fichier JSON téléchargé.',highest:'Montant reçu le plus élevé',none:'Aucun devis admissible et comparable.',debit:'Total débité',recipient:'Montant du bénéficiaire',rate:'Taux effectif',fee:'Frais indiqués',checked:'Vérifié',expiry:'État d’expiration',difference:'Écart par rapport au montant le plus élevé',boundary:'Le montant reçu le plus élevé n’est pas une recommandation. Vérifiez le devis, son expiration, le mode de réception, la sécurité et les limites avant l’envoi.',expiryStates:{expired:'expiré',unknown:'inconnue','not-expired':'non expiré'}},
    sw:{sendCountry:'Nchi ya kutuma',receiveCountry:'Nchi ya kupokea',corridorHelp:'Weka majina ya nchi au misimbo ya herufi mbili. Nukuu hupangwa tu kwa nchi, sarafu na jumla inayokatwa zinazofanana. Njia na muda wa kupokea vinaweza kutofautiana; huduma si lazima ziwe sawa.',payout:'Njia ya kupokea',delivery:'Muda wa kufika (dakika)',expires:'Inaisha',asOf:'Ililinganishwa',payouts:{bank:'Benki','mobile-wallet':'Pochi ya simu',cash:'Taslimu','crypto-wallet':'Pochi ya kripto',other:'Nyingine'},invalid:'Jaza nukuu zote kwa thamani halali.',updated:'Ulinganisho umekokotolewa kwenye kifaa hiki.',copied:'Muhtasari umenakiliwa.',copyFailed:'Imeshindikana kunakili. Pakua JSON badala yake.',downloaded:'JSON imepakuliwa.',highest:'Kiasi kikubwa zaidi cha mpokeaji',none:'Hakuna nukuu halali zinazolingana.',debit:'Jumla inayokatwa',recipient:'Kiasi cha mpokeaji',rate:'Kiwango halisi',fee:'Ada iliyoandikwa',checked:'Ilikaguliwa',expiry:'Hali ya muda',difference:'Tofauti na kiwango cha juu',boundary:'Kiasi kikubwa si pendekezo. Kagua nukuu, uhalali, njia ya kupokea, usalama na mipaka kabla ya kutuma.',expiryStates:{expired:'imeisha',unknown:'haijulikani','not-expired':'haijaisha'}}
  };
  const t=translations[locale]||translations.en;
  ['a','b','c'].forEach(letter=>{const host=document.getElementById('rm-'+letter+'-label').closest('.rm-fields');['sendCountry','receiveCountry'].forEach(key=>{const label=document.createElement('label');label.className='rm-field';label.htmlFor='rm-'+letter+'-'+key;const text=document.createElement('span');text.textContent=t[key];const input=document.createElement('input');input.id=label.htmlFor;input.required=true;input.maxLength=80;input.disabled=letter==='c';label.append(text,input);host.append(label);});});
  const corridorHelp=document.createElement('p');corridorHelp.className='rm-privacy';corridorHelp.textContent=t.corridorHelp;form.prepend(corridorHelp);
  function value(id){return document.getElementById(id).value.trim();}
  function quote(letter){return{sendCountry:document.getElementById(`rm-${letter}-sendCountry`).value,receiveCountry:document.getElementById(`rm-${letter}-receiveCountry`).value,label:value(`rm-${letter}-label`),sendCurrency:value(`rm-${letter}-send`),totalDebit:value(`rm-${letter}-debit`),receiveCurrency:value(`rm-${letter}-receive`),recipientAmount:value(`rm-${letter}-recipient`),statedFee:value(`rm-${letter}-fee`),payoutMethod:value(`rm-${letter}-payout`),deliveryMinutes:value(`rm-${letter}-delivery`),observedAt:value(`rm-${letter}-observed`),expiresAt:value(`rm-${letter}-expires`)};}
  ...
  function calculate(event){
    ...
    for(let index=0;index<quotes.length;index++){try{window.RemittanceQuoteComparatorEngine.calculate({requireCorridor:true,asOf,quotes:[quotes[index],quotes[index]]});}catch(exception){return fail(document.getElementById('rm-'+['a','b','c'][index]+'-'+(errorFields[exception.message]||'label')));}}
    try{last=window.RemittanceQuoteComparatorEngine.calculate({requireCorridor:true,asOf,quotes});}
  ...
  function summary(result){return [t.asOf+': '+result.asOf,t.highest+': '+(result.hasEligibleComparison?result.groups.map((group)=>fmt(group.highestRecipientAmount,group.receiveCurrency)).join('; '):t.none),...result.quotes.map((row)=>[
    row.label,t.sendCountry+': '+row.sendCountry+' ('+row.sendCountryCode+')',t.receiveCountry+': '+row.receiveCountry+' ('+row.receiveCountryCode+')',t.debit+': '+fmt(row.totalDebit,row.sendCurrency),t.recipient+': '+fmt(row.recipientAmount,row.receiveCurrency),
