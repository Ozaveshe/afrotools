(function(){
 ...
 document.addEventListener('DOMContentLoaded',function(){
  ...
  function calculate(e){
   ...
   result={schemaVersion:1,toolId:'fuel-tracker',locale:locale,inputs:Object.assign({},input,{currency:currency,fuelType:byId('fuel-type').value}),calculation:calc,priceBasis:reference?'market-reference':'manual',market:reference?{id:selected.market_id,name:place(selected),countryCode:selected.country_code,countryName:country(selected.country_code,selected.country_name),granularity:selected.granularity,effectiveDate:record.effective_date,lastVerifiedAt:record.last_verified_at,sourceName:record.source_name,sourceUrl:record.source_url}:null};
  ...
  window.AfroTools.fuelFillReport=function(){if(!result)return null;if(result.priceBasis==='market-reference'&&!status(record).available){render();return null;}return JSON.parse(JSON.stringify(result));};
