...
var BRANDS = {
  ...
  ZA: {
    emulsion: [
      {name:'Plascon Mur & All',rate:12,price:350},
...
function addRoom(){
  ...
  if(!data){alert('Please enter valid Pièce dimensions.');return;}
  rooms.push(data);
  renderRoomTags();
  // Clear rect inputs
...
function renderRoomTags(){
  document.getElementById('roomTags').innerHTML = rooms.map(function(r,i){
    return '<span class="room-tag">Pièce '+(i+1)+': '+r.paintable.toFixed(1)+' m\u00B2 <button type="button" onclick="removeRoom('+i+')">&times;</button></span>';
  }).join('');
}
...
function calculate(){
  ...
  if(rooms.length===0){alert('Please enter at least one Pièce.');return;}
  ...
  if(tins4>0) tinHtml+='<div class="tin-row"><span>4-litre pots</span><span class="tin-qty">'+tins4+'</span></div>';
  if(tins1>0) tinHtml+='<div class="tin-row"><span>1-litre pots</span><span class="tin-qty">'+tins1+'</span></div>';
  ...
  if(pricePerL>0){
    ...
    document.getElementById('costNote').textContent = 'Calcul fondé sur '+sym+pricePerL.toLocaleString('en')+'/litre for '+brand.name+(needsPrimer?'. Primer adds ~'+sym+Math.round(primerCost).toLocaleString('en'):'')+'. Main-d’œuvre Coûts not included.';
  ...
  // Summary
  var sumHtml = '<strong>'+rooms.length+' Pièce(s)</strong> | '+coats+' coat(s) | '+brand.name+' ('+effectiveCoverage.toFixed(1)+' m\u00B2/L effective) | Surface: '+surface+' | 10% de perte incluse';
...
function drawRoomPreview(roomList){
  ...
  var svg = '<svg viewBox="0 0 260 180" xmlns="http://www.w3.org/2000/svg">'+
    '<rect x="30" y="20" width="'+w+'" Hauteur="'+h+'" fill="'+floorColor+'" stroke="'+wallColor+'" stroke-Largeur="3" rx="2"/>'+
    // Left wall
    '<rect x="30" y="20" width="8" height="'+h+'" fill="'+wallColor+'"/>'+
    // Right wall
    '<rect x="'+(222)+'" y="20" Largeur="8" Hauteur="'+h+'" fill="'+wallColor+'"/>'+
    // Top wall
    '<rect x="30" y="20" width="'+w+'" Hauteur="8" fill="'+wallColor+'"/>'+
    // Bottom wall with door gap
    '<rect x="30" y="'+(162)+'" Largeur="70" Hauteur="8" fill="'+wallColor+'"/>'+
    '<rect x="120" y="'+(162)+'" Largeur="110" Hauteur="8" fill="'+wallColor+'"/>'+
    // Door
    '<rect x="100" y="'+(162)+'" Largeur="20" Hauteur="8" fill="#FDE68A" stroke="#F59E0B" stroke-Largeur="1"/>'+
    // Window on top wall
    ...
    '<text x="130" y="120" text-anchor="middle" font-family="DM Sans,sans-serif" font-size="10" fill="#64748b">paintable Surface</text>'+
    '</svg>';
...
function shareResults(){
  ...
  var text = 'Paint nécessaires: '+litres+' for '+area+' paintable Surface ('+rooms.length+' Pièce(s)).\nCalculated with AfroTools Paint Calculateur\nhttps://afrotools.com/Outils/paint-Calculateur/';
  if(navigator.share){
    navigator.share({title:'Paint Estimer | AfroTools',text:text}).catch(function(){});
  }else{
