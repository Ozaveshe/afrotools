...
function addRoom() {
  ...
  if (!data) {
    alert("Please enter valid Pièce dimensions.");
    return;
  }
  rooms.push(data);
  renderRoomTags();
...
function calculate() {
  ...
  if (rooms.length === 0) {
    alert("Please enter at least one Pièce.");
    return;
  }
  ...
  if (paintResult.error) {
    alert("Please check the Pièce and paint Hypothèses.");
    return;
  }
  ...
  if (tins4 > 0)
    tinHtml +=
      '<div class="tin-row"><span>4-litre pots</span><span class="tin-qty">' +
      tins4 +
      "</span></div>";
  if (tins1 > 0)
    tinHtml +=
      '<div class="tin-row"><span>1-litre pots</span><span class="tin-qty">' +
      tins1 +
      "</span></div>";
  tinHtml +=
  ...
  if (pricePerL > 0) {
    ...
    document.getElementById("costNote").textContent =
      ...
      (needsPrimer
        ...
        : "") +
      ". Main-d’œuvre Coûts not included.";
...
function drawRoomPreview(roomList) {
  ...
  var svg =
    ...
    '<rect x="30" y="' +
    162 +
    '" Largeur="70" Hauteur="8" fill="' +
    wallColor +
    '"/>' +
    '<rect x="120" y="' +
    162 +
    '" Largeur="110" Hauteur="8" fill="' +
    wallColor +
    '"/>' +
    // Door
    '<rect x="100" y="' +
    162 +
    '" Largeur="20" Hauteur="8" fill="#FDE68A" stroke="#F59E0B" stroke-Largeur="1"/>' +
    // Window on top wall
...
function shareResults() {
  ...
  if (navigator.share) {
    navigator
      .share({ title: "Paint Estimer | AfroTools", text: text })
      .catch(function () {});
  } else {
