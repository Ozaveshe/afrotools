...
var SITE_FACTORS = {
  ...
  remote: {mult:1.15, label:'Remote or rainy access', risk:2, note:'Adds transport, weather, and idle-time risque.'}
};
...
var BREAKDOWN = [
  {cat:'Fondation & Substructure', pct:0.16},
  {cat:'Murs & Blockwork', pct:0.13},
  {cat:'Béton Frame, Columns, Beams, Slabs', pct:0.12},
  {cat:'Roofing', pct:0.10},
  {cat:'Portes & Fenêtres', pct:0.08},
  {cat:'Plancher finitions', pct:0.09},
  {cat:'Mur finitions & Painting', pct:0.07},
  {cat:'Plomberie & Drainage', pct:0.08},
  {cat:'Électricité Installation', pct:0.07},
  {cat:'Cuisine, Bath, Fixtures, Sundries', pct:0.10}
];
...
function suggestRooms() {
  ...
  rooms = [];
  if (type === 'bungalow') {
    rooms = [{name:'Living Pièce',w:5,l:5},{name:'Master Bedroom',w:4,l:4},{name:'Bedroom 2',w:3.5,l:3.5},{name:'Bedroom 3',w:3.5,l:3.5},{name:'Cuisine',w:3,l:3},{name:'Bathroom',w:2.5,l:2},{name:'Toilet',w:1.5,l:1.5},{name:'Corridor',w:6,l:1.5}];
  } else if (type === 'duplex') {
    rooms = [{name:'Living Pièce',w:5.5,l:5},{name:'Dining',w:4,l:3.5},{name:'Cuisine',w:3.5,l:3},{name:'Master Bedroom',w:4.5,l:4},{name:'Bedroom 2',w:3.5,l:3.5},{name:'Bedroom 3',w:3.5,l:3.5},{name:'Bedroom 4',w:3.5,l:3},{name:'Master Bath',w:3,l:2.5},{name:'Bathroom 2',w:2.5,l:2},{name:'Toilet (GF)',w:1.5,l:1.5},{name:'Staircase',w:3,l:1.5},{name:'Corridor',w:8,l:1.5}];
  } else if (type === 'commercial') {
    rooms = [{name:'Reception',w:6,l:4},{name:'Ouvrir Bureau',w:10,l:8},{name:'Manager Bureau',w:4,l:4},{name:'Meeting Pièce',w:5,l:4},{name:'Kitchenette',w:3,l:2.5},{name:'Server Pièce',w:3,l:2},{name:'Toilet Block',w:4,l:3},{name:'Corridor',w:10,l:2}];
  } else {
    rooms = [{name:'Living Pièce',w:5,l:5},{name:'Master Bedroom',w:4,l:4},{name:'Bedroom 2',w:3.5,l:3.5},{name:'Cuisine',w:3,l:3},{name:'Bathroom',w:2.5,l:2},{name:'Corridor',w:5,l:1.5}];
  }
  renderRooms();
}
...
function renderRooms() {
  ...
  grid.innerHTML = rooms.map(function(r, i) {
    var area = r.w * r.l;
    totalArea += area;
    return '<div class="room-card"><div class="room-name">' + r.name + '</div><div class="room-dim">' + r.w + ' x ' + r.l + ' m</div><div class="room-area">' + area.toFixed(1) + ' m2</div><button type="button" onclick="removeRoom(' + i + ')" style="font-size:.68rem;color:#dc2626;border:none;background:none;cursor:pointer;margin-top:4px">Supprimer</button></div>';
  }).join('');
...
function renderConfidence(scope, totalArea, grossArea, totalCost, c) {
  ...
  document.getElementById('estimateSummary').innerHTML =
    '<div><span>Estimer Stage</span><strong>' + scope.stage.label + '</strong></div>' +
    '<div><span>Expected Range</span><strong>' + scope.stage.range + '</strong></div>' +
    '<div><span>Gross Plancher Surface</span><strong>' + grossArea.toFixed(0) + ' m2</strong></div>' +
    '<div><span>Confidence</span><strong>' + confidence + '%</strong></div>';

  var notes = [
    'Measured Surface des pièces is ' + totalArea.toFixed(0) + ' m2; grossed up by ' + Math.round(scope.shellAllowancePct * 100) + '% for Murs, circulation, stairs, and unlisted service space.',
    scope.site.note,
    'Total includes preliminaries, external works, professional fees, Imprévus, and escalation as separate allowances.',
    'Excluded: land, demolition, statutory approvals, finance charges, loose Mobilier, Appareils, specialist equipment, and taxes not included in local rates.'
  ];
...
/* -- Calculate -- */
function calculate() {
  if (!rooms.length) { alert('Please add at least one Pièce.'); return; }
  ...
  // Breakdown table
  var html = '<thead><tr><th>Category</th><th>Basis</th><th class="num">Estimatif Coût (' + c.curr + ')</th></tr></thead><tbody>';
  html += '<tr class="cat-row"><td colspan="3">Informations et hypothèses du calcul</td></tr>';
  ...
  html += '<tr class="cat-row"><td colspan="3">Informations et hypothèses du calcul</td></tr>';
  [
    ['Preliminaries', scope.preliminariesPct, preliminaries, 'of Bâtiment works'],
    ...
    ['Imprévus', scope.contingencyPct, contingency, 'of Coût plan Sous-total'],
    ['Prix Escalation', scope.escalationPct, escalation, 'of Sous-total + Imprévus']
  ...
  document.getElementById('timelineCards').innerHTML =
    ...
    '<div class="tl-item"><div class="tl-label">toiture</div><div class="tl-value">' + Math.max(1, Math.round(months * 0.10)) + ' months</div></div>' +
    '<div class="tl-item"><div class="tl-label">finitions</div><div class="tl-value">' + Math.max(1, Math.round(months * 0.30)) + ' months</div></div>' +
...
async function exportPDF() {
  ...
  return window.AfroToolsEngineeringPdf.download({
    title: 'Estimation du coût de construction',
    subtitle: sub,
    heroStats: [
      { label: 'Total estimé', value: total, highlight: true },
      { label: 'Pièces', value: String(rooms.length) }
    ],
    sections: [
      { title: 'Détail des coûts', rows: breakdownRows },
      { title: 'Pièces', rows: roomRows }
    ],
    disclaimer: 'Estimation de planification uniquement. Faites vérifier les quantités et les prix contractuels par un métreur qualifié.',
...
document.addEventListener('click', function(event) {
  ...
  exportPDF().catch(function(error) {
    console.error('Plancher plan PDF Exporter failed:', error);
  });
}, true);
...
function shareResult() {
  var total = document.getElementById('grandTotal').textContent;
  var sub = document.getElementById('costPerSqm').textContent;
  var text = 'Estimation du coût de construction: ' + total + '\n' + sub + '\nEstimatif with AfroTools\nhttps://afrotools.com/Outils/Plancher-plan/';
  if (navigator.share) { navigator.share({title:'Bâtiment Coût | AfroTools',text:text}).catch(function(){}); }
