...
function calculate() {
  ...
  if (result.error) { alert('Please enter valid mix Hypothèses.'); return; }
  var agg = parts[2]||0;
  ...
  // Display
  document.getElementById('rCement').textContent = cementBags + ' sacs';
  ...
  if(country!=='none' && PRICES[country]){
    ...
    var html = '<tr><td>Ciment ('+bagSize+'kg sacs)</td><td>'+cementBags+'</td><td class="num">'+p.sym+p.cement.toLocaleString()+'</td><td class="num">'+p.sym+Math.round(cCost).toLocaleString()+'</td></tr>';
    html += '<tr><td>Sable</td><td>'+sandVol.toFixed(2)+' m\u00B3</td><td class="num">'+p.sym+p.sand.toLocaleString()+' /m\u00B3</td><td class="num">'+p.sym+Math.round(sCost).toLocaleString()+'</td></tr>';
    if(agg>0) html += '<tr><td>Granulat / Gravier</td><td>'+aggVol.toFixed(2)+' m\u00B3</td><td class="num">'+p.sym+p.gravel.toLocaleString()+' /m\u00B3</td><td class="num">'+p.sym+Math.round(gCost).toLocaleString()+'</td></tr>';
  ...
  if(grade.startsWith('M2')||grade.startsWith('M3')){
    document.getElementById('curingText').textContent = 'This grade requires minimum 14 jours of curing. Keep Béton moist with wet hessian, plastic sheeting, or curing compound. In hot weather (>35°C), Eau 3-4 times daily.';
  } else {
    document.getElementById('curingText').textContent = 'Cure for minimum 7 jours. Keep Béton moist — cover with wet hessian or plastic sheeting. Avoid direct sun exposure in the first 24 hours.';
  }
...
function shareResult(){
  ...
  var text = 'Béton: '+vol+' wet volume\nCiment: '+bags+'\nSable: '+sand+'\nCalculated with AfroTools\nhttps://afrotools.com/Outils/Béton-mix/';
  if(navigator.share){navigator.share({title:'Béton Mix | AfroTools',text:text}).catch(function(){});}
