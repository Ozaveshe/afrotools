(function () {
  ...
  function addRow(name, watts, quantity, hours) {
    ...
    row.id = "row-" + id;
    row.innerHTML =
      '<input aria-label="Appliance name" type="text" value="' +
      String(name || "")
        .replace(/&/g, "&amp;")
        .replace(/"/g, "&quot;") +
      '" placeholder="Appliance" style="font-weight:600">' +
      '<input aria-label="Watts per appliance" type="number" value="' +
      (watts || "") +
      '" min="1" placeholder="W">' +
      '<input aria-label="Quantity" type="number" value="' +
      (quantity || 1) +
      '" min="1" max="50">' +
      '<input aria-label="Hours used per day" type="number" value="' +
      (hours == null ? 8 : hours) +
      '" min="0" max="24" step="0.5">' +
      '<button type="button" class="remove-btn" aria-label="Remove appliance">×</button>';
  ...
  var HOME = [
    ...
    ["Ceiling Fan", 60, 4, 10],
    ['TV (LED 43")', 100, 1, 6],
    ["Refrigerator", 150, 1, 24],
    ["AC 1.5HP", 1120, 2, 8],
    ["Washing Machine", 500, 1, 1],
    ["Water Pump", 750, 1, 2],
    ["Electric Iron", 1000, 1, 0.5],
    ["Microwave", 1000, 1, 0.5],
    ["Phone Charger", 15, 4, 3],
  ...
  var OFFICE = [
    ...
    ["AC 2HP", 1500, 3, 10],
    ["Desktop Computer", 300, 8, 10],
    ["Monitor", 40, 8, 10],
    ["Printer", 500, 2, 2],
    ["Router / Modem", 15, 2, 24],
    ["Water Dispenser", 100, 1, 12],
    ["Refrigerator", 150, 1, 24],
    ["Microwave", 1000, 1, 0.5],
  ...
  function recommendation(result) {
    if (result.recommendation === "three-phase-review")
      return [
        "red",
        "Three-phase review required",
        "The planning demand exceeds the calculator's 13 kW single-phase screen. Ask the utility and a licensed electrician to assess the supply.",
      ];
    if (result.recommendation === "single-phase-limit")
      return [
        "amber",
        "Approaching the single-phase screen",
  ...
  function render(result) {
    ...
    document.getElementById("loadBars").innerHTML =
      '<h4 style="font-size:.82rem;font-weight:700;margin-bottom:8px">Load distribution</h4>' +
    ...
    document.getElementById("genSection").innerHTML =
      '<div class="cost-note"><strong>Generator planning prompt:</strong> ' +
      result.generatorKva +
    ...
    document.getElementById("costSection").innerHTML =
      ...
      "/month</strong>. Confirm the current tariff on the latest bill.</div>";
    var warnings = [];
    if (result.breakerRangeExceeded)
      warnings.push(
        "The required breaker prompt exceeds the calculator's 160A table.",
      );
    if (result.generatorRangeExceeded)
      warnings.push(
        "The generator prompt exceeds the calculator's 100 kVA table.",
      );
    ...
    document.getElementById("electricalStatus").textContent =
      "Planning estimate ready. Verify every selection with a licensed electrician.";
  }
  function calculate() {
    try {
      ...
      document.getElementById("electricalStatus").textContent =
        "Check every appliance name, wattage, quantity and daily-use value.";
      return null;
    }
  }
  function brief() {
    ...
    if (!r) return "";
    return [
      "AfroTools electrical load planning brief",
      "Profile: " +
        r.profile.name +
        ", " +
        r.profile.voltage +
      ...
      "Monthly assumption: " +
        ...
        r.profile.symbol +
        r.monthlyCost,
      "Boundary: undated static assumptions with low confidence; licensed-electrician verification required.",
    ].join("\n");
  }
  function csv() {
    var r = lastResult || calculate();
    ...
    rows.push(
      ...
      [
        "confidence",
        "low; undated static assumptions; electrician verification required",
      ],
    );
    var value = rows
      .map(function (row) {
  ...
  window.copyElectricalBrief = function () {
    ...
    document.getElementById("electricalStatus").textContent =
      "Electrical planning brief copied.";
  };
  window.shareResult = function () {
    ...
    if (navigator.share)
      navigator
        .share({ title: "Electrical Load | AfroTools", text: value })
        .catch(function () {});
