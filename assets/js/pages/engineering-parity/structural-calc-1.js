...
function matCost(concreteM3,rebarKg,note){
  ...
  return '<div class="cost-box"><strong>\ud83d\udcb0 Estimated Material Cost \u2014 '+c.name+'</strong><br>'
    +'Concrete ('+concreteM3.toFixed(2)+' m\u00b3): '+fmt(concCost)+'<br>'
    +'Rebar (~'+Math.round(rebarKg)+' kg): '+fmt(rebarCost)+'<br>'
    +'<strong>Total: '+fmt(total)+'</strong>'+(note?'<br><em>'+note+'</em>':'')+'<br>'
    +'<span style="font-size:.78rem;color:#92400e">Indicative rates only. Prices vary by region and supplier. Select another currency above to compare.</span></div>';
}
...
function calcBeam(){
  ...
  document.getElementById('beam-summary').innerHTML=`
    <div class="result-card highlight"><div class="result-label">Beam Size</div><div class="result-value">${b} x ${h}<span class="result-unit"> mm</span></div></div>
    ...
    <div class="result-card highlight"><div class="result-label">Main Steel</div><div class="result-value">${As.toFixed(0)}<span class="result-unit"> mm2</span></div></div>
  `;

  document.getElementById('beam-table').innerHTML=`
    <thead><tr><th>Parameter</th><th>Value</th></tr></thead>
    <tbody>
    <tr><td>Effective depth (d)</td><td>${d} mm</td></tr>
    <tr><td>K factor</td><td>${K.toFixed(4)} ${compression?'> 0.156 (COMPRESSION STEEL NEEDED)':''}</td></tr>
    <tr><td>Lever arm (z)</td><td>${z.toFixed(1)} mm</td></tr>
    <tr><td>Required As</td><td>${As.toFixed(0)} mm2</td></tr>
    <tr><td>Min As (0.13%bh)</td><td>${AsMin.toFixed(0)} mm2</td></tr>
    <tr><td>Suggested bars (bottom)</td><td>${barChoice}</td></tr>
  ...
  document.getElementById('beam-cost').innerHTML=matCost(beamConc,beamRebar,'Per beam element (section \xd7 span)');
}
...
function calcColumn(){
  ...
  document.getElementById('col-summary').innerHTML=`
    <div class="result-card highlight"><div class="result-label">Column Size</div><div class="result-value">${dimLabel}</div></div>
    <div class="result-card"><div class="result-label">Req. Gross Area</div><div class="result-value">${Math.round(Ag).toLocaleString()}<span class="result-unit"> mm2</span></div></div>
    <div class="result-card"><div class="result-label">Steel Area</div><div class="result-value">${Asc.toLocaleString()}<span class="result-unit"> mm2</span></div></div>
  ...
  document.getElementById('col-table').innerHTML=`
    <thead><tr><th>Parameter</th><th>Value</th></tr></thead>
    <tbody>
    <tr><td>Design load</td><td>${N} kN</td></tr>
    <tr><td>Concrete grade</td><td>C${fcu}</td></tr>
    <tr><td>Steel percentage</td><td>${(pSteel*100).toFixed(1)}%</td></tr>
    ...
    <tr><td>Capacity check</td><td>${(0.4*fcu*(actualAg-Asc)/1000+0.8*fy*Asc/1000).toFixed(0)} kN ${(0.4*fcu*(actualAg-Asc)/1000+0.8*fy*Asc/1000)>=N?'> '+N+' OK':'< '+N+' INCREASE SIZE'}</td></tr>
    </tbody>`;
  document.getElementById('col-results').style.display='block';
  var colH=3.0;
  var colConc=(actualAg/1e6)*colH;
  var colRebar=Asc*1e-6*colH*7850*1.15;
  document.getElementById('col-cost').innerHTML=matCost(colConc,colRebar,'Based on 3 m storey height. Adjust for actual height.');
}
...
function calcSlab(){
  ...
  document.getElementById('slab-summary').innerHTML=`
    ...
    <div class="result-card"><div class="result-label">Ultimate Load</div><div class="result-value">${n.toFixed(1)}<span class="result-unit"> kN/m2</span></div></div>
    ...
    <div class="result-card highlight"><div class="result-label">Main Steel</div><div class="result-value" style="font-size:1rem">${barChoice}</div></div>
  `;
  ...
  document.getElementById('slab-table').innerHTML=`
    ...
    <tr><td>Effective depth</td><td>${d} mm</td></tr>
    <tr><td>Required As</td><td>${Math.round(AsDesign)} mm2/m</td></tr>
    <tr><td>Min As (0.13%)</td><td>${Math.round(AsMin)} mm2/m</td></tr>
    <tr><td>Distribution steel</td><td>Y10 @ 300 mm c/c (min)</td></tr>
    </tbody>`;
  ...
  document.getElementById('slab-cost').innerHTML=matCost(slabConcPerM2,slabRebarPerM2,'Cost per m\u00b2 of slab \u2014 multiply by total floor area');
}
...
function calcFooting(){
  ...
  document.getElementById('foot-summary').innerHTML=`
    <div class="result-card highlight"><div class="result-label">Footing Size</div><div class="result-value">${side}x${side}<span class="result-unit"> mm</span></div></div>
    <div class="result-card"><div class="result-label">Depth</div><div class="result-value">${Math.ceil(h/25)*25}<span class="result-unit"> mm</span></div></div>
  ...
  document.getElementById('foot-table').innerHTML=`
    <thead><tr><th>Parameter</th><th>Value</th></tr></thead>
    <tbody>
    <tr><td>Column load</td><td>${N} kN</td></tr>
    <tr><td>Bearing capacity</td><td>${sbc} kN/m2</td></tr>
    <tr><td>Required area</td><td>${areaReq.toFixed(2)} m2</td></tr>
    <tr><td>Provided area</td><td>${actualArea.toFixed(2)} m2</td></tr>
    <tr><td>Cantilever length</td><td>${(cantilever*1000).toFixed(0)} mm</td></tr>
    <tr><td>Concrete volume</td><td>${(actualArea*Math.ceil(h/25)*25/1000).toFixed(2)} m3</td></tr>
    </tbody>`;
