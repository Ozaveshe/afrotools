...
function estimate(){
  ...
  let html='';
  html+=`<div class="result-box highlight"><div class="num">${fmt(total)}</div><div class="lbl">Estimated Total Cost</div></div>`;
  html+=`<div class="result-box"><div class="num">${depth}m</div><div class="lbl">Drilling Depth</div></div>`;
  html+=`<div class="result-box"><div class="num">${fmt(drilling)}</div><div class="lbl">Drilling Cost Only</div></div>`;
  html+=`<div class="result-box"><div class="num">${fmt(data.drillingPerM[geo])}/m</div><div class="lbl">Cost per Metre</div></div>`;
  document.getElementById('resultGrid').innerHTML=html;

  let tbl='<tr><th>Item</th><th>Cost</th></tr>';
  tbl+=`<tr><td>Hydrogeological Survey</td><td>${fmt(survey)}</td></tr>`;
  tbl+=`<tr><td>Mobilization / Site Prep</td><td>${fmt(mobilization)}</td></tr>`;
  ...
  tbl+=`<tr><td>Gravel Pack</td><td>${fmt(gravel)}</td></tr>`;
  ...
  if(plumbing>0)tbl+=`<tr><td>Plumbing & Installation</td><td>${fmt(plumbing)}</td></tr>`;
  if(tank>0)tbl+=`<tr><td>Water Tank (${tankSize}L)</td><td>${fmt(tank)}</td></tr>`;
  tbl+=`<tr><td>Water Quality Test</td><td>${fmt(waterTest)}</td></tr>`;
  tbl+=`<tr class="total"><td>TOTAL ESTIMATE</td><td>${fmt(total)}</td></tr>`;
