(function () {
  ...
  function engine(path) {
    ...
    if (!value) throw new Error("Le propriétaire de calcul n’est pas disponible.");
    return value;
  }
  ...
  const runners = {
    ...
    gpa(input, api) {
      ...
      const result = api.calculateSemester([
        { name: "Cours 1", credits: Number(input.credits1), value: Number(input.points1) },
        { name: "Cours 2", credits: Number(input.credits2), value: Number(input.points2) }
      ], template);
    ...
    percentage(input, api) {
      ...
      result.modeFrench = input.mode === "percentageOf" ? "Part d’un total" : input.mode === "change" ? "Variation" : "Pourcentage d’une valeur";
    ...
    kcse(input, api) {
      const others = ["other1", "other2", "other3", "other4", "other5"].map((key, index) => ({ subject: `Matière ${index + 1}`, grade: input[key] }));
    ...
    async scholarship(input, api) {
      ...
      const fallback = {
        ...
        meta: { mode: "fallback", label: "Catalogue sélectionné (secours)", isDegraded: true }
      };
      ...
      if (!matches.length) return { ok: false };
      const top = matches[0];
      const categoryLabels = { "Strong Match": "Forte", "Good Match": "Bonne", Possible: "Possible", Unlikely: "Faible" };
      return {
        ok: true,
        ...
        topCategoryFrench: categoryLabels[top.category] || top.category,
  ...
  function display(value) {
    ...
    const translations = {
      "Bachelor's minimum": "Minimum pour une licence",
      "Timed practice": "Entraînement chronométré",
      upcoming: "À venir",
      met: "Respectée",
      one: "Une solution",
      decimal: "Décimal",
      DEG: "Degrés",
      stretch: "Budget sous tension",
      "within-half-band": "À moins de 0,5 point",
      "admission or further study": "Admission ou poursuite d’études",
      "The receiving education institution, with SAQA evaluation where required": "L’établissement destinataire, avec une évaluation SAQA si elle est requise",
      "Area per learner": "Surface par apprenant",
      below: "Sous le minimum",
      "APA 7 draft. The engine preserves the capitalisation you enter; apply APA sentence case and source-specific exceptions before submitting.": "Brouillon APA 7. Le moteur conserve les majuscules saisies; appliquez la casse de phrase APA et les exceptions propres à la source avant remise."
    };
  ...
  function render(result) {
    ...
    statusNode.textContent = "Scénario calculé localement. Vérifiez les hypothèses et le propriétaire officiel avant d’agir.";
  }
  ...
  async function run() {
    ...
    try {
      ...
      current = { schemaVersion: 1, locale: "fr", toolId: config.id, title: config.title, checkedAt: new Date().toISOString(), inputs: input, result: output, boundary: "Planification locale uniquement; aucune décision officielle." };
      render(output);
    } catch (_) {
      current = null;
      resultPanel.classList.remove("show");
      statusNode.className = "status error";
      statusNode.textContent = "Vérifiez les champs, les limites et les dates saisies. Aucun résultat n’a été accepté.";
    }
  }

  function textReport() {
    const lines = [current.title, "", "Résultat local"];
  ...
  function csvReport() {
    ...
    return ["Champ,Valeur"].concat(config.metrics.map(([path, label]) => `${quote(label)},${quote(display(pathValue(current.result, path)))}`)).join("\n");
  }
  ...
  function download(content, type, extension) {
    ...
    link.href = url;
    link.download = `${config.id}-fr.${extension}`;
    link.click();
  ...
  document.querySelectorAll("[data-action]").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      try {
        ...
        if (action === "save") localStorage.setItem(`afrotools:fr-education:${config.id}`, safeJson(current));
        if (action === "print") window.print();
        statusNode.textContent = action === "save" ? "Résultat enregistré uniquement sur cet appareil." : "Action terminée localement.";
      } catch (_) {
        statusNode.className = "status error";
        statusNode.textContent = "Cette action locale est indisponible dans ce navigateur.";
      }
    });
  });
