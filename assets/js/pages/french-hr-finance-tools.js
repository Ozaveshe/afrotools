(function () {
  ...
  function rowsFor(result) {
    ...
    if (mode === 'leave') {
      ...
      return [
        ['Pays', result.country],
        [country.value === 'CI' ? 'Base annuelle non arrondie' : 'Congé annuel légal', result.annualLeave.days + ' jours'],
        ...
        ...(country.value === 'GH' ? [['Conditions annuelles', 'Loi 651, articles 20–21 : service continu dans l’année civile, ou au moins 200 jours si le travail n’est pas régulièrement assuré. Entreprise employant uniquement la famille de l’employeur exclue (article 32). Applicabilité actuelle à confirmer.']] : []),
        ['Congé paternité', country.value === 'GH' ? 'Règle générale actuelle non vérifiée. Instructions de la fonction publique 2020, instruction 324 : maximum distinct de 10 jours ouvrés sous conditions, non applicable à tous.' : country.value === 'CI' ? 'Permission pour naissance : 2 jours ouvrables sous conditions (article 25.12). Six mois de présence, plafond familial annuel, autorisation et justificatifs : utilisez le vérificateur ci-dessous.' : dataSet()[country.value].paternityLeave && typeof dataSet()[country.value].paternityLeave.days === 'number' ? dataSet()[country.value].paternityLeave.days + ' jours' : 'Donnée absente — à vérifier'],
