(function(){
  ...
  var words={
    ...
    fr:{required:"Renseignez le scénario et chaque devis activé avec les valeurs des écrans du prestataire que vous avez vérifiés.",costs:"Les coûts de vente saisis ne peuvent pas dépasser le produit brut.",updated:"Comparaison mise à jour.",copied:"Résumé copié.",downloaded:"Téléchargement créé.",pdfError:"L’export PDF est indisponible. Utilisez l’impression.",title:"Comparaison de devis P2P exécutables",buyTotal:"Total fiat payé",sellTotal:"Montant fiat net reçu",effective:"Prix effectif",gross:"Valeur brute du devis",cost:"Coûts saisis",difference:"Écart par rapport à la cible observée",observedBuy:"Total saisi le plus bas",observedSell:"Produit saisi le plus élevé",checked:"Vérifié",reference:"Référence",boundary:"Comparaison arithmétique de devis saisis par l’utilisateur uniquement. Confirmez le devis exécutable, les limites, le moyen de paiement, le marchand et les frais avant d’agir.",buy:"ACHAT",sell:"VENTE",price:"Prix saisi"},
    sw:{required:"Jaza hali na kila bei iliyowezeshwa kwa thamani kutoka skrini ya mtoa huduma uliyokagua.",costs:"Gharama za uuzaji ulizoingiza haziwezi kuzidi mapato ghafi.",updated:"Ulinganisho umesasishwa.",copied:"Muhtasari umenakiliwa.",downloaded:"Upakuaji umeundwa.",pdfError:"PDF haipatikani. Tumia uchapishaji badala yake.",title:"Ulinganisho wa bei tekelezekaji za P2P",buyTotal:"Jumla ya fiat inayolipwa",sellTotal:"Fiat halisi inayopokelewa",effective:"Bei halisi",gross:"Thamani ghafi ya bei",cost:"Gharama zilizoingizwa",difference:"Tofauti na lengo lililoonekana",observedBuy:"Jumla ndogo zaidi iliyoingizwa",observedSell:"Mapato makubwa zaidi yaliyoingizwa",checked:"Ilikaguliwa",reference:"Rejea",boundary:"Ulinganisho wa kihesabu wa bei zilizoingizwa na mtumiaji pekee. Thibitisha bei inayotekelezeka, vikomo, njia ya malipo, mfanyabiashara na ada moja kwa moja kabla ya kuchukua hatua.",buy:"NUNUA",sell:"UUZA",price:"Bei iliyoingizwa"}
  }[locale];
  ...
  function formatter(){return new Intl.NumberFormat(locale==="fr"?"fr-FR":locale==="sw"?"sw-KE":"en",{maximumFractionDigits:6});}
  ...
  function render(result){
    ...
    result.quotes.forEach(function(row){
      ...
      meta.textContent=words.checked+": "+new Intl.DateTimeFormat(locale==="fr"?"fr-FR":locale==="sw"?"sw-KE":"en",{dateStyle:"medium",timeStyle:"short"}).format(new Date(row.checkedAt))+(row.reference?" · "+words.reference+": "+row.reference:"");
      card.append(head,metrics,meta);list.appendChild(card);
    });
    $("p2-results").removeAttribute("aria-busy");
  }
  function summary(data){
    var r=data.result,lines=[words.title,(r.side==="buy"?(words.buy||"BUY"):(words.sell||"SELL"))+" "+formatter().format(r.assetAmount)+" "+r.assetLabel+" / "+r.fiatCode,(r.side==="buy"?words.observedBuy:words.observedSell)+": "+amount(r.observedTargetFiat,r.fiatCode)];
    r.quotes.forEach(function(q){lines.push("",q.label,words.checked+": "+q.checkedAt,(q.reference?words.reference+": "+q.reference:""),(words.price||"Price entered")+": "+amount(q.priceFiatPerAsset,r.fiatCode)+" / "+r.assetLabel,words.gross+": "+amount(q.grossFiat,r.fiatCode),words.cost+": "+amount(q.totalCostsFiat,r.fiatCode),(r.side==="buy"?words.buyTotal:words.sellTotal)+": "+amount(q.settlementFiat,r.fiatCode),words.effective+": "+amount(q.effectivePriceFiatPerAsset,r.fiatCode)+" / "+r.assetLabel,words.difference+": "+amount(q.differenceFromObservedTargetFiat,r.fiatCode));});
