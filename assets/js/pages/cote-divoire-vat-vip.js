(function () {
  ...
  function money(value) {
    return new Intl.NumberFormat(locale.numberLocale || "fr-CI", {
      style: "currency",
      currency: "XOF",
  ...
  function pdfMoney(value) {
    return "XOF " + new Intl.NumberFormat(locale.numberLocale || "fr-CI", {
      minimumFractionDigits: 0,
      maximumFractionDigits: 0,
  ...
  function renderClassification(key) {
    var result = engine.classify(key);
    id("civClassificationResult").className = "civ-treatment " + result.treatment;
    id("civClassificationResult").textContent = t("classification_" + key, result.note);
  }
  function applyTreatment(kind, classificationKey) {
    if (state.rateKind !== kind) id("civEvidence").checked = false;
    state.rateKind = kind;
    document.querySelectorAll("[data-civ-rate]").forEach(function (item) {
      item.setAttribute("aria-pressed", String(item.dataset.civRate === kind));
    });
    if (classificationKey) id("civClassification").value = classificationKey;
    showEvidence();
    renderClassification(classificationKey || kind);
    calculate();
  }
  function calculate() {
    try {
  ...
  document.querySelectorAll("[data-civ-rate]").forEach(function (button) { button.addEventListener("click", function () { applyTreatment(button.dataset.civRate, button.dataset.civRate); }); });
  ...
  id("civClassification").addEventListener("change", function (event) {
    var classificationKey = event.target.value;
    applyTreatment(classificationKey === "review" ? "standard" : classificationKey, classificationKey);
  });
  ...
  id("civPdf").addEventListener("click", function () {
    ...
    [[t("net", "Amount before tax"), state.result.net], ["VAT " + state.result.rate + "%", state.result.vat], [t("gross", "Total including tax"), state.result.gross]].forEach(function (row, index) { doc.text(row[0], 48, 120 + index * 28); doc.setFont("helvetica", "bold"); doc.text(pdfMoney(row[1]), 300, 120 + index * 28); doc.setFont("helvetica", "normal"); });
