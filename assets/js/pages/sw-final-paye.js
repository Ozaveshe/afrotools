(function () {
  ...
  function initPaye(){
    ...
    q('[data-copy]').addEventListener('click',async function(){if(!result){announce('Kokotoa kwanza.',true);return;}var shown=displayedResult();var text='PAYE '+shown.country+' ('+periodLabel(shown.period)+'): neti '+money(shown.net,shown.currency)+', kodi '+money(shown.tax,shown.currency)+', mchango '+money(shown.contribution,shown.currency);try{if(!navigator.clipboard||typeof navigator.clipboard.writeText!=='function')throw new Error('clipboard-unavailable');await navigator.clipboard.writeText(text);announce('Muhtasari umenakiliwa.');}catch(error){announce('Imeshindikana kunakili. Pakua muhtasari kama TXT badala yake.',true);}});
