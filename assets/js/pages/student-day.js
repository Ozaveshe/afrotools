(function(){
 ...
 function mount(host){
  ...
  function render(){
   ...
   if(active){var focus=node('section','','sd-focus');focus.tabIndex=-1;focus.append(node('h3','Continue: '+active.subject));focus.append(node('p','Spend '+active.minutes+' minutes studying. Practise a question, check your reasoning, then mark this session complete.'));var paths=node('div','','sd-actions');paths.append(link(active.deckId?'Review this deck':'Open my flashcards','/tools/flashcard-maker/'+(active.deckId?'#review='+encodeURIComponent(active.deckId):'')),link('JAMB past questions','/jamb/past-questions/'));paths.append(button('Finish this session',function(){save(api.change(state,active.id,'done'),'Session complete. Your next task is ready.');}));focus.append(paths);host.append(focus);}
   ...
   var footer=node('div','','sd-actions');footer.append(link('Edit my weekly timetable','/tools/study-planner/#daily-study'),link('Open Education Hub','/tools/education-hub/#daily-study'));
