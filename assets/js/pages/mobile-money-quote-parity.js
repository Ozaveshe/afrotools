(function(){
  ...
  function expiryLabel(state){return fr ? (frenchExpiry[state] || 'État inconnu') : state;}
  function frenchError(message,fields){
  ...
  function clear(){last=null;output.replaceChildren();document.getElementById('mm-primary-label').textContent='';document.getElementById('mm-primary-value').textContent='—';status.textContent='';if(fr){error.textContent='';error.dataset.show='false';}form.querySelectorAll('[aria-invalid="true"]').forEach(node=>node.removeAttribute('aria-invalid'));}
  ...
  function calculate(event){if(event)event.preventDefault();if(!form.checkValidity()){const fields=Array.from(form.querySelectorAll(fr?'input:invalid,select:invalid':':invalid'));clear();fields.forEach(field=>field.setAttribute('aria-invalid','true'));if(fields[0])fields[0].focus();error.textContent=t.invalid;error.dataset.show='true';status.textContent=t.invalid;if(!fr)form.reportValidity();return null;}const quotes=[quote('a'),quote('b')];if(document.getElementById('mm-third').checked)quotes.push(quote('c'));
    const markets=fr?quotes.map(row=>marketIdentifier(row.market)):[];
    if(fr && markets.some(market=>!market)){
      frenchError('Pays non reconnu. Choisissez un pays africain de la liste ou son code à deux lettres.',Array.from(form.querySelectorAll('input[id$="-market"]')).filter(field=>!field.disabled&&!marketIdentifier(field.value)));return null;
    }
    if(fr && new Set(markets).size!==1){
      frenchError('Choisissez le même pays ou marché pour tous les devis. Une devise commune ne suffit pas à rendre deux pays comparables.',Array.from(form.querySelectorAll('input[id$="-market"]')).filter(field=>!field.disabled));return null;
    }
    quotes.forEach(row=>{const id=marketIdentifier(row.market);if(id)row.marketId=id;});
    try{last=window.MobileMoneyQuoteEngine.calculate({asOf:new Date().toISOString(),quotes});}catch(exception){
      if(fr){const detail=frenchErrors[exception.message];const fields=detail?Array.from(form.querySelectorAll('input[id$="-'+detail[1]+'"]')).filter(field=>!field.disabled):[form.querySelector('input')];frenchError(detail?detail[0]:t.invalid,fields);return null;}
      const field=form.querySelector('input');clear();if(field){field.setAttribute('aria-invalid','true');field.focus();}error.textContent=t.invalid;error.dataset.show='true';status.textContent=t.invalid;return null;}error.textContent='';error.dataset.show='false';render(last);status.textContent=t.updated;return last;}
  function summary(result){if(fr)return [t.lowest+': '+(result.hasEligibleComparison?result.groups.map(group=>fmt(group.lowestTotalFee,group.currency)).join('; '):t.none),...result.quotes.map(row=>`${row.label} (${row.market}): ${t.amount} ${fmt(row.amount,row.currency)}; ${t.send} ${fmt(row.senderFee,row.currency)}; ${t.receive} ${fmt(row.recipientFee,row.currency)}; ${t.total} ${fmt(row.totalFee,row.currency)}; ${t.checked} ${new Date(row.observedAt).toLocaleString(localeTag())}; ${t.expiry} ${expiryLabel(row.expiryState)}`),t.boundary].join('\n');return [t.lowest+': '+(result.hasEligibleComparison?result.groups.map(group=>fmt(group.lowestTotalFee,group.currency)).join('; '):t.none),...result.quotes.map(row=>`${row.label}: ${t.total} ${fmt(row.totalFee,row.currency)}; ${t.expiry} ${expiryLabel(row.expiryState)}`),t.boundary].join('\n');}function ensure(){return calculate();}
  document.getElementById('mm-copy').addEventListener('click',()=>{const result=ensure();if(fr){
    if(!result)return;
    if(!navigator.clipboard){status.textContent='La copie est indisponible dans ce navigateur. Téléchargez le fichier JSON pour conserver le résumé.';return;}
    navigator.clipboard.writeText(summary(result)).then(()=>{status.textContent=t.copied;}).catch(()=>{status.textContent='La copie a été refusée. Téléchargez le fichier JSON pour conserver le résumé.';});
