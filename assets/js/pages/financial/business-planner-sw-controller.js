(function(){
  ...
  $("[data-advisor]").addEventListener('click',async function(){
    ...
    try{var res=await fetch('/.netlify/functions/ai-advisor',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({toolId:'business-planner',messages:[{role:'user',content:prompt}]})});var data=await res.json();if(!res.ok||data.error)throw new Error(data.error||'AI error');usage.count+=1;try{localStorage.setItem('afrotools_business_advisor_usage',JSON.stringify(usage));}catch(_e){}out.textContent=data.content||data.response||data.message||'Hakuna jibu lililopokelewa.';}catch(_e){out.textContent='Mshauri wa AI hapatikani kwa sasa. Mpango wa hatua uliopo juu bado unafanya kazi ndani ya kivinjari.';}finally{this.disabled=false;}
  });
