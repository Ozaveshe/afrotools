// Readable owner recovered from 5ac8da46; canonical two-pass minification matched b027ff71 before scoped repairs.
!function () {
  "use strict";
  ...
  function updateControls() {
    ...
    els.downloadBtn.disabled = state.busy || !state.download;
  ...
  async function exportRedacted() {
    ...
    if (!review || !review.checked) {
      showResult("Review required", "Confirm the final review checkbox before exporting the flattened redacted PDF.");
  ...
  function downloadFile() {
    var review = byId("reviewConfirm");
    if (!state.download || state.busy || !review || !review.checked) return;
    ...
    // Redacted documents remain local and downloadable by guests.
    runDownload();
  }
  ...
  function bindControls() {
    ...
    els.qualitySelect.addEventListener("change", function () { resetResult(""); updateControls(); });
    (els.patternButtons || []).forEach(function (button) {
