...
(function (window) {
  ...
  function trackEducationPractice(exam, subject, action) {
    ...
    if (!marker) {
      ...
      var started = send('education_practice_cohort_started', {
        cohort_day_utc: new Date(day * DAY_MS).toISOString().slice(0, 10),
        cohort_exam: exam, cohort_subject: safeSubject, action: action
      });
      if (!started) { try { window.localStorage.removeItem(EDUCATION_COHORT_KEY); } catch (_) {} }
      return started;
    }
    ...
    var returned = send('education_practice_returned', {
      cohort_day_utc: new Date(marker.day * DAY_MS).toISOString().slice(0, 10),
      cohort_exam: marker.exam, cohort_subject: marker.subject,
      return_exam: exam, return_subject: safeSubject, return_day: elapsedDays
    });
    if (!returned) {
      marker.returned = false;
      try { window.localStorage.setItem(EDUCATION_COHORT_KEY, JSON.stringify(marker)); } catch (_) {}
    }
    return returned;
  }
  ...
  var analytics = {
    ...
    trackEducationPractice: trackEducationPractice,
