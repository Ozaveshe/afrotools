(function () {
  ...
  function bindEvents() {
    ...
    document.addEventListener('keydown', function (event) {
      if (!source || isTyping(event.target)) {
        return;
      }
      var direction = null;
  ...
  function applyTarget(key, resetCrop) {
    ...
    activeTarget = key;
    els.targetButtons.forEach(function (button) {
      var isActive = button.dataset.cropTarget === key;
  ...
  function getExportDimensions() {
    ...
    width = clampNumber(width, 1, 12000);
    height = clampNumber(height, 1, 12000);
  ...
  function exportCrop() {
    ...
    canvas.toBlob(function (blob) {
      ...
      if (lastOutput && lastOutput.url) {
        URL.revokeObjectURL(lastOutput.url);
      }
  ...
  function copyRecipe() {
    ...
    var dims = getExportDimensions();
    var target = TARGETS[activeTarget];
    var recipe = [
  ...
  function onPointerLeave() {
    if (!drag) {
      els.stageCanvas.style.cursor = 'crosshair';
    }
  }
  ...
  function getCanvasPoint(event) {
    var rect = els.stageCanvas.getBoundingClientRect();
    var scaleX = els.stageCanvas.width / rect.width;
    var scaleY = els.stageCanvas.height / rect.height;
    return {
  ...
  function updateQualityLabel() {
    ...
    els.qualityValue.textContent = (els.quality.value || 88) + '%';
  }

  function updateButtons() {
    var hasSource = !!source;
    [els.rotateLeft, els.rotateRight, els.flipH, els.flipV, els.reset, els.exportBtn].forEach(function (button) {
  ...
  function updateDetails() {
    ...
    var dims = getExportDimensions();
    els.detailSelection.textContent = formatPixels(Math.round(crop.w), Math.round(crop.h));
